...
Cry_HsmDecryptConfigType  Cry_HsmDecryptConfig =
        {.TimeOut = CRY_HSM_DECRYPT_ALL_TIMEOUT};

static Cry_HsmPrimitiveIdType Cry_PRMIDDecrypt = CRY_HSM_SYM_gDECRYPT;
...
** Traceability    :                                                          **
**                                                                            **
** Syntax          :Csm_ReturnType Cry_HsmDecrypt                             **
**                  (const void *CfgPtr,const Csm_SymKeyType *KeyPtr,         **
**                   const uint8 *IV,Hsm_CipherAlgorithmType Algorithm_Type,  **
**                   Crypto_AlgorithmModeType mode,const uint8 *DataPtr,      **
**                   uint32 DataLength ,  uint8 *ResultPtr, uint32 *ResultLength, **
**                   Hsm_CryptoOperationMode OperationMode)                   **
**                                                                            **
** Description     : sm4/aes256 decrypt interface                             **
**                                                                            **
**                                                                            **
** Service ID      : CRY_HSM_SID_DECRYPT                                      **
**                                                                            **
...
Csm_ReturnType  Cry_HsmDecrypt(const void *CfgPtr,const Csm_SymKeyType *KeyPtr,const uint8 *IV,Hsm_CipherAlgorithmType Algorithm_Type,
                                   Crypto_AlgorithmModeType mode,const uint8 *DataPtr, uint32 DataLength, uint8 *ResultPtr,uint32 *ResultLength,Hsm_CryptoOperationMode OperationMode)
{
	...
	Cry_Hsm *HandlePtr;
	/*Pointer containing elements for decrypt data */
	Hsm_CipherParamsType *ParamPtr;
	const Cry_HsmDecryptConfigType *DecryptCfgPtr;
...
#if(CRY_HSM_DEV_ERROR_DETECT == STD_ON)
	DetErr = DEV_NO_ERROR;
	PTR_VALIDATION(CRY_HSM_SID_DECRYPT, CfgPtr ,DetErr)
		
	if(DEV_NO_ERROR == DetErr)
	{
	    PTR_VALIDATION(CRY_HSM_SID_DECRYPT, KeyPtr , DetErr)
	}
	if(DEV_NO_ERROR == DetErr)
	{
	    PTR_VALIDATION(CRY_HSM_SID_DECRYPT, DataPtr , DetErr)
	}
	if(DEV_NO_ERROR == DetErr)
	{
	    PTR_VALIDATION(CRY_HSM_SID_DECRYPT, ResultPtr, DetErr)
	}
	/* Check the next DET only if the DET error doesn't occur.
	 * DetErr == DEV_NO_ERROR indicates that DET error doesn't occur */
	if(DEV_NO_ERROR == DetErr)
	{
	    INIT_VALIDATION(CRY_HSM_SID_DECRYPT, Cry_HsmInitStatus , DetErr)
	}
	if(DEV_NO_ERROR == DetErr)
	{
	    KEY_VALIDATION(CRY_HSM_SID_DECRYPT,Cry_lCalcKeyAddr(KeyPtr)
																  , DetErr)
	}
...
#endif
	{
		...
		ErrorVal = Cry_HsmPrimitiveStart(Cry_PRMIDDecrypt);
		/* Check if the session opened successfully */
		if(CSM_E_BUSY != ErrorVal)
		{
			/*Store the key*/
			Cry_HsmDecryptData.KeyAddr = Cry_lCalcKeyAddr(KeyPtr);
        #if(CRY_HSM_DEV_ERROR_DETECT == STD_ON)
			...
			Cry_PrimStatus[Cry_PRMIDDecrypt] = CRY_HSM_PRIM_STARTED;
        #endif
		HandlePtr = &(Cry_HsmData[Cry_PRMIDDecrypt]);
		
		/* Check the HSM status. COmmand is sent only if the
					   * HSM is not busy */
			ErrorVal = Cry_HsmStatusCheck(Cry_PRMIDDecrypt);
			if(CSM_E_OK == ErrorVal)
			{
				 ...
				 * input config pointer is of type void */
				DecryptCfgPtr = (const Cry_HsmDecryptConfigType *)CfgPtr;
				
				/*Store key address in Session data*/
				HsmCom_SetPackedParams(HandlePtr->Session,
										  Cry_HsmDecryptData.KeyAddr,
...
#if(CRY_HSM_DEV_ERROR_DETECT == STD_ON)
	  Cry_PrimStatus[CRY_HSM_SYM_gDECRYPT] = CRY_HSM_PRIM_FINISHED;
#endif
	return ErrorVal;
}

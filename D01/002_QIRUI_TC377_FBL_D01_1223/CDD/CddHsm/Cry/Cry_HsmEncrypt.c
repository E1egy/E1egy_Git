...
Cry_HsmEncryptConfigType  Cry_HsmEncryptConfig =
        {.TimeOut = CRY_HSM_ENCRYPT_ALL_TIMEOUT};

static Cry_HsmPrimitiveIdType Cry_PRMIDEncrypt = CRY_HSM_SYM_gENCRYPT;
...
** Traceability    :                                                          **
**                                                                            **
** Syntax          :Csm_ReturnType Cry_HsmEncrypt                             **
...
**                                                                            **
** Description     : sm4/aes256 encrypt interface                             **
**                                                                            **
**                                                                            **
** Service ID      : CRY_HSM_SID_ENCRYPT                                      **
**                                                                            **
...
**                                                                            **
*******************************************************************************/
Csm_ReturnType  Cry_HsmEncrypt(const void *CfgPtr,const Csm_SymKeyType *KeyPtr,const uint8 *IV,Hsm_CipherAlgorithmType Algorithm_Type,
                                   Crypto_AlgorithmModeType mode,const uint8 *DataPtr, uint32 DataLength, uint8 *ResultPtr,uint32 *ResultLength,Hsm_CryptoOperationMode OperationMode)
{
	...
	Cry_Hsm *HandlePtr;
	/*Pointer containing elements for encrypt data */
	Hsm_CipherParamsType *ParamPtr;
	const Cry_HsmEncryptConfigType *EncryptCfgPtr;
...
#if(CRY_HSM_DEV_ERROR_DETECT == STD_ON)
	DetErr = DEV_NO_ERROR;
	PTR_VALIDATION(CRY_HSM_SID_ENCRYPT, CfgPtr ,DetErr)
		
	if(DEV_NO_ERROR == DetErr)
	{
	    PTR_VALIDATION(CRY_HSM_SID_ENCRYPT, KeyPtr , DetErr)
	}
	if(DEV_NO_ERROR == DetErr)
	{
	    PTR_VALIDATION(CRY_HSM_SID_ENCRYPT, DataPtr , DetErr)
	}
	if(DEV_NO_ERROR == DetErr)
	{
	    PTR_VALIDATION(CRY_HSM_SID_ENCRYPT, ResultPtr, DetErr)
	}
	/* Check the next DET only if the DET error doesn't occur.
	 * DetErr == DEV_NO_ERROR indicates that DET error doesn't occur */
	if(DEV_NO_ERROR == DetErr)
	{
	    INIT_VALIDATION(CRY_HSM_SID_ENCRYPT, Cry_HsmInitStatus ,
																	   DetErr)
	}
	if(DEV_NO_ERROR == DetErr)
	{
	    KEY_VALIDATION(CRY_HSM_SID_ENCRYPT,Cry_lCalcKeyAddr(KeyPtr)
																  , DetErr)
	}
...
#endif
	{
		...
		ErrorVal = Cry_HsmPrimitiveStart(Cry_PRMIDEncrypt);
		/* Check if the session opened successfully */
		if(CSM_E_BUSY != ErrorVal)
		{
			/*Store the key*/
			Cry_HsmEncryptData.KeyAddr = Cry_lCalcKeyAddr(KeyPtr);
	        #if(CRY_HSM_DEV_ERROR_DETECT == STD_ON)
			...
			Cry_PrimStatus[Cry_PRMIDEncrypt] = CRY_HSM_PRIM_STARTED;
	        #endif

			HandlePtr = &(Cry_HsmData[Cry_PRMIDEncrypt]);
			/* Check the HSM status. COmmand is sent only if the
					   * HSM is not busy */
			ErrorVal = Cry_HsmStatusCheck(Cry_PRMIDEncrypt);
			if(CSM_E_OK == ErrorVal)
			{
				 ...
				 * input config pointer is of type void */
				EncryptCfgPtr = (const Cry_HsmEncryptConfigType *)CfgPtr;
				
				/*Store key address in Session data*/
				HsmCom_SetPackedParams(HandlePtr->Session,
										  Cry_HsmEncryptData.KeyAddr,
				...
				ErrorVal = Cry_HsmSendCommandCommon(HandlePtr,
								 HSMCOM_CMD_ENCRYPT,
								 CRY_HSM_GET_UPDATE_TIMEOUT_TICKS(EncryptCfgPtr,DataLength));
				/* Make the KEY as invalid as valid KEY has to be
									   sent only for the first UPDATE command */
				Cry_HsmEncryptData.KeyAddr = HSMCOM_INVALID_KEY;
			}
...
#if(CRY_HSM_DEV_ERROR_DETECT == STD_ON)
	  Cry_PrimStatus[CRY_HSM_SYM_gENCRYPT] = CRY_HSM_PRIM_FINISHED;
#endif
	return ErrorVal;
}

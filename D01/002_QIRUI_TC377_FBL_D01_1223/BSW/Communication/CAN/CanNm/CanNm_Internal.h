...
#define CanNm_GetChannelConfig(ch) \
            (CanNm_CfgPtr->ChannelConfig[ch])

#define CanNm_GetPduCbvPosition(ch) \
            (CanNm_GetChannelConfig(ch).PduCbvPosition)

#define CanNm_GetPduNidPosition(ch) \
            (CanNm_GetChannelConfig(ch).PduNidPosition)

#define CanNm_GetNodeId(ch) \
            (CanNm_GetChannelConfig(ch).NodeId)

#define CanNm_GetComMNetworkHandle(ch) \
            (CanNm_GetChannelConfig(ch).ComMNetworkHandleRef)

#define CanNm_GetBusLoadReduceEnabled(ch) \
            (CanNm_GetChannelConfig(ch).BusLoadReductionActive)

#define CanNm_GetCarWakeUpRxEnabled(ch) \
            (CanNm_GetChannelConfig(ch).CarWakeUpRxEnabled)

#define CanNm_GetCarWakeUpFilterEnabled(ch) \
            (CanNm_GetChannelConfig(ch).CarWakeUpFilterEnabled)

#define CanNm_GetCarWakeUpFilterNodeId(ch) \
            (CanNm_GetChannelConfig(ch).CarWakeUpFilterNodeId)

#if (STD_ON == CANNM_VER_4_2_2)
#define CanNm_GetNodeIdEnabled(ch) \
            CANNM_NODE_ID_ENABLED

#define CanNm_GetNodeDetectionEnabled(ch) \
            CANNM_NODE_DETECTION_ENABLED

#define CanNm_GetRepeatMsgIndEnabled(ch) \
            CANNM_REPEAT_MSG_IND_ENABLED
#else
#define CanNm_GetNodeIdEnabled(ch) \
            (CanNm_GetChannelConfig(ch).NodeIdEnabled)

#define CanNm_GetNodeDetectionEnabled(ch) \
            (CanNm_GetChannelConfig(ch).NodeDetectionEnabled)

#define CanNm_GetRepeatMsgIndEnabled(ch) \
            (CanNm_GetChannelConfig(ch).RepeatMsgIndEnabled)
#endif


#define CanNm_GetPnInfo() \
            (*(CanNm_CfgPtr->PnInfo))

#define CanNm_GetPnResetTime() \
            (CanNm_CfgPtr->PnResetTime)

#define CanNm_GetPnEiraRxNSduId() \
            (CanNm_CfgPtr->CanNmPnEiraRxNSduRef)

#define CanNm_GetPnEiraInfo() \
            (CanNm_InnerGlobal.pnEiraInfo)

#define CanNm_GetPnEiraResetTimer() \
            (CanNm_InnerGlobal.pnEiraResetTimer)

#define CanNm_SetTxPduCbvBit(chIndex,mask) do{\
        if(CANNM_PDU_OFF != CanNm_GetPduCbvPosition(chIndex))\
        {\
            SchM_Enter_CanNm_Network(chIndex);\
            CanNm_ChRunTime[chIndex].txPduData[\
                         (uint8)CanNm_GetPduCbvPosition(chIndex)] |= (mask);\
            SchM_Exit_CanNm_Network(chIndex);\
        }\
    }while(0)

#define CanNm_ClrTxPduCbvBit(chIndex,mask) do{\
        if(CANNM_PDU_OFF != CanNm_GetPduCbvPosition(chIndex))\
        {\
            SchM_Enter_CanNm_Network(chIndex);\
            CanNm_ChRunTime[chIndex].txPduData[\
...
*******************************************************************************/
#if (STD_ON == CANNM_GLOBAL_PN_SUPPORT)
extern VAR(CanNm_InnerGlobalType, CANNM_VAR) CanNm_InnerGlobal;
#endif/* (STD_ON == CANNM_GLOBAL_PN_SUPPORT) */
/*******************************************************************************
**                      Global Functions                                      **
*******************************************************************************/

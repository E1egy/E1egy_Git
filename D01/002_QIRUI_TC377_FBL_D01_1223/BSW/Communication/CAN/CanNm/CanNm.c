...
*******************************************************************************/
#define CANNM_C_AR_RELEASE_MAJOR_VERSION                        19u
#define CANNM_C_AR_RELEASE_MINOR_VERSION                        11u
#define CANNM_C_AR_RELEASE_PATCH_VERSION                        0u
#define CANNM_C_SW_MAJOR_VERSION                                2u
#define CANNM_C_SW_MINOR_VERSION                                0u
#define CANNM_C_SW_PATCH_VERSION                                3u
/*******************************************************************************
**                      Include Section                                       **
*******************************************************************************/
...
static FUNC(void, CANNM_CODE) CanNm_RxCarWakeUpHadle(
    uint8 chIndex
);
#endif/* STD_ON == CANNM_CAR_WAKEUP_RX_ENABLED */

static FUNC(boolean, CANNM_CODE) CanNm_FindChannelIndex(
    NetworkHandleType nmChannelHandle,
    P2VAR(uint8, AUTOMATIC, CANNM_APPL_DATA) chIndexPtr
);

static FUNC(void, CANNM_CODE) CanNm_StateChange(
    uint8 chIndex,
    Nm_StateType nmNewState
);

static FUNC(void, CANNM_CODE) CanNm_EntryRepeatMessageState(
    uint8 chIndex,
...
static FUNC(void, CANNM_CODE) CanNm_MsgcycleTimerManage(
    uint8 chIndex
);
#endif/*STD_OFF == CANNM_PASSIVE_MODE_ENABLED*/

static FUNC(void, CANNM_CODE) CanNm_TimerManagement(
    uint8 chIndex
);
...
static FUNC(boolean, CANNM_CODE) CanNm_RxPnFilter(
    ...
    P2VAR(CanNm_InnerChannelType, AUTOMATIC, CANNM_APPL_DATA) chRTPtr,
...
static FUNC(void, CANNM_CODE) CanNm_RxPnEiraEraHandle(
    uint8 chIndex
);
...
static FUNC(void, CANNM_CODE) CanNm_PnEraResetTimerHandle(
    uint8 chIndex
);
...
#if (STD_OFF == CANNM_PASSIVE_MODE_ENABLED)
static FUNC(Std_ReturnType, CANNM_CODE) CanNm_SendNmPdu(
    uint8 chIndex,
    boolean addTransFlag
);
#endif /*STD_OFF == CANNM_PASSIVE_MODE_ENABLED*/

#if ((STD_ON == CANNM_PN_EIRA_CALC_ENABLED) && \
    (STD_ON == CANNM_COM_USERDATA_SUPPORT))
static FUNC(void, CANNM_CODE) CanNm_TxPnEiraHandle(
    P2CONST(uint8, AUTOMATIC, CANNM_APPL_DATA) txDta
);
#endif

static FUNC(void, CANNM_CODE) CanNm_StateManagement(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_BusSleepStateHandle(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_PrepareBusSleepStateHandle(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_RepeatMessageStateHandle(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_NormalOperationStateHandle(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_ReadySleepStateHandle(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_RxDataMainHandle(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_SendMsgMainHandle(
    uint8 chIndex
);

#if (STD_ON == CANNM_COORDINATOR_SYNC_SUPPORT)
static FUNC(void, CANNM_CODE) CanNm_CoordReadyToSleepIndHandle(
    uint8 chIndex
);
#endif/* STD_ON == CANNM_COORDINATOR_SYNC_SUPPORT */

static FUNC(void, CANNM_CODE) CanNm_EnterPrepareBusSleepModeHandle(
    uint8 chIndex
);

static FUNC(void, CANNM_CODE) CanNm_MemCpy(
...
#include "CanNm_MemMap.h"
static VAR(CanNm_InitStatusType, CANNM_VAR_POWER_ON_INIT)
...
#include "CanNm_MemMap.h"
static P2CONST(CanNm_ConfigType, AUTOMATIC, CANNM_APPL_CONST)
CanNm_CfgPtr = NULL_PTR;
...
#include "CanNm_MemMap.h"
static VAR(CanNm_InnerChannelType, CANNM_VAR)
CanNm_ChRunTime[CANNM_NUMBER_OF_CHANNEL];


#if (STD_ON == CANNM_GLOBAL_PN_SUPPORT)
VAR(CanNm_InnerGlobalType, CANNM_VAR) CanNm_InnerGlobal;
#endif/* (STD_ON == CANNM_GLOBAL_PN_SUPPORT) */
...
)
{
    ...
    else
    {
        ...
        #endif/* #if (STD_ON == CANNM_GLOBAL_PN_SUPPORT)*/
        for(chIndex = 0u; chIndex < CANNM_NUMBER_OF_CHANNEL; chIndex++)
        {
            ...
            {
                chRTPtr->txPduData[(uint8)CanNm_GetPduNidPosition(chIndex)]
                                                 = CanNm_GetNodeId(chIndex);
            }
            ...
            {
                chRTPtr->txPduData[(uint8)CanNm_GetPduCbvPosition(chIndex)]
                                                                      = 0x00;
            }
            ...
            if((boolean)TRUE == CanNm_CfgPtr->ChannelConfig[chIndex].PnEnabled)
            {
                ...
                for(pnIdx = 0; pnIdx < pnInfo.CanNmPnInfoLength; pnIdx++)
                {
                     ...
                     */
                    chRTPtr->txPduData[ \
                         pnInfo.CanNmPnInfoOffset + pnIdx] = 0x0u;
                }
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_PASSIVESTARUP,
                CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_NETWORKREQUEST,
                CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_NETWORKRELEASE,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_DISABLECOMMUNICATION,
                CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_ENABLECOMMUNICATION,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
    ...
    else
    {
        ...
        if (((boolean)FALSE == chRTPtr->canNmTxEnable) &&
            (NM_MODE_NETWORK == chRTPtr->cannmMode))
        {
            chRTPtr->canNmTxEnable = TRUE;
            /* @req SWS_CanNm_00178 */
            ...
            chRTPtr->nmTimeoutTimer =
                chCfgPtr->TimeoutTime / CanNm_CfgPtr->MainFunctionPeriod;
...
)
{
    ...
    else
    {
        ...
        /* @req SWS_CanNm_00159 */
        /* Set NM user data*/
        uint8 userDtaLen = chCfgPtr->UserDataTxPdu->UserDataLen;
        uint8 userDtaOffset = chCfgPtr->UserDataTxPdu->UserDataStartPos;
        uint8* userDtaPtr = &chRTPtr->txPduData[userDtaOffset];

        SchM_Enter_CanNm_Network(chIndex);
        CanNm_MemCpy(userDtaPtr, nmUserDataPtr, userDtaLen);
        SchM_Exit_CanNm_Network(chIndex);
...
)
{
    ...
    else
    {
        ...
        uint8 userDtaLen = chCfgPtr->UserDataTxPdu->UserDataLen;
        uint8 userDtaOffset = chCfgPtr->UserDataTxPdu->UserDataStartPos;
        uint8* userDtaPtr = &chRTPtr->rxPduData[userDtaOffset];

        SchM_Enter_CanNm_Network(chIndex);
        CanNm_MemCpy(nmUserDataPtr, userDtaPtr ,userDtaLen);
        SchM_Exit_CanNm_Network(chIndex);
...
/* @SWS_CanNm_00330 */
#if ((TRUE == CANNM_COM_USERDATA_SUPPORT) ||\
...
)
{
    ...
    else
    {
        ...
        if(((NM_STATE_NORMAL_OPERATION == chRTPtr->canNmState) ||
            ...
            (PduInfoPtr->SduLength >= chCfgPtr->UserDataTxPdu->UserDataLen))
        {
            /* Copy tx data to tx buffer */
            CanNm_MemCpy(&chRTPtr->txPduData[chCfgPtr->UserDataTxPdu->UserDataStartPos],
                         PduInfoPtr->SduDataPtr,
                         chCfgPtr->UserDataTxPdu->UserDataLen);
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_GETNODEIDENTIFIER,
                CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
    ...
    else
    {
        ...
        {
            SchM_Enter_CanNm_Network(chIndex);
            *nmNodeIdPtr = chRTPtr->rxPduData[
                 (uint8)chCfgPtr->PduNidPosition];
            SchM_Exit_CanNm_Network(chIndex);
            ret = E_OK;
        }
    }
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_GETLOCALNODEIDENTIFIER,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_REPEATMESSAGEREQUEST,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
    ...
    else
    {
        ...
        SchM_Enter_CanNm_Network(chIndex);
        if ((NM_STATE_NORMAL_OPERATION == chRTPtr->canNmState) ||
            (NM_STATE_READY_SLEEP == chRTPtr->canNmState))
        {
...
)
{
    ...
    else
    {
        ...
        #if (STD_ON == CANNM_USER_DATA_ENABLED)
        if((TRUE == CanNm_GetNodeIdEnabled(chIndex)) ||
           (TRUE == CanNm_GetNodeDetectionEnabled(chIndex)))
        {
            SchM_Enter_CanNm_Network(chIndex);
            CanNm_MemCpy(nmPduDataPtr,
                         chRTPtr->rxPduData,
                         CANNM_DEFAULT_PDU_LENGTH);
            SchM_Exit_CanNm_Network(chIndex);
...
)
{
    ...
    {
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_GETVERSIONINFO,
                CANNM_E_PARAM_POINTER
        );
    }
    else
...
#if ((STD_ON == CANNM_BUS_SYNCHRONIZATION_ENABLED) &&\
(STD_OFF == CANNM_PASSIVE_MODE_ENABLED))
/******************************************************************************/
/*
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_REQUESTBUSSYNCHRONIZATION,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_CHECKREMOTESLEEPINDICATION,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_SETSLEEPREADYBIT,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_TXCONFIRMATION,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
 ...
 */
/******************************************************************************/
uint8 g_CanNmSleepFlag = 0xFF;
uint8 g_CanNmDetCounter = 0;
uint8 g_CanNmActiveCounter  = 0;
uint8 g_CanNmUnactiveCounter  = 0;
uint8 validNmMsgFlag = FALSE;
FUNC(void, CANNM_CODE)
CanNm_RxIndication(
    PduIdType RxPduId,
    P2CONST(PduInfoType, AUTOMATIC, CANNM_APPL_DATA) PduInfoPtr
)
{
    ...
    {
        P2CONST(CanNm_ChannelConfigType, AUTOMATIC, CANNM_APPL_CONST)
        chCfgPtr = &CanNm_GetChannelConfig(0u);
        P2VAR(CanNm_InnerChannelType, AUTOMATIC, CANNM_APPL_DATA)
        chRTPtr = NULL_PTR;
        uint8 chIndex;
        PduIdType pduIdx;

        g_CanNmActiveCounter++;
        ...
        #endif
        {
            validNmMsgFlag = TRUE;
            /* @req SWS_CanNm_00035 */
            CanNm_MemCpy(chRTPtr->rxPduData,
                         PduInfoPtr->SduDataPtr,
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_CONFIRMPNAVAILABILITY,
                CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    else if (CANNM_INIT_FAIL == CanNm_InitStatus)
...
)
{
    ...
    if (CANNM_UNINIT == CanNm_InitStatus)
    {
        ...
        #if (STD_ON == CANNM_DEV_ERROR_DETECT)
        CANNM_DET_REPORT(
            CANNM_SERVICE_ID_TRIGGERTRANSMIT,
            CANNM_E_UNINIT );
        #endif/* STD_ON == CANNM_DEV_ERROR_DETECT */
    }
    ...
    if((boolean)TRUE == detNoErr)
    {
        ...
        if(PduInfoPtr->SduLength >= CANNM_DEFAULT_PDU_LENGTH)
        {
            ...
            #if (STD_ON == CANNM_COM_USERDATA_SUPPORT)
            /* @req SWS_CanNm_00350 */
            pduInfo.SduLength = chCfgPtr->UserDataTxPdu->UserDataLen;
            pduInfo.SduDataPtr = &chRTPtr->txPduData[chCfgPtr->UserDataTxPdu->UserDataStartPos];
            pduInfo.MetaDataPtr = NULL_PTR;
            ret = PduR_CanNmTriggerTransmit(
                        chCfgPtr->UserDataTxPdu->UpperLayerPduId,
                        &pduInfo);
            #endif
            ...
            {
                /* @req SWS_CanNm_00351 */
                CanNm_MemCpy(PduInfoPtr->SduDataPtr,
                             chRTPtr->txPduData,
                             CANNM_DEFAULT_PDU_LENGTH);
            }
        }
...
)
{
    ...
    SchM_Enter_CanNm_Network(chIndex);
    uint8 carWakeupByte =
                chRTPtr->rxPduData[chCfgPtr->CarWakeUpBytePosition];
    SchM_Exit_CanNm_Network(chIndex);

    #if (STD_ON == CANNM_CAR_WAKEUP_RX_ENABLED)
    if(((boolean)TRUE == CanNm_GetCarWakeUpRxEnabled(chIndex)) &&
       ((carWakeupByte & carWakeupMask) > 0u))
    {
        ...
        if((boolean)TRUE == CanNm_GetCarWakeUpFilterEnabled(chIndex))
        {
            ...
            if(CANNM_PDU_OFF != chCfgPtr->PduNidPosition)
            {
                SchM_Enter_CanNm_Network(chIndex);
                uint8 rxNodeId =
                        chRTPtr->rxPduData[(uint8)chCfgPtr->PduNidPosition];
                SchM_Exit_CanNm_Network(chIndex);
...
)
{
    ...
    Nm_StateChangeNotification(
        CanNm_GetComMNetworkHandle(chIndex),
        chRTPtr->canNmState,
        nmNewState );
...
)
{
    ...
    /* @req SWS_CanNm_00102 */
    chRTPtr->stateTimer = \
            chCfgPtr->RepeatMessageTime / CanNm_CfgPtr->MainFunctionPeriod;

    #if (STD_OFF == CANNM_PASSIVE_MODE_ENABLED)
    /* @req SWS_CanNm_00100 */
    /* Enter Repeat Message State,transmission shall start*/
    if(((boolean)TRUE != isNetWorkRequest) ||
       (0u == chCfgPtr->ImmediateNmTransmissions))
    {
        /* @req SWS_CanNm_00005 */
        /*
        ...
        chRTPtr->msgTxCylceTimer = \
            chCfgPtr->MsgCycleOffset / CanNm_CfgPtr->MainFunctionPeriod;

        if(0 == chRTPtr->msgTxCylceTimer)
        {
        ...
        /* @req SWS_CanNm_00007 */
        if((NM_MODE_BUS_SLEEP == chRTPtr->cannmMode) &&
           ((boolean)TRUE == chCfgPtr->RetryFirstMessageRequest))
        {
        ...
        if((TRUE == isNetWorkRequest) &&
           (NM_MODE_PREPARE_BUS_SLEEP == chRTPtr->cannmMode))
        {
            /* @req SWS_CanNm_00122 */
    ...
    else
    {
        ...
        if((NM_MODE_BUS_SLEEP == chRTPtr->cannmMode) ||
           ...
           #if(STD_ON == CANNM_GLOBAL_PN_SUPPORT)
           || ((NM_MODE_NETWORK == chRTPtr->cannmMode) &&
               ((boolean)TRUE == chCfgPtr->PnHandleMultipleNetworkRequests))
           #endif /* STD_ON == CANNM_GLOBAL_PN_SUPPORT */
        )
        {
            /* Set send NM message flag */
            ...
            chRTPtr->msgTxCylceTimer = \
                chCfgPtr->ImmediateNmCycleTime / CanNm_CfgPtr->MainFunctionPeriod;
    ...
    if (((boolean)TRUE == chRTPtr->remoteSleepInd) &&
        ((NM_STATE_NORMAL_OPERATION == chRTPtr->canNmState) ||
         (NM_STATE_READY_SLEEP  == chRTPtr->canNmState)))
    {
        chRTPtr->remoteSleepInd = FALSE;
...
)
{
    ...
    {
        chRTPtr->msgTxCylceTimer--;
        if((0u == chRTPtr->msgTxCylceTimer) && (((CANNM_NETWORK_REQUESTED == chRTPtr->netRequestStatus))
                || (NM_STATE_REPEAT_MESSAGE == chRTPtr->canNmState)))
        {
            /* @req SWS_CanNm_00032 */
            /* Set send NM message flag */
            ...
            #endif
            {
                /* @req SWS_CanNm_00040 */
                chRTPtr->msgTxCylceTimer =
                  chCfgPtr->MsgCycleTime / CanNm_CfgPtr->MainFunctionPeriod;
            }
        }
    }
}
...
)
{
    ...
    if((boolean)TRUE == chCfgPtr->PnEnabled)
    {
        ...
        {
            uint8 cbv = PduInfoPtr->SduDataPtr[
                                        (uint8)chCfgPtr->PduCbvPosition];
            ...
            else
            {
                ...
                chRTPtr->pnRxIndFlg = FALSE;
                /*@req SWS_CanNm_00411*/
                if((boolean)FALSE == chCfgPtr->AllNmMessagesKeepAwake)
                {
...
)
{
    ...
    P2CONST(CanNm_PnInfoType, AUTOMATIC, CANNM_APPL_CONST)
    pnInfoPtr = &CanNm_GetPnInfo();
    P2VAR(uint8, AUTOMATIC, CANNM_APPL_DATA)
    pnEiraInfoPtr = CanNm_GetPnEiraInfo();
    P2VAR(uint16, AUTOMATIC, CANNM_APPL_DATA)
    pnEiraResetTimerPtr = CanNm_GetPnEiraResetTimer();
    uint8 rxPnInfoDta;
    uint8 bitMask;
    #if (STD_ON == CANNM_PN_ERA_CALC_ENABLED)
    ...
    #endif /* (STD_ON == CANNM_PN_EIRA_CALC_ENABLED) */
    uint8 index;
    P2CONST(uint8, AUTOMATIC, CANNM_APPL_DATA) rxPnDtaPtr = NULL_PTR;
    uint8 filterMaskData;
    uint8 pnIdx = 0u;
    uint8 find0To1;

    rxPnDtaPtr =
            &(chRTPtr->rxPduData[pnInfoPtr->CanNmPnInfoOffset]);
    ...
    for(index = 0u; index < (pnInfoPtr->CanNmPnInfoLength * 8u); index++)
    {
        ...
        filterMaskData =
            pnInfoPtr->PnFilterMaskByte[index / 8u].PnFilterMaskByteValue;
            ...
            if(0u != (rxPnInfoDta & bitMask))
            {
                ...
                if((boolean)TRUE == chCfgPtr->PnEraCalcEnabled)
                {
                    ...
                    chRTPtr->pnEraResetTimer[pnIdx] =
                        CanNm_GetPnResetTime() / CanNm_CfgPtr->MainFunctionPeriod;
                }
                #endif/* (STD_ON == CANNM_PN_ERA_CALC_ENABLED) */
    ...
    if((boolean)TRUE == chCfgPtr->PnEraCalcEnabled)
    {
        ...
        {
            filterMaskData =
                pnInfoPtr->PnFilterMaskByte[index].PnFilterMaskByteValue;
    ...
    #endif /* (STD_ON == CANNM_COM_CONTROL_ENABLED) */
    {
        ...
        if((uint8)E_OK == ret)
        {
            ...
            if((uint8)E_OK == ret)
            {
                #if (STD_OFF == CANNM_IMMEDIATE_TXCONF_ENABLED)
                chRTPtr->msgTimeoutTimer =
                    chCfgPtr->MsgTimeoutTime / CanNm_CfgPtr->MainFunctionPeriod;
                chRTPtr->msgToutFlg = FALSE;
    ...
    for(index = 0u; index < (pnInfoPtr->CanNmPnInfoLength * 8u); index++)
    {
        ...
        if((filterMaskData & bitMask) > 0u)
        {
            ...
            if((txPnInfoDta & bitMask) > 0u)
            {
                ...
                if(((boolean)FALSE == rxIndEiraFlag) &&
                   ((localPnInfoDta & bitMask) == 0u))
                {
                    rxIndEiraFlag = TRUE;
                }
            }
...
)
{
    ...
    else if((boolean)TRUE == chRTPtr->rxPduFlg)
    {
        /* @req SWS_CanNm_00124 */
...
)
{
       ...
       ((boolean)FALSE == stateChanged))
    {
        ...
        if (CANNM_NETWORK_REQUESTED == chRTPtr->netRequestStatus)
        {
            ...
            chRTPtr->remoteTimer =
                chCfgPtr->RemoteSleepIndTime / CanNm_CfgPtr->MainFunctionPeriod;
            #endif /*STD_ON == CANNM_REMOTE_SLEEP_IND_ENABLED*/
        }
        else
        {
        ...
        {
            CanNm_ClrTxPduCbvBit(
                        chIndex, CANNM_CBV_BIT_RMP_MASK);
        }
        #endif/* (STD_ON == CANNM_NODE_DETECTION_ENABLED) */
    }
}
...
)
{
           ...
           ((boolean)FALSE == stateChanged))
        {
            if((boolean)TRUE == chRTPtr->repeatMessageRequest)
            {
                ...
                CanNm_SetTxPduCbvBit(chIndex, CANNM_CBV_BIT_RMP_MASK);
                    ...
                    (CANNM_PDU_OFF != chCfgPtr->PduCbvPosition))
            {
        ...
        && (TRUE == chRTPtr->rxPduFlg))
    {
        ...
        {
            chRTPtr->msgTxCylceTimer =
            chCfgPtr->MsgReducedTime / CanNm_CfgPtr->MainFunctionPeriod;
        }
    }
...
)
{
    ...
    #if (STD_ON == CANNM_NODE_DETECTION_ENABLED)
    if((TRUE == CanNm_GetNodeDetectionEnabled(chIndex)) &&
       ((boolean)TRUE == chRTPtr->repeatMessageRequest) &&
       ((boolean)FALSE == stateChanged))
    {
        /* @req SWS_CanNm_00112 */
        CanNm_EntryRepeatMessageState(chIndex, FALSE);
    ...
    if(((boolean)TRUE == chRTPtr->nmToutFlg) &&
       ((boolean)FALSE == stateChanged))
    {
        /* @req SWS_CanNm_00109 */
    ...
    #if (STD_ON == CANNM_COM_CONTROL_ENABLED)
    if(((boolean)FALSE == chRTPtr->canNmTxEnable) &&
       (CANNM_NETWORK_RELEASED == chRTPtr->netRequestStatus )&&
       ((boolean)FALSE == stateChanged))
    {
...
)
{
    ...
    if((CANNM_PDU_OFF != chCfgPtr->PduCbvPosition) &&
       (TRUE == CanNm_GetNodeDetectionEnabled(chIndex)) &&
       (TRUE == CanNm_GetRepeatMsgIndEnabled(chIndex)))
    {
        uint8 cbvPos = (uint8)chCfgPtr->PduCbvPosition;
        uint8 cbvDta = chRTPtr->rxPduData[cbvPos];
    ...
    if(CANNM_PDU_OFF != chCfgPtr->PduCbvPosition)
    {
        ...
        CanNm_CoordReadyToSleepIndHandle(chIndex);
         ...
         ((STD_ON == CANNM_PN_ERA_CALC_ENABLED)||\
          (STD_ON == CANNM_PN_EIRA_CALC_ENABLED)))
    if(((boolean)TRUE == chCfgPtr->PnEnabled) &&
       ((boolean)TRUE == chRTPtr->pnRxIndFlg))
    {
...
)
{
    ...
    if(0u == (cbvDta & CANNM_CBV_BIT_CSR_MASK))
    {
        ...
        if ((NM_MODE_NETWORK == chRTPtr->cannmMode) &&
            (CANNM_CSR_IND_INDICATED == chRTPtr->coordReadyToSleepInd))
        {
    ...
    else
    {
        ...
        if (((NM_MODE_NETWORK == chRTPtr->cannmMode) &&
             (CANNM_CSR_IND_INIT == chRTPtr->coordReadyToSleepInd)) ||
...
)
{
    ...
    {
        chRTPtr->stateTimer =
            chCfgPtr->WaitBusSleepTime / CanNm_CfgPtr->MainFunctionPeriod;
    }

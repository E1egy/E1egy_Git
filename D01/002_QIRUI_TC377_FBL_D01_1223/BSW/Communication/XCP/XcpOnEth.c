...
)
{
    ...
    {
        Xcp_CopyU1BufferToU2(&(XcpRxPduPtr->SduDataPtr[2u]), &counter, LOW_BYTE_FIRST);
        ...
        Xcp_CopyU1BufferToU2(&(XcpRxPduPtr->SduDataPtr[0u]), &len, LOW_BYTE_FIRST);
        Xcp_RxIndication(len, &(XcpRxPduPtr->SduDataPtr[4u]));
    }
    return;
}

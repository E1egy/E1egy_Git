...
#define XCP_CAL_C_SW_MAJOR_VERSION  2u
...
Xcp_CALInit(void)
{
    ...
    {
        Xcp_BufferCopy(Xcp_SegmentInfo.pageInfoPtr[segNum].romStart,
        ...
        Xcp_BufferCopy(Xcp_SegmentInfo.pageInfoPtr[segNum].ramStart,
...
Xcp_ShortDownload(void)
{
    ...
    else if ((Xcp_CmdBuffer[1u] > XCP_SHORT_DOWNLOAD_SIZE)
    || (0u == Xcp_CmdBuffer[1u]))
    {
        Xcp_SetErrorCode(XCP_ERR_OUT_OF_RANGE);
        Xcp_RespLength = 0x02u;
...
Xcp_SwitchToWP(void)
{
    ...
    for (segNum = 0; segNum < Xcp_SegmentInfo.maxSegNum; segNum++)
    {
        Xcp_BufferCopy(Xcp_SegmentInfo.pageInfoPtr[segNum].bufferPtr,
                          Xcp_SegmentInfo.pageInfoPtr[segNum].ramStart,
                          Xcp_SegmentInfo.pageInfoPtr[segNum].size);
        Xcp_ActivPagNum[segNum] = XCP_RAM_PAGE_NUM;
    }

 ...
 *  V1.0.0       2020-01-20  qinchun.yang    Modify SET_REQUEST command.
...
#define XCP_STD_C_SW_MAJOR_VERSION  2u
...
FUNC(void, XCP_CODE) Xcp_GetCommModeInfo(void)
{
    ...
    #endif
    {
        ...
        Xcp_RespBuffer[7u] = ((uint8)((XCP_STD_C_SW_MAJOR_VERSION & 0x0fu) << 4u))
...
)
{
     ...
     * */
    if (XCP_SESSION_STATE_STORE_CAL ==
            (Xcp_SessionStatus & XCP_SESSION_STATE_STORE_CAL))
    {
        Xcp_SessionStatus &= (~XCP_SESSION_STATE_STORE_CAL);
    ...
    else if (XCP_SESSION_STATE_STORE_DAQ ==
                (Xcp_SessionStatus & XCP_SESSION_STATE_STORE_DAQ))
    {
        Xcp_SessionStatus &= (~XCP_SESSION_STATE_STORE_DAQ);
        Xcp_EvBuffer[XCP_PID_OFFSET] = XCP_EV_PID;
    ...
    else if (XCP_SESSION_STATE_CLEAR_DAQ ==
                (Xcp_SessionStatus & XCP_SESSION_STATE_CLEAR_DAQ))
    {
        Xcp_SessionStatus &= (~XCP_SESSION_STATE_CLEAR_DAQ);
        Xcp_EvBuffer[XCP_PID_OFFSET] = XCP_EV_PID;
...
Xcp_SetMta(void)
{
    ...
    || ((boolean)TRUE == Xcp_CheckAddress(Xcp_Mta2Ptr(extAddr, Addr), 0, XCP_MEM_CAL_ALL))
#endif
#if (0 != (XCP_PL_PGM & XCP_RESOURCE))
    || ((boolean)TRUE == Xcp_CheckAddress(Xcp_Mta2Ptr(extAddr, Addr), 0, XCP_MEM_PGM))
#endif
    )
    {
...
#if (0 != (XCP_PL_CAL & XCP_RESOURCE))
    || ((boolean)TRUE == Xcp_CheckAddress(Xcp_Mta2Ptr(Xcp_MTA.extensionAddr, Xcp_MTA.transferAddr), ((uint32)len * XCP_AG), XCP_MEM_CAL_ALL))
#endif
#if (0 != (XCP_PL_PGM & XCP_RESOURCE))
    || ((boolean)TRUE == Xcp_CheckAddress(Xcp_Mta2Ptr(Xcp_MTA.extensionAddr, Xcp_MTA.transferAddr), ((uint32)len * XCP_AG), XCP_MEM_PGM))
#endif
#if ((XCP_GET_ID == STD_ON) || (XCP_GET_DAQ_EVENT_INFO == STD_ON) || (XCP_GET_SECTOR_INFO == STD_ON))
...
#if (0 != (XCP_PL_CAL & XCP_RESOURCE))
    || ((boolean)TRUE == Xcp_CheckAddress(Xcp_Mta2Ptr(extAddr, Addr), ((uint32)len * XCP_AG), XCP_MEM_CAL_ALL))
#endif
#if (0 != (XCP_PL_PGM & XCP_RESOURCE))
    || ((boolean)TRUE == Xcp_CheckAddress(Xcp_Mta2Ptr(extAddr, Addr), ((uint32)len * XCP_AG), XCP_MEM_PGM))
#endif
    )
    {
        /* Set MTA */
...
Xcp_BuildChecksum(void)
{
    ...
    else
    {
        ...
        /* address check */
        else if ((boolean)FALSE == Xcp_CheckAddress(checksumStartAddress,

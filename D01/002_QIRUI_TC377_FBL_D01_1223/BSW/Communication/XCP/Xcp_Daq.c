 ...
 *  V1.0.2       2020-01-20  qinchun.yang    Modify DAQ resume function.
...
#define XCP_DAQ_C_SW_MAJOR_VERSION  2u
...
{
    P2VAR(Xcp_DaqType, AUTOMATIC, XCP_APPL_DATA) daqPtr;
...
/******************************************************************************/
static FUNC(void,XCP_CODE)
Xcp_FillEventBuffer(P2VAR(Xcp_DaqType, AUTOMATIC, AUTOMATIC) daqPtr)
{
    ...
    #if (XCP_TIMESTAMP_TYPE != XCP_TS_NO_TS)
    daqPtr->timeStamp = Frt_ReadOutMS();
    #endif
    return;
}
        ...
        if (XCP_DAQ_PENDING != (XCP_DAQ_PENDING & Xcp_SendStatus))
        {
            ...
            #endif
            {
                ...
                if (XCP_DAQ_SS_AL == (XCP_DAQ_SS_AL & currentDaqPtr->sendStatus))
                {
                    ...
                    || (0u == (currentDaqPtr->odt[currentDaqPtr->currentPid - currentDaqPtr->firstPid].XcpOdtEntry[0u].XcpOdtEntryLength))
                    )
                    {
                ...
                ((currentDaqPtr->daqListSize + currentDaqPtr->firstPid)<
                    (currentDaqPtr->currentPid))
                /* size of new entry is zero */
                || (0u == (currentDaqPtr->odt[currentDaqPtr->currentPid - currentDaqPtr->firstPid].XcpOdtEntry[0].XcpOdtEntryLength))
                )
                {
...
FUNC(void,XCP_CODE) Xcp_RxStimHal(P2CONST(uint8,AUTOMATIC,XCP_APPL_DATA) dataPtr)
{
    ...
    if (XCP_MAX_DAQ >= daqNum)
    {
        ...
        if (
                XCP_DAQ_MOD_DIR_STIM != (daqPtr->mode & XCP_DAQ_MOD_DIR_STIM)
        ...
        else
        {
            ...
            if (
                (0x00 == odtNum)
                && (0x00 != (XCP_DAQ_MOD_TS & daqPtr->mode))
            )
            {
                pos += XCP_TIMESTAMP_TYPE;
            }
            ...
            for(entryIdx = 0; entryIdx < odtPtr->XcpOdtEntryCount; entryIdx++)
            {
                ...
                if ((0x00u == entryPtr->XcpOdtEntryLength)
                        || (XCP_MAX_CTO <= pos))
                {
...
Xcp_EventChannelDaqHal(uint16 eventNum, const Xcp_DaqNumType daqNum)
{
    ...
    if((Xcp_DaqListQueueLength > Xcp_MaxQueueLength)
        && (Xcp_DaqListQueueLength <= XCP_MAX_DAQ))
    {
        Xcp_MaxQueueLength = Xcp_DaqListQueueLength;
    }
    return;
...
Xcp_EventIndication(uint16 eventNum)
{
    ...
    for (cnt = 0; cnt < eventPtr->XcpEvChMaxDaqList; cnt++)
    {
        ...
        if(
        (eventPtr->XcpEvChMaxDaqList == (cnt + 1u))
            && (Xcp_DaqListQueueLength > 0u)
          )
        {
            #if (XCP_DAQ_PRIORITY_SUPPORT == STD_ON)
...
Xcp_StopDaq(Xcp_DaqNumType daqNum)
{
    ...
    if (XCP_DAQ_MOD_RUNNING == (XCP_DAQ_MOD_RUNNING & (Xcp_Daq[daqNum].mode)))
    {
        ...
        Xcp_Daq[daqNum].sendStatus &= (uint8)(~XCP_DAQ_SS_AL);;  /* clear DAQ sendStatus flag */
        Xcp_Daq[daqNum].eventBufferPtr = NULL_PTR;
        Xcp_EvChActivCnt[evChNum]--;
...
Xcp_WriteDaqHal(void)
{
    ...
    addressOfOdtEntry = Xcp_Mta2Ptr(addressExtension,addressOfDaqElement);
     ...
     /*Check DAQ address.*/
//     else if ((addressOfOdtEntry < Xcp_MeaStartAddr)
////             || (addressOfOdtEntry > (Xcp_MeaStartAddr + Xcp_SegmentInfo.pageInfoPtr[0].size))
//			 || (addressOfOdtEntry > (Xcp_MeaStartAddr + 0x2800))
//             )
//     {
//         Xcp_SetErrorCode(XCP_ERR_OUT_OF_RANGE);
//         Xcp_RespLength = 0x02u;
...
Xcp_SetDaqListModeHal(void)
{
    ...
    if ( /* check input parameter */
        ...
        || (daqPriority > 1u)			/*XCP_TEST  (daqPriority > 0u)*/
        #else
        /*Note:Yqc 2020/03/18
...
Xcp_GetDaqProcessorInfo(void)
{
    ...
    else
    {
        ...
        Xcp_RespBuffer[7u] = (((uint8)(XCP_IDENTIFICATION_FIELD_TYPE << 6u))/**< Identification Field Type*/
...
FUNC(void,XCP_CODE) Xcp_GetDaqEventInfo(void)
{
    ...
    else
    {
        ...
        {
            Xcp_RespBuffer[1u] = (uint8)(
                                  ((uint8)((uint8)Xcp_GlobalCfgPtr->XcpEvCh[eventChannelNo].XcpEvChType << 2u))
...
Xcp_AllocOdtHal(void)
{
    ...
    else if ((XCP_DAQ_CFG_DAQ != Xcp_Daq[daqListNo].DynaCfgStatus)

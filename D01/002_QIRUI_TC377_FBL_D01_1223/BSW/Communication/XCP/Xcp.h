...
typedef struct  {
    ...
    P2VAR(Xcp_OdtType, TYPEDEF, XCP_CONST) XcpOdt;
...
typedef struct  {
    ...
    P2VAR(Xcp_AGType, TYPEDEF, XCP_CONST)
                                XcpEvChBuffPtr;
...
typedef struct
{
    P2FUNC(Std_ReturnType, BSWM_APPL_CODE, flsWrite)(
            Xcp_FlsAddressType TargetAddress,
            const uint8* SourceAddressPtr,
            Xcp_FlsLengthType Length
    );
    P2FUNC(Std_ReturnType, BSWM_APPL_CODE, flsErase)(
            Xcp_FlsAddressType TargetAddress,
            Xcp_FlsLengthType Length
    );
    P2FUNC(MemIf_JobResultType, BSWM_APPL_CODE, flsGetJobResult)(
            void
    );
} Xcp_FlsFuncPtrType;
#endif /* (XCP_PL_PGM == (XCP_PL_PGM&XCP_RESOURCE)) */
...
#include "XCP_MemMap.h"
extern const Xcp_ConfigType Xcp_PBConfig;		/*XCP_TEST*/
#endif  /* endof XCP_H */

...
Xcp_Init(CONSTP2CONST(Xcp_ConfigType, AUTOMATIC, XCP_APPL_CONST) Xcp_ConfigPtr)
{
    ...
    #endif /* (XCP_DEV_ERROR_DETECT == STD_ON) */
    {
        ...
        if ((Std_ReturnType)E_NOT_OK == Xcp_CALInit())
        {
//            Dem_ReportErrorStatus(XCP_E_NOT_INITIALIZED, DEM_EVENT_STATUS_FAILED);
        ...
        if ((Std_ReturnType)E_NOT_OK == Xcp_DAQInit())
        {
//            Dem_ReportErrorStatus(XCP_E_NOT_INITIALIZED, DEM_EVENT_STATUS_FAILED);
        }
        else
...
static FUNC(boolean,XCP_CODE)
Xcp_IsInterleavedBufFull(void)
{
                              ...
                              P2VAR(uint32,AUTOMATIC,XCP_VAR) pu4Data, uint8 byteOrder)
{
    ...
    if ((uint8)HIGH_BYTE_FIRST == byteOrder)
    {
        *pu4Data = ((uint32)pu1BufferPtr[0U]<<24U)
                 + ((uint32)pu1BufferPtr[1U]<<16U)
                 + ((uint32)pu1BufferPtr[2U]<<8U)
                 + ((uint32)pu1BufferPtr[3U]);
    }
    /*in case of Intel MSB last*/
    else
    {
        *pu4Data = ((uint32)pu1BufferPtr[3U]<<24U)
                 + ((uint32)pu1BufferPtr[2U]<<16U)
                 + ((uint32)pu1BufferPtr[1U]<<8U)
                 + ((uint32)pu1BufferPtr[0U]);
    }
    return;
}
                              ...
                              P2VAR(uint16,AUTOMATIC,XCP_VAR) pu2Data, uint8 byteOrder)
{
    ...
    if ((uint8)HIGH_BYTE_FIRST == byteOrder)
    {
        *pu2Data = (uint16)((uint16)pu1BufferPtr[0U]<<8U)
                 + (uint16)pu1BufferPtr[1U];

    }
    /*in case of Intel MSB last*/
    else
    {
        *pu2Data = (uint16)((uint16)pu1BufferPtr[1U]<<8U)
                 + (uint16)pu1BufferPtr[0U];
    }
    return;
}
...
Xcp_SendResp(void)
{
    ...
    P2VAR(Xcp_AGType, AUTOMATIC, BSWM_VAR_CLEARED) destPtr;
    uint8 pos;
    uint8 len;
    #endif
    Std_ReturnType ret;
    ...
    #endif
    {
        ...
        && (Xcp_BlockBufferPos != Xcp_BlockBufferLen))
        {
            ...
            do
            {
                ...
                if ((Std_ReturnType)E_OK == Xcp_Transmit
                    ((PduIdType)Xcp_PbCfgPtr->XcpPduRef->XcpTxPdu->LowLayerTxPduId, (PduLengthType)Xcp_RespLength, &Xcp_RespBuffer[0U]))
                {
...
FUNC(void, XCP_CODE) Xcp_SendEv(void)
{
    ...
    #endif
    {
        if ((Std_ReturnType)E_OK == 
            Xcp_Transmit((PduIdType)Xcp_PbCfgPtr->XcpPduRef->XcpTxPdu->LowLayerTxPduId, 
                (PduLengthType)Xcp_EvLength, &Xcp_EvBuffer[0]))
        {
...
static FUNC(void, XCP_CODE) Xcp_SendServ(void)
{
    ...
    #endif
    {
        if ((Std_ReturnType)E_OK == 
            Xcp_Transmit((PduIdType)Xcp_PbCfgPtr->XcpPduRef->XcpTxPdu->LowLayerTxPduId,
                (PduLengthType)Xcp_ServLength, &Xcp_ServBuffer[0]))
        {
...
#endif
{
    ...
    #if (XCP_PL_PGM == (XCP_PL_PGM & XCP_RESOURCE))
    else if ((XCP_CMD_PROGRAM == pid)
    && (dataPtr[XCP_PROGRAM_LEN_OFFSET] > XCP_PROGRAM_SIZE)
#if (XCP_PL_PGM == (XCP_PL_PGM & XCP_RESOURCE))
    && (XCP_PGM_IDLE != Xcp_PgmStauts)
    && (XCP_PGM_START != Xcp_PgmStauts)
#endif
    )
    {
        /* the length of the *_next shall be check here to prevent the overload
...
Xcp_RxBlockHal(const uint8 pid, const PduLengthType len,P2CONST(uint8, AUTOMATIC, XCP_APPL_DATA) dataPtr)
{
    ...
    if (pid == Xcp_CmdBuffer[XCP_PID_OFFSET])
    {
        ...
        && (dataPtr[XCP_PROGRAM_LEN_OFFSET] == (Xcp_BlockBufferLen - Xcp_BlockBufferPos)))
        {
            ...
            lenCounter = XCP_PROGRAM_DATA_OFFSET + XCP_AG;
            for (pos = 0u; lenCounter <= lenMax; pos++)
            {
                destPtr[pos] = sourPtr[pos];
                lenCounter += XCP_AG;
            }
 ...
 */
/******************************************************************************/
FUNC(void, XCP_CODE) Xcp_TxConfirmation(const PduIdType pduId)
{

...
#define XCP_PGM_C_SW_MAJOR_VERSION  2u
...
#include "XCP_MemMap.h"
static P2VAR(uint8, AUTOMATIC, AUTOMATIC) Xcp_FlsBufAlignPtr;
...
Xcp_CheckPgmStatus(void);

static FUNC(Std_ReturnType, XCP_CODE)
Xcp_FlsReq(
    uint8 Mode,
    Xcp_FlsAddressType u4Address,
    Xcp_FlsLengthType u4Size,
    const uint8* pu1DataPtr);

static FUNC(uint8, XCP_CODE)
Xcp_FlsGetStatus(void);

static FUNC(Std_ReturnType,XCP_CODE)
    Xcp_Program_ZeroHandle(void);

static FUNC(Std_ReturnType,XCP_CODE)
    Xcp_Program_NonZero
    (uint8 dataCount, uint8 pgmSize);
...
static FUNC(Xcp_SeqCheckType,XCP_CODE) Xcp_GetPgm_Functional_StartAddr
    (P2VAR(uint32, AUTOMATIC, XCP_VAR) startAddr, uint8 noOfBytes,uint32 rangType);

static FUNC(Xcp_SeqCheckType,XCP_CODE)
    Xcp_GetPgmClearStartAddr
    (uint32 clearRange, uint32 *startAddr, uint32 *clearSize);
...
Xcp_CheckPgmMem(uint32 addr, uint32 size)
{
    ...
    {
        if ((addr >= (Xcp_SectorInfo[secNum].progStartAddr))
        && (size <= Xcp_SectorInfo[secNum].progDataSize)
        )
        {
            result = TRUE;
            break;
...
static FUNC(Std_ReturnType,XCP_CODE) Xcp_Program_ZeroHandle(void)
{
    ...
    if (0u != Xcp_FlsBufferSize)
    {
        ...
        else
        {
            ...
            if ((Std_ReturnType)E_OK == Xcp_FlsReq(XCP_FLS_WRITE, Xcp_FlsTargetAddr, XCP_FLS_MIN_WRITE_SIZE, Xcp_FlsBufAlignPtr))
            {
...
static FUNC(void, XCP_CODE) Xcp_FlsClearTask(void)
{
    if ((Std_ReturnType)E_OK == Xcp_FlsReq(XCP_FLS_ERASE, Xcp_ClearStartAddr,
...
static FUNC(void, XCP_CODE) Xcp_FlsProgramTask(void)
{
    if ((Std_ReturnType)E_OK == Xcp_FlsReq(XCP_FLS_WRITE, Xcp_FlsTargetAddr,
                     XCP_FLS_MIN_WRITE_SIZE, Xcp_FlsBufAlignPtr))
    {
        Xcp_Program_RetryCounters = 0u;
...
FUNC(void, XCP_CODE) Xcp_ProgramInit(void)
{
    ...
    cnt = 0u;
    while (((uint32)(&Xcp_FlsBuffer[cnt]) % 4u) != 0u)
    {
        cnt++;
    }
...
FUNC(void, XCP_CODE)  Xcp_ProgramNext(void)
{
    ...
    if (((Std_ReturnType)E_OK == Xcp_CheckPgmStatus())
            && (XCP_PGM_PROGRAMMING != Xcp_PgmStauts))
    {

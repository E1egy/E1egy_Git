...
*******************************************************************************/
#define NM_C_AR_RELEASE_MAJOR_VERSION                        19u
#define NM_C_AR_RELEASE_MINOR_VERSION                        11u
#define NM_C_AR_RELEASE_PATCH_VERSION                        0u
#define NM_C_SW_MAJOR_VERSION                                2u
#define NM_C_SW_MINOR_VERSION                                0u
#define NM_C_SW_PATCH_VERSION                                3u
/*******************************************************************************
**                      Include Section                                       **
*******************************************************************************/
...
Nm_FindChannelIndex(
    ...
    P2VAR(uint8, AUTOMATIC, NM_APPL_DATA) chIndexPtr
);
...
static FUNC(Std_ReturnType, NM_CODE)
Nm_BusNmNetworkRequest(
    uint8 chIndex
);

/* special bus network release */
static FUNC(Std_ReturnType, NM_CODE)
Nm_BusNmNetworkRelease(
    uint8 chIndex
);
...
static FUNC(Std_ReturnType, NM_CODE)
Nm_BusNmRequestBusSynchronization(
    uint8 chIndex
);
#endif/* (STD_ON == NM_COORDINATOR_SUPPORT_ENABLED)&&\
(STD_ON == NM_BUS_SYNCHRONIZATION_ENABLED) */
...
#include "Nm_MemMap.h"
static VAR(boolean, NM_VAR_CLEARED_BOOLEAN)
Nm_InitStatus = FALSE;
...
#include "Nm_MemMap.h"
static VAR(Nm_InnerChannelType, NM_VAR_NO_INIT_UNSPECIFIED)
Nm_InnerChannel[NM_NUMBER_OF_CHANNELS];
...
#define NM_START_SEC_VAR_CLEARED_UNSPECIFIED
#include "Nm_MemMap.h"
static VAR(Nm_InnerClusterType, NM_VAR_NO_INIT_UNSPECIFIED)
Nm_InnerCluster[NM_NUMBER_OF_CLUSTERS];
#define NM_STOP_SEC_VAR_CLEARED_UNSPECIFIED
#include "Nm_MemMap.h"
...
#include "Nm_MemMap.h"
static P2CONST(Nm_ConfigType, NM_CONST, NM_APPL_DATA)
Nm_LocalCfgPtr = NULL_PTR;
...
)
{
    ...
    if((NULL_PTR == ConfigPtr) ||
       ((0u < ConfigPtr->numOfClu) && (NULL_PTR == ConfigPtr->perCluPtr)))
    {
        NM_DET_REPORT(
            NM_SERVICE_ID_INIT,
            NM_E_PARAM_POINTER);
    ...
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(NetworkHandle, &chIndex))
        {
            ...
            if(0xff !=  Nm_ChLConfig[chIndex].NmCoordClusterIndex)
            {
                ...
                if ((uint8)E_OK == Nm_FindClusterIndex(chIndex, &clusterIndex))
                {
                    if((NM_SHUTDOWN_STU_RUN ==
                        Nm_InnerCluster[clusterIndex].shutdownStatus)
            ...
            {
                ComM_Nm_NetworkMode(
                    Nm_ChLConfig[chIndex].NmComMChannelRef.comMChannel);
                ret = E_OK;
                #if (STD_ON == NM_COORDINATOR_SUPPORT_ENABLED)
...
)
{
    ...
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if (STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_DISABLECOMMUNICATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    ...
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if (STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_ENABLECOMMUNICATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
#if ((STD_OFF == NM_COM_USER_DATA_SUPPORT)&&\
...
)
{
    ...
    else if(TRUE == Nm_InitStatus)
    {
        ...
        if(TRUE == Nm_FindChannelIndex(NetworkHandle, &chIndex))
        {
            if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
            {
                BusNm_SetUserData_Type specific_SetUserData =
                    Nm_ChLConfig[chIndex].busNmApi->BusNm_SetUserData;
...
)
{
    ...
    else if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(NetworkHandle, &chIndex))
        {
            if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
            {
                BusNm_GetUserData_Type specific_GetUserData =
                        Nm_ChLConfig[chIndex].busNmApi->BusNm_GetUserData;
...
)
{
    ...
    else if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(NetworkHandle, &chIndex))
        {
            if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
            {
                BusNm_GetPduData_Type specific_GetPduData =
                    Nm_ChLConfig[chIndex].busNmApi->BusNm_GetPduData;
...
)
{
    ...
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if (STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_REPEATMESSAGEREQUEST,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    ...
    if(NULL_PTR == nmNodeIdPtr)
    {
        #if (STD_ON == NM_DEV_ERROR_DETECT)
        NM_DET_REPORT(
            NM_SERVICE_ID_GETNODEIDENTIFIER,
            NM_E_PARAM_POINTER);
        #endif/* STD_ON == NM_DEV_ERROR_DETECT */
    }
    else if((boolean)TRUE == Nm_InitStatus)
...
)
{
    ...
    if(NULL_PTR == nmNodeIdPtr)
    {
        #if (STD_ON == NM_DEV_ERROR_DETECT)
        NM_DET_REPORT(
            NM_SERVICE_ID_GETLOCALNODEIDENTIFIER,
            NM_E_PARAM_POINTER);
        #endif/* STD_ON == NM_DEV_ERROR_DETECT */
    }
    else if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(NetworkHandle, &chIndex))
        {
            if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
            {
                ...
                {
                    ret = specific_GetLocalNodeIdentifier(NetworkHandle,
                                                      nmNodeIdPtr);
                }
            }
        }
...
)
{
    ...
    if(NULL_PTR == nmRemoteSleepIndPtr)
    {
        #if (STD_ON == NM_DEV_ERROR_DETECT)
        NM_DET_REPORT(
            NM_SERVICE_ID_CHECKREMOTESLEEPINDICATION,
            NM_E_PARAM_POINTER);
        #endif/* STD_ON == NM_DEV_ERROR_DETECT */
    }
    else if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(nmNetworkHandle, &chIndex))
        {
            if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
            {
                ...
                {
                    ret = spf_CheckRemoteSleepIndication(nmNetworkHandle,
                                                     nmRemoteSleepIndPtr);
                }
            }
        }
...
)
{
    ...
    else if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(nmNetworkHandle, &chIndex))
        {
            if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
            {
                BusNm_GetState_Type specific_GetState =
                    Nm_ChLConfig[chIndex].busNmApi->BusNm_GetState;
...
)
{
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_NETWORKSTARTINDICATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_PREPAREBUSSLEEPMODE,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_REMOTESLEEPINDICATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_REMOTESLEEPCANCELLATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(nmNetworkHandle, &chIndex))
        {
            ...
            if((boolean)TRUE == Nm_ChLConfig[chIndex].NmSynchronizingNetwork)
            {
                ...
                */
                ...
                {
                    Nm_InnerChannel[chIndex].lowLayBusSyncPoint =
                        Nm_InnerChannel[chIndex].remoteSleepIndFlg;
                }
                else
                {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_SYNCHRONIZATIONPOINT,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_COORDREADYTOSLEEPINDICATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
...
)
{
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_COORDREADYTOSLEEPCANCELLATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
    ...
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(nmNetworkHandle, &chIndex))
        {
            ...
            if((boolean)TRUE == Nm_ChLConfig[chIndex].NmStateReportEnabled)
            {
                ...
                for(index = 0u;
                    (index < NM_UBOND(Nm_StateReportKind)) && ((boolean)FALSE == findflag);
                    index++)
                {
                    if(
                        (nmCurrentState == Nm_StateReportKind[index].nmCurSta) &&
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_STATECHANGENOTIFICATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
    ...
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        if((boolean)TRUE == Nm_FindChannelIndex(nmChannelHandle, &chIndex))
        {
            if(NULL_PTR != Nm_GlobalLConfig.NmCarWakeUpCallout)
            {
                ...
                Nm_GlobalLConfig.NmCarWakeUpCallout(
                    Nm_ChLConfig[chIndex].NmComMChannelRef.comMChannel);
            }
            else
            {
        ...
        #endif/* 0u < NM_NUMBER_OF_CHANNELS */
        {
            #if(STD_ON == NM_DEV_ERROR_DETECT)
            NM_DET_REPORT(
                NM_SERVICE_ID_CARWAKEUPINDICATION,
                NM_E_INVALID_CHANNEL);
            #endif/* STD_ON == NM_DEV_ERROR_DETECT */
        }
    }
    ...
    if((boolean)TRUE == Nm_InitStatus)
    {
        ...
        for(clusterIndex = 0u; clusterIndex < Nm_LocalCfgPtr->numOfClu; clusterIndex++)
        {
            ...
            const Nm_PerClusterConfigType* perClusPtr =
                &Nm_LocalCfgPtr->perCluPtr[clusterIndex];
            if( NM_SHUTDOWN_STU_START ==
                Nm_InnerCluster[clusterIndex].shutdownStatus)
            {
...
)
{
    ...
    for(chIndex = 0u;
        ((chIndex < NM_NUMBER_OF_CHANNELS) && (findChannelFlg == (boolean)FALSE));
        chIndex++)
    {
        if(NetworkHandle ==
           Nm_ChLConfig[chIndex].NmComMChannelRef.comMChannel)
...
)
{
    ...
    {
        for(index = 0u;
            (index < Nm_LocalCfgPtr->numOfClu) && ((uint8)E_NOT_OK == findFlg);
            index++)
        {
            if(Nm_ChLConfig[chIndex].NmCoordClusterIndex ==
               Nm_LocalCfgPtr->perCluPtr[index].coorClusterIndex)
            {
                findFlg = E_OK;
...
)
{
    ...
    for(index = 0u;
        (index < perClusPtr->incNumCh) && ((boolean)TRUE == CoorShutdownReadyFlag);
        index++)
    {
        chIndex = perClusPtr->incCh[index];
        /* new check mode */
        ...
        if((boolean)TRUE == Nm_InnerChannel[chIndex].busAwakeFlg)
        {
            ...
            else
            {
                ...
                */
                if(
                    (((boolean)FALSE == Nm_ChLConfig[chIndex].NmChannelSleepMaster) &&
                    #if (STD_ON == NM_COORDINATOR_SYNC_SUPPORT)
    ...
    if((uint8)E_OK == ret)
    {
        for(index = 0u; index < perClusPtr->incNumCh; index++)
        {
            ...
            if((boolean)TRUE == Nm_InnerChannel[chIndex].busAwakeFlg)
            {
                ...
                if((NM_GLOBAL_COORDINATOR_TIME <
                    Nm_ChLConfig[chIndex].busNmShutdownTime)
                    #if (STD_ON == NM_COORDINATOR_SYNC_SUPPORT)
...
)
{
    ...
    if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
    {
        BusNm_NetworkRequest_Type specific_NetworkRequest =
                Nm_ChLConfig[chIndex].busNmApi->BusNm_NetworkRequest;
        if(NULL_PTR != specific_NetworkRequest)
        {
            ret = specific_NetworkRequest(
                      Nm_ChLConfig[chIndex].NmComMChannelRef.comMChannel);
            #if (STD_ON == NM_COORDINATOR_SUPPORT_ENABLED)
...
)
{
    ...
    if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
    {
        BusNm_NetworkRequest_Type specific_NetworkRelease =
            Nm_ChLConfig[chIndex].busNmApi->BusNm_NetworkRelease;
        if(NULL_PTR != specific_NetworkRelease)
        {
            ret = specific_NetworkRelease(
                  Nm_ChLConfig[chIndex].NmComMChannelRef.comMChannel);
            #if (STD_ON == NM_COORDINATOR_SUPPORT_ENABLED)
            Nm_InnerChannel[chIndex].lowLayNetRequest = FALSE;
            #endif/* STD_ON == NM_COORDINATOR_SUPPORT_ENABLED */
...
)
{
    ...
    if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
    {
        ...
        {
            ret = specific_RequestBusSynchronization(
                  Nm_ChLConfig[chIndex].NmComMChannelRef.comMChannel);
        }
    }
    return ret;
...
)
{
    ...
    if(NULL_PTR != Nm_ChLConfig[chIndex].busNmApi)
    {
        BusNm_SetSleepReadyBit_Type specific_SetSleepReadyBit =
            Nm_ChLConfig[chIndex].busNmApi->BusNm_SetSleepReadyBit;
        if(NULL_PTR != specific_SetSleepReadyBit)
        {
            ret = specific_SetSleepReadyBit(
                  Nm_ChLConfig[chIndex].NmComMChannelRef.comMChannel,
                  nmSleepReadyBit);
        }
    }

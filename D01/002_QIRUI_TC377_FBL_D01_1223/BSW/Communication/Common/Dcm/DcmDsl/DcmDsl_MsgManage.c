...
#define  DCM_START_SEC_VAR_NOINIT_8
#include "Dcm_MemMap.h"
VAR(uint8,DCM_VAR_POWER_ON_INIT)Dcm_Channel[DCM_CHANNEL_LENGTH];
#define  DCM_STOP_SEC_VAR_NOINIT_8
#include "Dcm_MemMap.h"
...
#include "Dcm_MemMap.h"
static   VAR(boolean,DCM_VAR_POWER_ON_INIT)CheckProtocolFaild = FALSE;
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void, DCM_CODE)Dcm_MemoryCopy(
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static  FUNC(Std_ReturnType, DCM_CODE) Dcm_CopyRxData_CheckProtocol(
        PduIdType id,Dcm_ServiceType  ServiceType)
{
    ...
    if(DCM_PROTOCOL_VALID != Dcm_ProtocolCtrl[ProtocolCfgId].Dcm_ProtocolState)
    {
        ...
        for (iloop = 0; (iloop < DCM_DSLPROTOCOLROW_NUM_MAX) &&
        (FindProtocolValid == FALSE); iloop++)
        {
        ...
        if (FindProtocolValid == TRUE)
        {
            ...
            else
            {
                ...
                {
                    SchM_Enter_Dcm(Dcm_MsgCtrl);
                    Dcm_MsgCtrl[ProtocolCfgId].SID =
                            Dcm_MsgCtrl[ProtocolCfgId].MsgContext.pReqData[0];
                    SchM_Exit_Dcm(Dcm_MsgCtrl);
                }
            }
            OldProtocolPri = Dcm_ProtocolCtrl[FindProtocolCfgId].ProtocolPri;
            NewProtocolPri = (Dcm_DslCfg.pDcmDslProtocol->pDcmDslProtocolRow)[ProtocolCfgId].
...
#if (PREEMPTION_PROTOCOL_CANCEL_SUPPORT == STD_ON)
                        DcmTxPduIdx   = Dcm_MsgCtrl[Dcm_ProtocolCtrl[FindProtocolCfgId].
                                                    MsgCtrlIndex].DcmTxPduId;
...
#endif
                        SchM_Enter_Dcm(Dcm_MsgCtrl);
                        Dcm_MsgCtrl[Dcm_ProtocolCtrl[FindProtocolCfgId].MsgCtrlIndex].
                        Dcm_OpStatus = DCM_CANCEL;
                        SchM_Exit_Dcm(Dcm_MsgCtrl);
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(BufReq_ReturnType, DCM_CODE) Dcm_StartOfReception_metadata(
        PduIdType id,
        P2CONST(PduInfoType, AUTOMATIC, DCM_VAR)  info)
{
    ...
    ProtocolCfgCtrlId   = Dsl_Protocol_ConnectionCfg[ConnectionCfgCtrlId].
            DcmDslParentProtocolRowCtrlId;
    MsgCtrlIndex        = ProtocolCfgCtrlId;
    RxAddrType = Dsl_Protocol_Connection_RxCfg[id].DcmDslProtocolRxAddrType;

    MetaDataLen = Dsl_Protocol_Connection_RxCfg[id].DcmDslMetaDataFlag
            & DCM_METADATA_LENGTH_MASK;
    if ((MetaDataLen > 0u) && ((DCM_ADDRESS_EXTENSION_8_MASK ==
            (Dsl_Protocol_Connection_RxCfg[id].DcmDslMetaDataFlag
                    & DCM_ADDRESS_EXTENSION_8_MASK))
            || (DCM_SA16_AND_TA16_MASK == (Dsl_Protocol_Connection_RxCfg[id].
                    DcmDslMetaDataFlag & DCM_SA16_AND_TA16_MASK))))
    {
        SchM_Enter_Dcm(Dcm_MsgCtrl);
        if (((Dsl_Protocol_Connection_RxCfg[id].DcmDslMetaDataFlag
                & DCM_SA16_AND_TA16_MASK) == DCM_SA16_AND_TA16_MASK)
            && (NULL_PTR != info))
        {
#if (CPU_BYTE_ORDER == HIGH_BYTE_FIRST)
            Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_SA =
                    (uint16)((uint16)info->MetaDataPtr[0u] << 8u) | info->MetaDataPtr[1u];
            Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_TA =
...
#endif
            Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_TA_Backup =
                    Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_TA;
            Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_SA_Backup =
                    Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_SA;
        }
        if (((Dsl_Protocol_Connection_RxCfg[id].DcmDslMetaDataFlag
                & DCM_ADDRESS_EXTENSION_8_MASK) == DCM_ADDRESS_EXTENSION_8_MASK)
            && (NULL_PTR != info))
        {
            Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_AE = info->MetaDataPtr[4u];
            Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_AE_Backup =
                    Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_AE;
        }
        SchM_Exit_Dcm(Dcm_MsgCtrl);
        if(DCM_PHYSICAL == RxAddrType)
        {
            if (Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MetaData_TA !=
                    Dsl_ProtocolRowCfg[ProtocolCfgCtrlId].pDcmDslConnection->pDcmDslMainConnection->
        ...
        P2VAR(PduLengthType, AUTOMATIC, DCM_VAR) bufferSizePtr)
{
    ...
    ProtocolCfgCtrlId   = Dsl_Protocol_ConnectionCfg[ConnectionCfgCtrlId].
            DcmDslParentProtocolRowCtrlId;
    MsgCtrlIndex        = ProtocolCfgCtrlId;
    RxChannelCfgCtrlId  = Dsl_ProtocolRowCfg[ProtocolCfgCtrlId].
            DcmDslProtocolRxBufferRef->Dcm_DslBufferId;
    TxChannelCfgCtrlId  = Dsl_ProtocolRowCfg[ProtocolCfgCtrlId].
    ...
    else
    {
                ...
                ||(Dcm_ChannelCtrl[TxChannelCfgCtrlId].Dcm_ChannelTxState == DCM_CH_OCCUPIED))
        {
            if (FALSE == Dcm_ConnectionCtrl[ConnectionCfgCtrlId].Dcm_ConnectionActive)
            {
                ...
                {
                    if (TRUE == Dcm_DslCfg.pDcmDslDiagRespCfg->
                            DcmDslDiagRespOnSecondDeclinedRequest)
                    {
                        ...
                        DsdInternal_SetMsgState(ProtocolCfgCtrlId,DCM_MSG_PROCESSED);
    ...
    if (BUFREQ_OK == bufRet)
    {
        if ((info != NULL_PTR) && (len < info->SduLength))
        {
            *bufferSizePtr = 0;
        }
        else
                    ...
                    if(DCM_FUNCTIONAL == RxAddrType)
                    {
                        switch (ServiceType)
                        {
                            ...
                            case DCM_UDS:
                            {
                                   /*The UDS protocol functions addressing message, bypass treatment*/
                                    Dcm_MemoryCopy(info->SduDataPtr,
                                           &(Dcm_FunctionalMessage.Buffer[0]),info->SduLength);
                            ...
                            case DCM_OBD:
                            {
                                    ...
                                    Dcm_MemoryCopy(info->SduDataPtr,
                                            &(Dcm_OBDMessage.Buffer[0]),info->SduLength);
        ...
        P2VAR(PduLengthType,AUTOMATIC,DCM_VAR) bufferSizePtr)
{
...
#endif
    {
        ConnectionCfgId = Dsl_Protocol_Connection_RxCfg[id].DcmDslParentConnectionCtrlId;
        ProtocolCfgId  = Dsl_Protocol_ConnectionCfg[ConnectionCfgId].DcmDslParentProtocolRowCtrlId;
        RxChannelCfgCtrlId = Dsl_ProtocolRowCfg[ProtocolCfgId].DcmDslProtocolRxBufferRef->
                Dcm_DslBufferId;
        RxAddrType = Dsl_Protocol_Connection_RxCfg[id].DcmDslProtocolRxAddrType;
        ProtocolRowID = ((Dcm_DslCfg.pDcmDslProtocol)->pDcmDslProtocolRow)[ProtocolCfgId].
                DcmDslProtocolID;
        ...
        if (DCM_FUNCTIONAL == RxAddrType)
        {
            switch (ServiceType)
            {
                ...
                case DCM_UDS:
                   /*The UDS protocol functions addressing message, bypass treatment*/
                    Dcm_MemoryCopy(info->SduDataPtr,&(Dcm_FunctionalMessage.
                            Buffer[Dcm_FunctionalMessage.Length]),info->SduLength);
                ...
                case DCM_OBD:
                    /*The OBD protocol only have functions addressing message*/
                    Dcm_MemoryCopy(info->SduDataPtr,&(Dcm_OBDMessage.
                            Buffer[Dcm_OBDMessage.Length]),info->SduLength);
                    Dcm_OBDMessage.Length += info->SduLength;/*save request length*/
                    break;
#endif
                default:
        ...
        if(!((0x3Eu == Sid) && (0x80u == SubFunction) && (0x02u == Dcm_FunctionalMessage.Length)))
        {
            ...
            else
            {
                ...
                SchM_Exit_Dcm(Dcm_ChannelCtrl);
                SchM_Enter_Dcm(Dcm_MsgCtrl);
                Dcm_MsgCtrl[ProtocolCfgId].MsgContext.Dcm_oldBufferCunrentPosition = Dcm_ChannelCtrl[RxChannelCfgCtrlId].Dcm_BufferCunrentPosition;
                SchM_Exit_Dcm(Dcm_MsgCtrl);
                *bufferSizePtr = Dcm_DslCfg.pDcmChannelCfg[RxChannelCfgCtrlId].Dcm_DslBufferSize
                        - (Offset - (Dcm_DslCfg.pDcmChannelCfg)[RxChannelCfgCtrlId].offset);
                bufRet = BUFREQ_OK;
            }
        }
        ...
        {
            SchM_Enter_Dcm(Dcm_ChannelCtrl);
            Dcm_ChannelCtrl[RxChannelCfgCtrlId].Dcm_BufferCunrentPosition =
                    Dcm_MsgCtrl[ProtocolCfgId].MsgContext.Dcm_oldBufferCunrentPosition;
            SchM_Exit_Dcm(Dcm_ChannelCtrl);
            SchM_Enter_Dcm(Dcm_MsgCtrl);
            Dcm_MsgCtrl[ProtocolCfgId].MsgContext.Dcm_oldBufferCunrentPosition = 0;
...
#if (STD_ON == DCM_GENERIC_CONNECTION)
            Dcm_MsgCtrl[ProtocolCfgId].Dcm_MetaData_AE =
                    Dcm_MsgCtrl[ProtocolCfgId].Dcm_MetaData_AE_Backup;
            Dcm_MsgCtrl[ProtocolCfgId].Dcm_MetaData_SA =
                    Dcm_MsgCtrl[ProtocolCfgId].Dcm_MetaData_SA_Backup;
            Dcm_MsgCtrl[ProtocolCfgId].Dcm_MetaData_TA =
                    Dcm_MsgCtrl[ProtocolCfgId].Dcm_MetaData_TA_Backup;
...
static  FUNC(Std_ReturnType,DCM_CODE)Dcm_TpRxIndication_IsFuncAddress(PduIdType DcmRxPduId)
{
    ...
    if(DCM_FUNCTIONAL == DslProtocolRxAddrType)
    {
        ...
        if((E_OK == ret) && (DCM_MSG_WAIT != Dcm_MsgCtrl[MsgCtrlIndex].Dcm_MsgState))
        {
        ...
        if(E_OK == ret)
        {
            ...
            SchM_Enter_Dcm(Dcm_MsgCtrl);
            Dcm_MsgCtrl[MsgCtrlIndex].MsgContext.ReqDataLen =
                    (Dcm_MsgLenType)Dcm_FunctionalMessage.Length;
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static  FUNC(void,DCM_CODE)Dcm_TpRxIndication_SubDeal(uint8 ProtocolCfgCtrlId)
{
    NetworkHandleType NetworkChannel;
    Std_ReturnType  ret;
    uint8    Index;
    ...
    pDslConnection = Dcm_DslCfg.pDcmDslProtocol->pDcmDslProtocolRow[ProtocolCfgCtrlId].
            pDcmDslConnection;
    NetworkChannel = pDslConnection->pDcmDslMainConnection->
            DcmDslProtocolComMChannelId;
    if (StopProtocolFaild == TRUE)
    {
        ...
        Dcm_MsgCtrl[Dcm_ProtocolCtrl[ProtocolCfgCtrlId].MsgCtrlIndex].
          Dcm_MsgState = DCM_MSG_PROCESSED;
        SchM_Exit_Dcm(Dcm_MsgCtrl);
        DsdInternal_ProcessingDone(ProtocolCfgCtrlId);
    }
    else
        ...
        else
        {
            for (Index = 0; Index < DCM_MAINCONNECTION_NUM; Index++)
            {
                        ...
                        Dcm_CommCtrl[Index].DcmDslProtocolComMChannelId)
                {
                    ...
                    if((DCM_COMM_ACTIVE == Dcm_CommCtrl[Index].
                            Dcm_ActiveDiagnostic) &&
                            (DCM_DEFAULT_SESSION == Dcm_MkCtrl.Dcm_ActiveSes))
                    {
...
FUNC(void,DCM_CODE)Dcm_TpRxIndication(PduIdType id, Std_ReturnType result)
{
...
#endif
    {
        ...
        ConnectionCfgId = Dsl_Protocol_Connection_RxCfg[id].DcmDslParentConnectionCtrlId;
        ProtocolCfgCtrlId  = Dsl_Protocol_ConnectionCfg[ConnectionCfgId].
                DcmDslParentProtocolRowCtrlId;
        ...
        SchM_Enter_Dcm(Dcm_ProtocolCtrl);
        Dcm_ProtocolCtrl[ProtocolCfgCtrlId].ProtocolId  = (Dcm_DslCfg.pDcmDslProtocol->
                pDcmDslProtocolRow)[ProtocolCfgCtrlId].DcmDslProtocolID;
        Dcm_ProtocolCtrl[ProtocolCfgCtrlId].ProtocolPri = (Dcm_DslCfg.pDcmDslProtocol->
                pDcmDslProtocolRow)[ProtocolCfgCtrlId].DcmDslProtocolPriority;
        Dcm_ProtocolCtrl[ProtocolCfgCtrlId].ProtocolPreemptTime = (Dcm_DslCfg.pDcmDslProtocol->
                pDcmDslProtocolRow)[ProtocolCfgCtrlId].DcmDslProtocolPreemptTime;
        ...
        else
        {
            ...
            if(E_OK == ret)
            {
                ...
                else
                {
                    ...
                    {
                        SchM_Enter_Dcm(Dcm_MsgCtrl);
                        Dcm_MsgCtrl[MsgCtrlIndexx].MsgContext.ReqDataLen =
                                (Dcm_MsgLenType)Dcm_OBDMessage.Length;
                    ...
                    {
                        SchM_Enter_Dcm(Dcm_MsgCtrl);
                        Dcm_MsgCtrl[MsgCtrlIndexx].SID = Dcm_MsgCtrl[MsgCtrlIndexx].
                                MsgContext.pReqData[0];
                        Dcm_MsgCtrl[MsgCtrlIndexx].Subfunction =
                                (Dcm_MsgCtrl[MsgCtrlIndexx].MsgContext.pReqData)[1];
                        SchM_Exit_Dcm(Dcm_MsgCtrl);
                    }
                    SchM_Enter_Dcm(Dcm_MsgCtrl);
                    Dcm_MsgCtrl[MsgCtrlIndexx].MsgContext.MsgAddInfo.ReqType =
                            (uint8)DslProtocolRxAddrType;
                    ...
                    {
#if (STD_ON == DCM_UDS_SERVICE0X86_ENABLED)
                        if (TRUE == Dcm_UDS0x86_GetTxPduId(
                    ...
                    {
                        Dcm_TpRxIndication_SubDeal(ProtocolCfgCtrlId);
                    }
                }
            }
        }
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(BufReq_ReturnType,DCM_CODE)Dcm_CopyTxData_PagebufferDeal(uint8  ProtocolCfgId,
        P2CONST(PduInfoType, AUTOMATIC, DCM_VAR)  info,
        uint8  TxChannelCtrlIndex,
        ...
        )
{
    ...
    else
    {
        ...
        {
            if ((Dcm_PageBufferData.ThisPageTxSize + info->SduLength) >
            Dcm_PageBufferData.ThisPageSize)
            {
                /*last one message not has 8 byte, memory it for next page to send*/
                if ((Dcm_PageBufferData.ThisPageSize -
                        Dcm_PageBufferData.ThisPageTxSize) != 0u)
                {
                    SchM_Enter_Dcm(Dcm_PageBufferData);
                    Dcm_PageBufferData.LastNotTxDataSize = Dcm_PageBufferData.
                            ThisPageSize - Dcm_PageBufferData.ThisPageTxSize;
                    Dcm_MemoryCopy( &(Dcm_PageBufferData.
                            pResData[Dcm_PageBufferData.ThisPageTxSize]),
            ...
            else
            {
                if (info->SduLength != 0u)
                {
                    ...
                    Dcm_PageBufferData.pResData = &(Dcm_Channel[(Dcm_DslCfg.
                            pDcmChannelCfg)[Dcm_ChannelCtrl[TxChannelCtrlIndex].
                                            Dcm_ChannelCfgIndex].offset]);
        ...
        P2VAR(PduLengthType,AUTOMATIC,DCM_VAR) availableDataPtr)
{
...
#endif
    {
        ...
        else
        {
            ...
            ProtocolCfgId = Dsl_Protocol_ConnectionCfg[ConnectionCfgId].
                    DcmDslParentProtocolRowCtrlId;
            MsgCtrlId = Dcm_ProtocolCtrl[ProtocolCfgId].MsgCtrlIndex;
            ...
            {
                bufRet = Dcm_CopyTxData_PagebufferDeal(
                        ProtocolCfgId,info,TxChannelCtrlIndex,availableDataPtr);
            }
            else
#endif
            {
                ...
                else
                {
                    ...
                    if (info->SduLength != 0u)
                    {
                        ...
                        SchM_Enter_Dcm(Dcm_ChannelCtrl);
                        Dcm_ChannelCtrl[TxChannelCtrlIndex].Dcm_BufferCunrentPosition =
                                Position + info->SduLength;
                        SchM_Exit_Dcm(Dcm_ChannelCtrl);
                    }

                    *availableDataPtr = (PduLengthType)(Dcm_MsgCtrl[MsgCtrlId].
                            MsgContext.ResMaxDataLen -
                            Dcm_ChannelCtrl[TxChannelCtrlIndex].Dcm_BufferCunrentPosition);
                    bufRet = BUFREQ_OK;
               }
            }
...
FUNC(void,DCM_CODE)DslInternal_ProcessingDone(uint8 ProtocolCtrlId)
{
    ...
    if ((Dsl_Protocol_Connection_RxCfg[DcmRxPduIdx].DcmDslMetaDataFlag
            & DCM_METADATA_LENGTH_MASK) > 0u)
    {
        if ((Dsl_Protocol_Connection_RxCfg[DcmRxPduIdx].DcmDslMetaDataFlag
            & DCM_ADDRESS_EXTENSION_8_MASK) == DCM_ADDRESS_EXTENSION_8_MASK)
        {
        ...
        {
            #if (CPU_BYTE_ORDER == HIGH_BYTE_FIRST)
            PduInfo.MetaDataPtr[0u] =
                    (uint8)((Dcm_MsgCtrl[MsgCtrlIndexx].Dcm_MetaData_TA & 0xFF00u) >> 8u);
            PduInfo.MetaDataPtr[1u] =
                    (uint8)(Dcm_MsgCtrl[MsgCtrlIndexx].Dcm_MetaData_TA & 0xFFu);
            PduInfo.MetaDataPtr[2u] =
                    (uint8)((Dcm_MsgCtrl[MsgCtrlIndexx].Dcm_MetaData_SA & 0xFF00u) >> 8u);
            PduInfo.MetaDataPtr[3u] =
                    (uint8)(Dcm_MsgCtrl[MsgCtrlIndexx].Dcm_MetaData_SA & 0xFFu);
            #else
            PduInfo.MetaDataPtr[0u] =
                    (uint8)(Dcm_MsgCtrl[MsgCtrlIndexx].Dcm_MetaData_TA & 0xFFu);
            PduInfo.MetaDataPtr[1u] =
        ...
        if (Dsl_Protocol_ConnectionCfg[Index].DcmDslParentProtocolRowCtrlId == ProtocolCtrlId)
        {
            for (iloop = 0; iloop < DCM_MAINCONNECTION_NUM; iloop++)
            {
                if((Dsl_Protocol_ConnectionCfg[Index].pDcmDslMainConnection != NULL_PTR)
                  && (Dsl_Protocol_ConnectionCfg[Index].pDcmDslMainConnection->
                          DcmDslProtocolComMChannelId
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE)DslInternal_ResponseOnOneEvent(
        P2VAR(PduInfoType,TYPEDEF,TYPEDEF) PduInfo,

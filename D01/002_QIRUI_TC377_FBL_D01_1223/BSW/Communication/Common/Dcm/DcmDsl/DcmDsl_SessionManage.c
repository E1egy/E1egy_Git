...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
extern void DcmDspSessionCallback(Dcm_SesCtrlType formerState, Dcm_SesCtrlType newState);
static FUNC(void,DCM_CODE) DslInternal_SesTranslation(Dcm_SesCtrlType NewSes)
{
    Dcm_SesCtrlType formerState;
#if (STD_ON == DCM_UDS_SERVICE0X86_ENABLED)
    Dcm_UDS0x86_SesTranslation(NewSes);
#endif

    DslInternal_InitDspProgramInfo();
    formerState = Dcm_SesCtrl.Dcm_ActiveSes;
    ...
    DcmDspSessionCallback(formerState,NewSes);
}
#define  DCM_STOP_SEC_CODE
#include "Dcm_MemMap.h"

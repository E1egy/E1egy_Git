...
#include "Dcm_MemMap.h"
static FUNC(void,DCM_CODE)DslInternal_SetProtocolTimer(uint8  ProtocolCtrlId,
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE)DslInternal_InitProtocolCtrl(uint8 ProtocolCtrlId)
{
...
#if(STD_ON == DCM_SESSION_FUNC_ENABLED)
    for(Index = 0; (Index < (Dcm_DspCfg.pDcm_DspSession->DcmDspSessionRow_Num)) &&
    (FALSE == Flag); Index++)
    {
       if(DCM_DEFAULT_SESSION ==
               (Dcm_DspCfg.pDcm_DspSession->pDcmDspSessionRow)[Index].DcmDspSessionLevel)
       {
...
FUNC(Std_ReturnType,DCM_CODE)DslInternal_ProtocolStart(uint8 ProtocolCtrlId)
{
    ...
    {
        ProtocolIdx = Dcm_DslCfg.pDcmDslProtocol->pDcmDslProtocolRow[ProtocolCtrlId].
                DcmDslProtocolID;
        Flag = FALSE;
        for(Index = 0; (Index < Dcm_DslCfg.DCMCallBackDcmRequest_PortNum) &&
        (FALSE == Flag); Index++)
        {
            ...
            else
            {
                ...
                ret = (*(Dcm_DslCfg.pDcmDslCallback_DCMRequestService[Index].
                        StartProtocol))(ProtocolIdx);
                switch(ret)
                {
                    case E_OK:
                         break;
        ...
        else
        {
            ...
            SchM_Switch_DcmDiagnosticSessionControl(
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE)DslInternal_SetProtocolLinkLayerTimer(
        uint32 P2ServerTimer,uint32 P2StarServerTimer)
{
   ...
   for(ProtocolCtrlId = 0; ProtocolCtrlId < Num; ProtocolCtrlId++)
   {
       ...
       P2ServerTimerLimit = Dcm_DslCfg.pDcmDslProtocol->
               pDcmDslProtocolRow[ProtocolCtrlId].DcmTimStrP2ServerAdjust;
       P2StarServerTimerLimit = Dcm_DslCfg.pDcmDslProtocol->
               pDcmDslProtocolRow[ProtocolCtrlId].DcmTimStrP2StarServerAdjust;

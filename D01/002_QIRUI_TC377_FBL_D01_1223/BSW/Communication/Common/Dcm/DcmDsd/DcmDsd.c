...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static  FUNC(Std_ReturnType,DCM_CODE)DsdInternal_AddressingFormatCheck(
        uint8  ProtocolCtrlId,uint8 Sid);
#define  DCM_STOP_SEC_CODE
#include "Dcm_MemMap.h"
           ...
           P2VAR(uint16,AUTOMATIC, DCM_VAR)pSidTabServieCfgIndex)
{
    ...
    if((NULL_PTR != pSidTabIndex) && (NULL_PTR != pSidTabServieCfgIndex))
    {
        ...
        SidTabId  = (Dcm_DslCfg.pDcmDslProtocol->pDcmDslProtocolRow)[ProtocolCtrlId].
                DcmDslServiceTableID;
        /*find SidTabId configuration position in the service configuration table*/
        for(SidTabIndex=0; (SidTabIndex < Dcm_DsdCfg.DcmDsdServiceTable_Num)
            && (FALSE == Flag); SidTabIndex++)
        {
        ...
        if (E_OK == ret)
        {
            ...
            Service_Num = (Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabIndex - 1u].
          ...
          P2VAR(uint16,AUTOMATIC, DCM_VAR)pSubServieCfgIndex)
{
    ...
    SubService_Num = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabIndex].
            pDcmDsdService)[ServiceIndex].DcmDsdSubService_Num;
    if(SubService_Num > 0u)
    {
        for (SubServiceIndex = 0; (SubServiceIndex < SubService_Num)
        && (FALSE == Flag); SubServiceIndex++)
        {
            if (SubFunctionId == (((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabIndex].
              pDcmDsdService)[ServiceIndex].DcmDsdSubService)[SubServiceIndex].DcmDsdSubServiceId)
            {
                Flag = TRUE;
            }
        }
...
static  FUNC(Std_ReturnType,DCM_CODE)DsdInternal_SuppressPosRsp(uint8 ProtocolCtrlId)
{
    ...
    ret = DsdInternal_SearchSidTabServiceIndex(
            Sid,ProtocolCtrlId,&SidTabCfgIndex,&SidTabServiceCfgIndex);
    if((E_OK == ret) && (TRUE == ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
            pDcmDsdService)[SidTabServiceCfgIndex].DcmDsdSubfuncAvial))
    {
        ...
        if((0x80u == (SubFunction & 0x80u)) && (TRUE == ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                pDcmDsdService)[SidTabServiceCfgIndex].DcmDsdSuppressPosRsp))
        {
        ...
        uint8  ProtocolCtrlId,uint8  Sid)
{
    ...
    if (E_OK == ret)
    {
        ...
        CfgServiceAddressingFormat = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                pDcmDsdService)[SidTabServiceCfgIndex].AddressingFormat;
        if(DCM_PHYANDFUNC != CfgServiceAddressingFormat)
        {
            /*Read request packet addressing type*/
            MsgCtrlIndexx = Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex;
            RxServiceAddressingFormat = Dcm_MsgCtrl[MsgCtrlIndexx].
                    MsgContext.MsgAddInfo.ReqType;
...
static  FUNC(Std_ReturnType,DCM_CODE)DsdInternal_ManufacturerIndication(uint8 ProtocolCtrlId)
{
    ...
    SourceAddress = Dsl_Protocol_ConnectionCfg[ConnectionCfgId].
            pDcmDslMainConnection->DcmDslProtocolRxTesterSourceAddr;
    for(PortIndex = 0; (PortIndex < Dcm_DsdCfg.DcmDsdServiceReqManufacturerNoti_PortNum) &&
    (FALSE == Flag); PortIndex++)
    {
        ...
        else
        {
            ret = (*((Dcm_DsdCfg.pDcmDsdServiceReqManufacturerNoti)[PortIndex].Indication))(
                    Sid,pReqDatax,ReqDatalen,ReqType,SourceAddress,&ErrorCode);
            switch(ret)
            {
                case E_OK:
                     break;
    ...
    {
        DsdInternal_SetMsgState(ProtocolCtrlId,DCM_MSG_PROCESSED);
...
static  FUNC(void,DCM_CODE)DsdInternal_ManufacturerConfirmation(uint8 ProtocolCtrlId)
{
    ...
    Dcm_DsdCfg.DcmDsdServiceReqManufacturerNoti_PortNum; PortIndex++)
    {
        (void)(*((Dcm_DsdCfg.pDcmDsdServiceReqManufacturerNoti)[PortIndex].Confirmation))(
                Sid,ReqType,SourceAddress,DCM_RES_NEG_NOT_OK);
    }
}
#define  DCM_STOP_SEC_CODE
#include "Dcm_MemMap.h"
...
static  FUNC(Std_ReturnType,DCM_CODE)DsdInternal_SupplierIndication(uint8 ProtocolCtrlId)
{
    ...
    SourceAddress =  Dsl_Protocol_ConnectionCfg[ConnectionCfgId].
            pDcmDslMainConnection->DcmDslProtocolRxTesterSourceAddr;
    for(PortIndex = 0; (PortIndex < Dcm_DsdCfg.DcmDsdServiceReqSupplierNoti_PortNum) &&
    (FALSE == Flag); PortIndex++)
    {
...
static  FUNC(void,DCM_CODE)DsdInternal_SupplierConfirmation(uint8 ProtocolCtrlId)
{
    ...
    SourceAddress =  Dsl_Protocol_ConnectionCfg[ConnectionCfgId].
            pDcmDslMainConnection->DcmDslProtocolRxTesterSourceAddr;
    for(PortIndex = 0; PortIndex <
    Dcm_DsdCfg.DcmDsdServiceReqSupplierNoti_PortNum; PortIndex++)
    {
...
FUNC(Std_ReturnType,DCM_CODE)DsdInternal_SesCheck(uint8 ProtocolCtrlId, uint8 Sid)
{
    ...
    if(E_OK == ret)
    {
        SesRefNum = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                pDcmDsdService)[SidTabServiceCfgIndex].DcmDsdSessionLevel_Num;
        if(0u != SesRefNum)
        {
            ...
            {
                if(ActiveSes == ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                    pDcmDsdService)[SidTabServiceCfgIndex].pDcmDsdSessionLevelRef[SesCfgIndex])
                {
                    Flag = TRUE;
                }
            }
...
FUNC(Std_ReturnType,DCM_CODE)DsdInternal_SubSesCheck(uint8 ProtocolCtrlId, uint8 Sid)
{
    ...
    if(E_OK == ret)
    {
        ret = DsdInternal_SearchSidTabSubServiceIndex(
                ProtocolCtrlId,SidTabCfgIndex,SidTabServiceCfgIndex,&SubServieCfgIndex);
    }
    if (E_OK == ret)
    {
        SubServiceNum = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                pDcmDsdService)[SidTabServiceCfgIndex].DcmDsdSubService_Num;
        if (0u != SubServiceNum)
        {
            pDcmDsdSubService = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                    pDcmDsdService)[SidTabServiceCfgIndex].DcmDsdSubService;
            SubSesRefNum = pDcmDsdSubService[SubServieCfgIndex].DcmDsdSubServiceSessionLevel_Num;
            if(0u != SubSesRefNum)
            {
                /*get Currently valid session state*/
                ActiveSes = Dcm_MkCtrl.Dcm_ActiveSes;
                for(SesCfgIndex=0; (SesCfgIndex < SubSesRefNum)
                    && (FALSE == Flag); SesCfgIndex++)
                {
                    if(ActiveSes == pDcmDsdSubService[SubServieCfgIndex].
                            DcmDsdSubServiceSessionLevelRef[SesCfgIndex])
                    {
                        Flag = TRUE;
                    }
                }
...
FUNC(Std_ReturnType,DCM_CODE)DsdInternal_SecurityCheck(uint8 ProtocolCtrlId,uint8 Sid)
{
    ...
    if(E_OK == ret)
    {
        ...
        SecRefNum = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                pDcmDsdService)[SidTabServiceCfgIndex].DcmDsdSecurityLevel_Num;
        if(SecRefNum != 0u)
        {
            pSecCfg = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                    pDcmDsdService)[SidTabServiceCfgIndex].pDcmDsdSecurityLevelRef;
...
FUNC(Std_ReturnType,DCM_CODE)DsdInternal_SubSecurityCheck(uint8 ProtocolCtrlId,uint8 Sid)
{
    ...
    ret = DsdInternal_SearchSidTabServiceIndex(Sid,ProtocolCtrlId,
            &SidTabCfgIndex,&SidTabServiceCfgIndex);
    if(E_OK == ret)
    {
        ret = DsdInternal_SearchSidTabSubServiceIndex(ProtocolCtrlId,
                SidTabCfgIndex,SidTabServiceCfgIndex,&SubServieCfgIndex);
        if (E_OK == ret)
        {
            /*get Currently valid security level*/
            ActiveSec = Dcm_MkCtrl.Dcm_ActiveSec;
            ...
            if (0u != SubServiceNum)
            {
                pSubService = ((Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabCfgIndex].
                        pDcmDsdService)[SidTabServiceCfgIndex].DcmDsdSubService;
                SubSecRefNum = pSubService[SubServieCfgIndex].DcmDsdSubServiceSecurityLevel_Num;
                if(0u != SubSecRefNum)
                {
                    for(SecCfgIndex=0; (SecCfgIndex < SubSecRefNum)
                        && (FALSE == Flag); SecCfgIndex++)
                    {
                        if(ActiveSec == pSubService[SubServieCfgIndex].
                                DcmDsdSubServiceSecurityLevelRef[SecCfgIndex])
                        {
                            Flag = TRUE;
                        }
                    }
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE)DsdInternal_SetNrc(uint8 ProtocolCtrlId,
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE)DsdInternal_SetMsgState(uint8 ProtocolCtrlId,
...
static FUNC(Std_ReturnType,DCM_CODE)DsdInternal_RxIndication_Check(uint8 ProtocolCtrlId)
{
    ...
    if ((E_OK == ret) && (DCM_PROTOCOL_VALID != ProtocolState))
    {
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(Std_ReturnType, DCM_CODE)DsdInternal_DidSecurityCheck(
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(Std_ReturnType, DCM_CODE)DsdInternal_DidSessionCheck(
        uint16  DidInfoCfgIndex,
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    uint8   Index;
    ...
    else
    {
        ...
        {
            for (Index = 0; (Index < (pDspDidRead->DcmDspDidReadSessionRefNum))
            && (FALSE == Flag); Index++)
            {
    ...
    if (FALSE == Flag)
    {
        ...
        if ((receiveDid < 0xF200u) || (receiveDid > 0xF3FFu))
        {
            ...
            for (Idx = 0; (Idx < Dcm_DspCfg.DcmDspDidRangeNum) && (FALSE == Flagx); Idx++)
            {
                ...
                if ((receiveDid >= Dcm_DspCfg.pDcmDspDidRange[Idx].
                        DcmDspDidRangeIdentifierLowerLimit)
                    && (receiveDid <= Dcm_DspCfg.pDcmDspDidRange[Idx].
                            DcmDspDidRangeIdentifierUpperLimit))
                {
                    if (TRUE == Dcm_DspCfg.pDcmDspDidRange[Idx].DcmDspDidRangeHasGaps)
                    {
                        ...
                        {
                            ret = Dcm_DspCfg.pDcmDspDidRange[Idx].
                             DcmDspDidRangeIsDidAvailableFnc(receiveDid,DCM_INITIAL,&DidSupported);
...
static FUNC(Std_ReturnType, DCM_CODE)DsdInternal_DDDIDcheckMemory(void)
{
    ...
    for(IdInfoIndex = 0u; (E_OK == ret) && (IdInfoIndex <
    Dcm_DspCfg.pDcmDspMemory->DcmDspMemoryIdInfoNum); IdInfoIndex++)
    {
        pDcmDspMemoryIdInfo = &(Dcm_DspCfg.pDcmDspMemory->
                DcmDspMemoryIdInfo[IdInfoIndex]);
        if (pDcmDspMemoryIdInfo != NULL_PTR)
        {
            for(RangeInfoIndex = 0; (RangeInfoIndex < pDcmDspMemoryIdInfo->
            DcmDspReadMemoryRangeInfoNum) && (E_OK == ret); RangeInfoIndex++)
            {
                pDcmDspReadMemoryRangeInfo = &(pDcmDspMemoryIdInfo->
                        DcmDspReadMemoryRangeInfo[RangeInfoIndex]);
                Num = pDcmDspReadMemoryRangeInfo->
                        DcmDspReadMemorySecurityLevelRefNum;
                if (Num != 0u)
                {
                    Find = FALSE;
                ...
                {
                    Num = pDcmDspReadMemoryRangeInfo->
                            DcmDspReadMemoryRangeSessionLevelRefNum;
                    if (Num != 0u)
                    {
                        Find = FALSE;
                        for (index = 0; (index < Num) && (Find == FALSE); index++)
                        {
                            if (Dcm_MkCtrl.Dcm_ActiveSes ==
                                    pDcmDspReadMemoryRangeInfo->
                                    pDcmDspReadMemoryRangeSessionLevelRow[index])
                            {
                                Find = TRUE;
                            }
                        }
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(Std_ReturnType, DCM_CODE)DsdInternal_DDDIDcheckPerSourceDID(
        uint16 DDDidIdx,
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    Std_ReturnType  ret = E_OK;
            ...
            (TRUE == *Dcm_DspCfg.DcmDspDDDIDcheckPerSourceDID))
    {
        ...
        for (iloop = 0; (iloop < DDDidNum) && (E_OK == ret); iloop++)
        {
            if (DDDid->Subfunction == DCM_UDS0X2C_01_DDBYDID)
            {
                ...
                ret = DsdInternal_DidCheck(Did,
                        &RangeDidCfgIndex,&DidCfgIndex,&RangeDidFlag);
                if (E_OK == ret)
                {
                ...
                const uint8 addressAndLengthFormatIdentifier)
{
    ...
    for (iloop = 0; (iloop < Dcm_DspCfg.pDcmDspMemory->
            DcmDspAddressAndLengthFormatIdentifierNum) && (ret == E_NOT_OK); iloop++)
    {

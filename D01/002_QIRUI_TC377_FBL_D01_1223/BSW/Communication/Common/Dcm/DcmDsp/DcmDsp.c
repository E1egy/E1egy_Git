...
static FUNC(void,DCM_CODE)DspInternal_DcmConfirmForRePrg(uint8 ProtocolCtrlId)
{
    ...
    {
        if ((Dcm_SesCtrl.Dcm_NewSes == Dcm_DspCfg.pDcm_DspSession->
                     ...
                     DcmSendRespPendOnTransToBoot == TRUE))
        {
                            ...
                            DcmDspSessionForBoot == DCM_SYS_BOOT_RESPAPP))
            {
                ...
                Dcm_Channel[Offset + 2u] = (uint8)(Dcm_DspCfg.pDcm_DspSession->
                        pDcmDspSessionRow[iloop].DcmDspSessionP2ServerMax >>8u);
                Dcm_Channel[Offset + 3u] = (uint8)(Dcm_DspCfg.pDcm_DspSession->
                        pDcmDspSessionRow[iloop].DcmDspSessionP2ServerMax);
                Dcm_Channel[Offset + 4u] = (uint8)((Dcm_DspCfg.pDcm_DspSession->
                        pDcmDspSessionRow[iloop].DcmDspSessionP2StarServerMax) >>8u);
                Dcm_Channel[Offset + 5u] = (uint8)(Dcm_DspCfg.pDcm_DspSession->
                        pDcmDspSessionRow[iloop].DcmDspSessionP2StarServerMax);
                SchM_Exit_Dcm(Dcm_Channel);
                SchM_Enter_Dcm(Dcm_MsgCtrl);
                Dcm_MsgCtrl[MsgCtrlId].MsgContext.ResMaxDataLen = 6u;
                Dcm_MsgCtrl[MsgCtrlId].MsgContext.ResDataLen    = 6u;
                Dcm_MsgCtrl[MsgCtrlId].MsgContext.pResData = &Dcm_Channel[Offset];
                Dcm_MsgCtrl[Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex].RspStyle =
                        DCM_POS_RSP;
                Dcm_MsgCtrl[Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex].Dcm_MsgState =
                        DCM_MSG_PROCESSED;
                SchM_Enter_Dcm(Dcm_ChannelCtrl);
                Dcm_ChannelCtrl[TxChannelCtrlId].Dcm_BufferCunrentPosition = 0;
                SchM_Exit_Dcm(Dcm_ChannelCtrl);
                if (Dcm_MsgCtrl[MsgCtrlId].MsgContext.MsgAddInfo.SuppressPosResponse ==
                        TRUE)
                {
                    Dcm_MsgCtrl[Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex].RspStyle =
                            DCM_POS_RSP_SUPPRESS;
                }
...
FUNC(void,DCM_CODE)DspInternal_DcmConfirmation(uint8 ProtocolCtrlId)
{
    ...
    else
    {
            ...
            && (0x10u == Dcm_MsgCtrl[MsgCtrlId].SID))
        {
            for (iloop = 0;iloop < Dcm_DspCfg.pDcm_DspSession->DcmDspSessionRow_Num;iloop++)
            {
                if (Dcm_SesCtrl.Dcm_NewSes == Dcm_DspCfg.pDcm_DspSession->pDcmDspSessionRow[iloop].
                        DcmDspSessionLevel)
                {

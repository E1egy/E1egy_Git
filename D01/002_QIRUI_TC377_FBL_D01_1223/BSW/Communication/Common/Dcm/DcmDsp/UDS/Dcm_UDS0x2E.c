...
typedef struct
{
    ...
    P2VAR(uint8, AUTOMATIC, AUTOMATIC)DidInfoCfgIndex;
}Dcm_0x2ETypes;
/***************************/
/**********************************************************************/
/*
                    ...
                    if (TRUE == Dcm_DspCfg.pDcmDspDidRange[Idx].DcmDspDidRangeHasGaps)
                    {
                        if (Dcm_DspCfg.pDcmDspDidRange[Idx].DcmDspDidRangeIsDidAvailableFnc != NULL_PTR)
                        {
                            ret = Dcm_DspCfg.pDcmDspDidRange[Idx].DcmDspDidRangeIsDidAvailableFnc(
                                    RecDid,OpStatus,&DidSupported);
                            if ((ret == E_OK)
                                && (DCM_DID_SUPPORTED == DidSupported))
                            {
        ...
        uint8 DidInfoCfgIndex)
{
    ...
    else
    {
        ...
        {
            for(Index = 0; (Index < (pDspDidWrite->DcmDspDidWriteSessionRefNum)) &&
            (E_NOT_OK == ret);Index++)
            {
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    ...
    else
    {
        ...
        {
            for (Index = 0; (Index < (pDspDidWrite->DcmDspDidWriteSecurityLevelRefNum))
            && (E_NOT_OK == ret); Index++)
            {
        ...
        P2VAR(uint16, AUTOMATIC, AUTOMATIC)pDidSize)
{
    ...
    *pDidSize = 0;
    for (DidSignalIndex = 0; DidSignalIndex < Dcm_DspCfg.pDcmDspDid[DidCfgIndex].
    DcmDspDidSignalNum; DidSignalIndex++)
    {
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC) pNrc)
{
            ...
            (ret == E_OK)); DidSignalIndex++)
    {
        ...
        if ((pDspDidData->DcmDspDataWriteFnc == NULL_PTR) &&
                (USE_BLOCK_ID != pDspDidData->DcmDspDataUsePort))
        {
        ...
        else
        {
            ret = pDspDidData->DcmDspDataWriteFnc(
                        ...
                        ((pDspDidData->DcmDspDataSize + 7u) >> 3u),OpStatus,pNrc);
        }
        ...
        if ((USE_BLOCK_ID == pDspDidData->DcmDspDataUsePort) && (ret == E_OK))
        {
            ...
            else
            {
                ...
                if (E_OK == ret)
                {
                    if ((NVM_REQ_OK == NvmRet) || (NVM_REQ_BLOCK_SKIPPED == NvmRet))
                    {
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if(E_OK == ret)
    {
        ...
        {
            *Dcm_0x2EType->DidInfoCfgIndex = Dcm_DspCfg.pDcmDspDidRange[*Dcm_0x2EType->pRangeDidCfgIndex].DcmDspDidRangeInfoIndex;
        }
        else
        {
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if(E_OK == ret)
    {
        if (FALSE == RangeDidFlag)
        {
            ...
            if((E_OK == ret) &&
                ((3u + DidSize )!= (uint16)Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen))
            {
            ...
            if(E_OK == ret)
            {
                ret = DspInternalUDS0x2E_DidWrite(OpStatus,
                        MsgCtrlId,DidCfgIndex,ErrorCode);
            }

            if(DCM_E_FORCE_RCRRP == ret)
            {
                SchM_Enter_Dcm(Dcm_MsgCtrl);
        ...
        else
        {
            ...
            /*write data*/
           if ((E_OK == ret) && (NULL_PTR == Dcm_DspCfg.pDcmDspDidRange[RangeDidCfgIndex].
           ...
           if (E_OK == ret)
           {
               ret = Dcm_DspCfg.pDcmDspDidRange[RangeDidCfgIndex].DcmDspDidRangeWriteDidFnc(
                       RecDid,&(Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[3]),
                       OpStatus,DidSize,ErrorCode);
               if (DCM_E_PENDING == ret)
               {
                   SchM_Enter_Dcm(Dcm_MsgCtrl);
                   Dcm_MsgCtrl[MsgCtrlId].Dcm_OpStatus = DCM_PENDING;

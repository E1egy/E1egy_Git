...
#define DCM_UDS86_DO_NOT_STORE_EVENT        0x00u
#define DCM_UDS86_STORE_EVENT               0x01u
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(void, DCM_CODE)Dcm_Uds0X86_CheckEventWindowTime(
        uint8 RoeEventId,boolean SubRequest)
{
    boolean end = FALSE;
    ...
    case (uint8)DCM_ROE_EVENT_WINDOW_CURRENT_AND_FOLLOWING_CYCLE:
        ...
        {
            Dcm_RoeRuntime.Dcm_RoeStates[RoeEventId].EventWindowTime =
                    (uint8)DCM_ROE_EVENT_WINDOW_CURRENT_CYCLE;
        }
        break;
    ...
    case (uint8)DCM_ROE_EVENT_WINDOW_INFINITE:
        if ((DCM_DEFAULT_SESSION == Dcm_RoeRuntime.Dcm_RoeStates[RoeEventId].ROEActiveSes)
                && (DCM_UDS86_DO_NOT_STORE_EVENT ==
                        Dcm_RoeRuntime.Dcm_RoeStates[RoeEventId].Dcm_RoeStoreEvent))
        {
            end = TRUE;
        }
        break;
    ...
    if ((end == TRUE) &&
            (Dcm_RoeRuntime.Dcm_RoeStates[RoeEventId].Dcm_RoeEventStates == DCM_ROE_STARTED))
    {
...
FUNC(void, DCM_CODE)Dcm_UDS0x86_Init(void)
{
    ...
    for (iloop = 0; iloop < DCM_DSP_ROE_NUM; iloop++)
    {
        ...
        if (DCM_ROE_STOPPED == Dcm_RoeRuntime.Dcm_RoeStates[iloop].Dcm_RoeEventStates)
        {
            ...
            Dcm_RoeRuntime.Dcm_RoeStates[iloop].EventWindowTime =
                    (uint8)DCM_ROE_EVENT_WINDOW_INFINITE;
            Dcm_RoeRuntime.Dcm_RoeStates[iloop].ROEActiveSes = DCM_ALL_SESSION_LEVEL;
            Dcm_RoeRuntime.Dcm_RoeStates[iloop].RoeCnt = 0;
            if (NULL_PTR != Dcm_DspCfg.Dcm_DspRoe->DcmDspRoeEvent[iloop].
                    DcmDspRoeEventProperties->DcmDspRoeOnDTCStatusChange)
            {
                Dcm_RoeRuntime.Dcm_RoeStates[iloop].DTCStatusMask = Dcm_DspCfg.Dcm_DspRoe->
                        DcmDspRoeEvent[iloop].DcmDspRoeEventProperties->
                        DcmDspRoeOnDTCStatusChange->DcmDspRoeDTCStatusMask;
            }
            if (NULL_PTR != Dcm_DspCfg.Dcm_DspRoe->DcmDspRoeEvent[iloop].
                    DcmDspRoeEventProperties->DcmDspRoeOnChangeOfDataIdentifier)
            {
                Dcm_RoeRuntime.Dcm_RoeStates[iloop].DcmDspRoeDidRef = Dcm_DspCfg.Dcm_DspRoe->
                        DcmDspRoeEvent[iloop].DcmDspRoeEventProperties->
                        DcmDspRoeOnChangeOfDataIdentifier->DcmDspRoeDidRef;
            }
        }
    }
#if(NVM_ENABLE == STD_ON)
    if (DCM_ROE_STORAGE_BLOCKID != 0xFFu)
    {
        (void)NvM_GetErrorStatus(DCM_ROE_STORAGE_BLOCKID,&NvmRet);
        if (NVM_REQ_OK == NvmRet)
        {
            Dcm_MemoryCopy(NvM_BlockDescriptor[DCM_ROE_STORAGE_BLOCKID - 1u].NvmRamBlockDataAddress,
                    (uint8*)&(pRoeRuntime),sizeof(Dcm_RoeRuntimeTypes));
            for (iloop = 0; iloop < DCM_DSP_ROE_NUM; iloop++)
            {
                if (DCM_UDS86_STORE_EVENT == pRoeRuntime.Dcm_RoeStates[iloop].Dcm_RoeStoreEvent)
                {
                    Dcm_MemoryCopy((uint8*)&(pRoeRuntime.Dcm_RoeStates[iloop]),
                            (uint8*)&(Dcm_RoeRuntime.Dcm_RoeStates[iloop]),sizeof(Dcm_RoeStatesTypes));
                }
            }
        }
    }
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(boolean, DCM_CODE)Dcm_UDS0x86_GetTxPduId(PduIdType* pduTxId)
{
    boolean ret = TRUE;
    uint8 iloop;
    ...
    if (TRUE == Dcm_DspRoeNeedSend.IsRoeMessage)
    {
        for (iloop = 0; iloop < DCM_CONNECTION_NUM; iloop++)
        {
            ...
            {
                ret = FALSE;
                *pduTxId = Dsl_Protocol_ConnectionCfg[iloop].pDcmDslMainConnection->
                        pDcmDslROEConnectionRef->DcmDslTxPduRPduId;
                break;
            }
        }
    }
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(boolean, DCM_CODE)Dcm_UDS0x86_TxConfirmation(uint8  ProtocolCtrlId)
{
    boolean ret = TRUE;
    if ((ProtocolCtrlId == Dcm_RoeRuntime.ProtocolCtrlId) &&
            (TRUE == Dcm_DspRoeNeedSend.IsRoeMessage))
    {
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void, DCM_CODE)Dcm_UDS0x86_CheckTimer(void)
{
    ...
    else
    {
        ...
        if ((Dcm_DspRoeNeedSend.Dcm_DspRoeNeedSend == TRUE) && (Find == FALSE))
        {
            ...
            for (iloop = 0; iloop < DCM_CONNECTION_NUM; iloop++)
            {
                if (Dcm_RoeRuntime.ProtocolCtrlId  ==
                        pDsl_Protocol_ConnectionCfg[iloop].DcmDslParentProtocolRowCtrlId)
                {
                    for (index = 0; index < pDsl_Protocol_ConnectionCfg[iloop].
                    pDcmDslMainConnection->DcmDslProtocolRx_Num; index++)
                    {
                        if (Dcm_DspRoeNeedSend.pduRxId == pDsl_Protocol_ConnectionCfg[iloop].
            ...
            if ((pDsl_Protocol_ConnectionCfg[iloop].pDcmDslMainConnection->pDcmDslProtocolRx->DcmDslMetaDataFlag & DCM_METADATA_LENGTH_MASK) > 0u)
            {
...
#endif
            DslInternal_ResponseOnOneEvent(PduInfo
                    ,Dcm_DspRoeNeedSend.pduRxId);
        }
    }
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE) Dcm_UDS0x86_SesTranslation(Dcm_SesCtrlType NewSes)
{
    uint8 iloop;
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    for (iloop = 0; (iloop < Dcm_DspCfg.Dcm_DspRoe->
            DcmDspRoeEventWindowTimeNum) && (ret == E_NOT_OK); iloop++)
    {
        if (EventWindowTime == (uint8)Dcm_DspCfg.Dcm_DspRoe->
                DcmDspRoeEventWindowTime[iloop].DcmDspRoeEventWindowTime)
        {
            ret = E_OK;
        }
    }
    ...
    {
        if ((EventWindowTime != (uint8)DCM_ROE_EVENT_WINDOW_INFINITE) &&
    ...
    for (iloop = 0; iloop < DCM_DSP_ROE_NUM; iloop++)
    {
        if (DCM_ROE_STARTED == Dcm_RoeRuntime.Dcm_RoeStates[iloop].Dcm_RoeEventStates)
        {
                    ...
                    DcmDspRoeEventProperties->DcmDspRoeOnChangeOfDataIdentifier)
            {
                ...
                ResOffest++;
                Dcm_Channel[ResOffest] = (uint8)(Dcm_RoeRuntime.
                        Dcm_RoeStates[iloop].DcmDspRoeDidRef >> 8u);
                ResOffest++;
                Dcm_Channel[ResOffest] = (uint8)Dcm_RoeRuntime.
                        Dcm_RoeStates[iloop].DcmDspRoeDidRef;
                ResOffest++;
            ...
            else if (NULL_PTR != Dcm_DspCfg.Dcm_DspRoe->DcmDspRoeEvent[iloop].
                    DcmDspRoeEventProperties->DcmDspRoeOnDTCStatusChange)
            {
                ResOffest++;
...
static FUNC(boolean, DCM_CODE)Dcm_UDS0x86_CheckFullCom(void)
{
    ...
    {
        if (Dsl_Protocol_ConnectionCfg[Index].DcmDslParentProtocolRowCtrlId ==
                Dcm_RoeRuntime.ProtocolCtrlId)
        {
...
FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x86_ChangeOfDataIdentifier (uint8 RoeEventId)
{
    ...
    if ((ProcressOn == TRUE) && (DCM_ROE_STARTED == Dcm_RoeRuntime.
            Dcm_RoeStates[ROEIndex].Dcm_RoeEventStates)
       && (NULL_PTR != Dcm_DspCfg.Dcm_DspRoe->DcmDspRoeEvent[ROEIndex].
                    DcmDspRoeEventProperties->DcmDspRoeOnChangeOfDataIdentifier))
    {
        /*check all protocol whether have one protocol had already started*/
        for (iloop = 0; iloop < DCM_DSLPROTOCOLROW_NUM_MAX; iloop++)
        ...
        for (iloop = 0; iloop < DCM_CONNECTION_NUM; iloop++)
        {
                    ...
                    pDsl_Protocol_ConnectionCfg[iloop].DcmDslParentProtocolRowCtrlId)
            {
                ...
                pDcmDslMainConnection->DcmDslProtocolRx_Num; index++)
                {
                    if (DCM_FUNCTIONAL != pDsl_Protocol_ConnectionCfg[iloop].
...
FUNC(void, DCM_CODE)Dcm_UDS0x86_DTCStatusChange(uint8 DTCStatusOld,uint8 DTCStatusNew)
{
    ...
    for (Cnt = 0; (Cnt < DCM_DSP_ROE_NUM) && (ProcressOn == TRUE); Cnt++)
    {
        if ((DCM_ROE_STARTED == Dcm_RoeRuntime.Dcm_RoeStates[Cnt].Dcm_RoeEventStates)
           && (NULL_PTR != Dcm_DspCfg.Dcm_DspRoe->DcmDspRoeEvent[Cnt].
                        DcmDspRoeEventProperties->DcmDspRoeOnDTCStatusChange)
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if((E_OK == ret) &&
        ((Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen) < DCM_UDS0X86_REQ_DATA_MINLENGTH))
    {
    ...
    if(E_OK == ret)
    {
        ...
        ret = DsdInternal_SearchSidTabServiceIndex(SID_RESPONSE_ON_EVENT,ProtocolCtrlId,
                &SidTabCfgIndex,&SidTabServiceCfgIndex);
        if(E_OK == ret)
        {
            ret = DsdInternal_SearchSidTabSubServiceIndex(ProtocolCtrlId,SidTabCfgIndex,
                    SidTabServiceCfgIndex,&SubServieCfgIndex);
        }
        if (E_NOT_OK == ret)
        {
    ...
    if ((E_OK == ret) && ((SubFunction == DCM_UDS0X86_ON_DTC_STATUS_CHANGE) ||
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x86(
    ...
    if(E_OK == ret)
    {
        ...
        else
        {
            ...
            case DCM_UDS0X86_ON_DTC_STATUS_CHANGE:
                if (6u == Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)
                {
                    ret = Dcm_Uds0X86SubDeal_0x01(
                            ProtocolCtrlId,StoreEvent,EventWindowTime,&ResLength,ErrorCode);
                }
                else
                {
                    /*the length of massage is not correct,send NRC 0x13*/
            ...
            case DCM_UDS0X86_ON_CHANGE_OF_DATA_IDENTIFIER:
                if (8u == Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)
                {
                    ret = Dcm_Uds0X86SubDeal_0x03(
                            ProtocolCtrlId,StoreEvent,EventWindowTime,&ResLength,ErrorCode);
                }
                else
                {
            ...
            case DCM_UDS0X86_CLEAR_RESPONSE_ON_EVENT:
                ret = Dcm_Uds0X86SubDeal_0x06(ProtocolCtrlId,StoreEvent,
                        Offset,&ResLength);
                break;
#endif
            default:

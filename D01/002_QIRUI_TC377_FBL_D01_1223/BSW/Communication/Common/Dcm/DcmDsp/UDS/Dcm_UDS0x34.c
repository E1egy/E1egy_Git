        ...
        uint8 MemoryRangeInfoIndex)
{
    ...
    pDcmDspWriteMemoryRangeInfo = &(Dcm_DspCfg.pDcmDspMemory->
       DcmDspMemoryIdInfo[MemoryIdInfoIndex].DcmDspWriteMemoryRangeInfo[MemoryRangeInfoIndex]);
    ...
    if (pDcmDspWriteMemoryRangeInfo->DcmDspWriteMemoryRangeSessionLevelRefNum != 0u)
    {
        for (index = 0; index < pDcmDspWriteMemoryRangeInfo->DcmDspWriteMemoryRangeSessionLevelRefNum; index++)
        {
            if (Dcm_MkCtrl.Dcm_ActiveSes ==
                    pDcmDspWriteMemoryRangeInfo->pDcmDspWriteMemoryRangeSessionLevelRow[index])
            {
                ret = E_OK;
            }
        }
        ...
        uint8 MemoryRangeInfoIndex)
{
    ...
    {
        for (index = 0; (index < pDcmDspWriteMemoryRangeInfo->
                DcmDspWriteMemorySecurityLevelRefNum) && (E_NOT_OK == ret); index++)
        {
            if (Dcm_MkCtrl.Dcm_ActiveSec == pDcmDspWriteMemoryRangeInfo->
        ...
        P2VAR(uint8, AUTOMATIC, AUTOMATIC)MemoryAddressMatchNum)
{
            ...
            IdInfoIndex++)
    {
        ...
        if (pDcmDspMemoryIdInfo != NULL_PTR)
        {
            ...
            DcmDspWriteMemoryRangeInfoNum; RangeInfoIndex++)
            {
                pDcmDspWriteMemoryRangeInfo = &(pDcmDspMemoryIdInfo->
                        DcmDspWriteMemoryRangeInfo[RangeInfoIndex]);
                if (pDcmDspWriteMemoryRangeInfo != NULL_PTR)
                {
                    if ((pDcmDspWriteMemoryRangeInfo->DcmDspWriteMemoryRangeLow <=
                            Dcm_0x34Type->MemoryAddress)
...
#endif
    /*min-length check*/
    if ((E_OK == ret) && (DCM_UDS0X34_REQ_DATA_MINLENGTH >
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if (E_OK == ret)
    {
        ...
        ret = Dcm_0x34MemoryRangeCheck(&Dcm_0x34Type,&MemoryIdInfoIndex,
                       &MemoryRangeInfoIndex,&MemoryAddressMatchNum);
    ...
    if (E_OK == ret)
    {
        ...
        ret = Dcm_ProcessRequestDownload(OpStatus,DataFormatIdentifier,
        ...
        if (BlockLength > Dcm_DslCfg.
             pDcmChannelCfg[Dcm_ChannelCtrl[Dcm_MsgCtrl[MsgCtrlId].Dcm_RxCtrlChannelIndex].
                            Dcm_ChannelCfgIndex].Dcm_DslBufferSize)
        {
    ...
    if (E_OK == ret)
    {
        ...
        Dcm_UDS_UDS0x31_37Flag = Unperform;
    }

        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if ((E_OK == ret) && (Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen < DCM_UDS0X85_REQ_DATA_MINLENGTH))
    {
    ...
    if((E_OK == ret) && ((DCM_UDS0X85_ON  != Dcm_MsgCtrl[MsgCtrlId].Subfunction) &&
    ...
    if(E_OK == ret)
    {
        if ((Dcm_DspCfg.pDcmDspControlDTCSetting != NULL_PTR) && (TRUE ==
              Dcm_DspCfg.pDcmDspControlDTCSetting->DcmSupportDTCSettingControlOptionRecord))
        {
            if ((0u != ((Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen -
                  DCM_UDS0X85_REQ_DATA_MINLENGTH) % 3u)) || (Dcm_MsgCtrl[MsgCtrlId].
                          MsgContext.ReqDataLen < DCM_UDS0X85_REQ_DATA_DTC_MINLENGTH))
            {
                /*the length of massage is not correct,send NRC 0x13*/
                *ErrorCode = DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT;
                ret = E_NOT_OK;
    ...
    if (E_OK == ret)
    {
        ...
        if (DCM_UDS0X85_REQ_DATA_MINLENGTH == Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)
        {
            ...
            if (DCM_UDS0X85_ON == subFunc)
            {
                ...
                returnCtrlDtcSetting = Dem_DcmEnableDTCSetting(
                        DEM_DTC_GROUP_ALL_DTCS,DEM_DTC_KIND_ALL_DTCS);
            ...
            if (DCM_UDS0X85_OFF == subFunc)
            {
                ...
                /*invoke the corresponding API provided by DEM*/
                returnCtrlDtcSetting = Dem_DcmDisableDTCSetting(
                        DEM_DTC_GROUP_ALL_DTCS,DEM_DTC_KIND_ALL_DTCS);
#else
                returnCtrlDtcSetting = DEM_CONTROL_DTC_SETTING_OK;
#endif
            }
        }
        else if(0u == ((Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen -
                DCM_UDS0X85_REQ_DATA_MINLENGTH) % 3u))
        {
            ...
            if (DCM_UDS0X85_ON == subFunc)
            {
                ...
                for (index = 0;(index < numOfDTC) &&
                (DEM_CONTROL_DTC_SETTING_OK == returnCtrlDtcSetting); index++)
                {
                    groupOfDTC = (((uint32)(Dcm_MsgCtrl[MsgCtrlId].MsgContext.
                      pReqData[2u + (3u * index)])) << 16u) | (((uint32)(Dcm_MsgCtrl[MsgCtrlId].
                      MsgContext.pReqData[3u + (3u * index)])) << 8u) |
                      ((uint32)(Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[4u + (3u * index)]));
                    /*invoke the corresponding API provided by DEM*/
                    returnCtrlDtcSetting = Dem_DcmEnableDTCSetting(
                            groupOfDTC,DEM_DTC_KIND_ALL_DTCS);
                }
            }
            /*get and assemble DTC, high byte first*/

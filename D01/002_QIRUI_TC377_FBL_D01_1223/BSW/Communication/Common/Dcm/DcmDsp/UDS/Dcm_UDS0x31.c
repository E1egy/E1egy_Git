...
typedef struct
{
    uint8 RoutineCfgId;
    boolean Find;
    P2VAR(uint8,AUTOMATIC,DCM_VAR) ResLen;
}Dcm_0x31Types;
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    {
        pDspStartRoutineAuthorization = Dcm_DspCfg.pDcmDspRoutine[RoutineCfgIdx].
                DcmDspStartRoutine->DcmDspStartRoutineCommonAuthorizationRef;
    ...
    {
        pDspStopRoutineAuthorization = Dcm_DspCfg.pDcmDspRoutine[RoutineCfgIdx].
                DcmDspStopRoutine->DcmDspStopRoutineCommonAuthorizationRef;
    ...
    {
        pDspRequestRoutineResultsAuthorization = Dcm_DspCfg.pDcmDspRoutine[RoutineCfgIdx].
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)ErrorCode)
{
    ...
    if (E_OK == *Ret)
    {
        ...
        {
            for (Index = 0; Index < StartRoutine->DcmDspStartRoutineIn->
            RoutineInOutSignalNum; Index++)
            {
                ReqLen = ReqLen + ((uint32)((StartRoutine->DcmDspStartRoutineIn->
        ...
        {
            for (Index = 0; Index < StartRoutine->DcmDspStartRoutineOut->
            RoutineInOutSignalNum; Index++)
            {
                ResLen = ResLen + ((uint32)((StartRoutine->DcmDspStartRoutineOut->
                        DcmDspRoutineInOutSignal[Index].DcmDspRoutineSignalLength + 7u)>> 3u));
            }
        }
        else
        ...
        if ((ReqLen + DCM_UDS0X31_REQ_DATA_MINLENGTH) !=
                ((uint32)Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen))
        {
    ...
    //if ((E_OK == *Ret) && (DCM_DSP_ROUTINE_ON == Dcm_RoutineControlState[RoutineCfgId]))
    //{
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC) ErrorCode)
{
    ...
    if (E_OK == *Ret)
    {
        ...
        {
            for (Index = 0; Index < StopRoutine->DcmDspStopRoutineIn->
            RoutineInOutSignalNum; Index++)
            {
        ...
        {
            for (Index = 0; Index < StopRoutine->DcmDspStopRoutineOut->
            RoutineInOutSignalNum; Index++)
            {
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC) ErrorCode)
{
    ...
    if (E_OK == *Ret)
    {
        ...
        {
            for (Index = 0; Index < RequestRoutineResults->DcmDspRequestRoutineResultsIn->
            RoutineInOutSignalNum; Index++)
            {
                ReqLen = ReqLen + ((uint8)((RequestRoutineResults->DcmDspRequestRoutineResultsIn->
                    DcmDspRoutineInOutSignal[Index].DcmDspRoutineSignalLength + 7u) >> 3u));
            }
        }
        else
        ...
        {
            for (Index = 0; Index < RequestRoutineResults->DcmDspRequestRoutineResultsOut->
            RoutineInOutSignalNum; Index++)
            {
                ResLen = ResLen + ((uint8)((RequestRoutineResults->
                     DcmDspRequestRoutineResultsOut->DcmDspRoutineInOutSignal[Index].
    ...
    if ((E_OK == *Ret) && (DCM_DSP_ROUTINE_INIT == Dcm_RoutineControlState[RoutineCfgId]))
    {
        /*the request sequence error,send NRC 0x24*/
        *ErrorCode  = DCM_E_REQUESTSEQUENCEERROR;
        *Ret = E_NOT_OK;
    ...
    if (E_OK == *Ret)
    {
        ...
        *Ret = (*RequestResultsFnc)(pInbuffer,pOutBuffer, ErrorCode);
    }
    return ResLen;
}
#define DCM_STOP_SEC_CODE
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(uint8, DCM_CODE)Dcm_UdsOBDStartRoutine(
        uint8  MsgCtrlId,
        uint8  RoutineCfgId,
        P2VAR(Std_ReturnType, AUTOMATIC, AUTOMATIC) Ret,
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)ErrorCode)
{
    ...
    if (E_OK == *Ret)
    {
        if (NULL_PTR != StartRoutine->DcmDspStartRoutineIn)
        {
            ...
            RoutineInOutSignalNum; Index++)
            {
                ReqLen = ReqLen + ((uint8)((StartRoutine->DcmDspStartRoutineIn->
                    DcmDspRoutineInOutSignal[Index].DcmDspRoutineSignalLength + 7u) >> 3u));
            }
        }
        else
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    case 0xE0E0u:
        if ((Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen % 2u) == 0u)
        {
            ...
            {
                routineId = (uint16)(((uint16)((uint16)Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[2u + (2u * index)]) << 8u)
                ...
                case 0xE0E0u:
                    ...
                    else
                    {
                        for (iloop = 0; iloop < Dcm_DspCfg.DcmDspRoutineNum; iloop++)
                        {
                            ...
                            {
                                SupportBuffer |= (uint32)1UL << (0x20u -
                                        (Dcm_DspCfg.pDcmDspRoutine[iloop].DcmDspRoutineId - routineId));
                            }
    ...
    default:
        if ((Dcm_DspCfg.DcmDspEnableObdMirror == TRUE) && (FALSE == Dcm_0x31Type.Find))
        {
            if(E_OK == OBD_FindCfgIndexOfTestID((uint8)(routineId & 0xFFu),&CtrlIndex))
            {
                ...
                if (Dcm_DspCfg.pDcmDspRequestControl[CtrlIndex].
                        DcmDspRequestControlInBufferSize == (Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen - 4u))
                {
                    if (E_OK == Rte_RequestControl((uint8)(routineId & 0xFFu),
                            &(Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[4]),
                            &(Dcm_Channel[Offset + 4u])))
                    {
                        *Dcm_0x31Type.ResLen = Dcm_DspCfg.pDcmDspRequestControl[CtrlIndex].
        ...
        else
        {
            ...
            {
                (*Dcm_0x31Type.ResLen) = Dcm_UdsOBDStartRoutine(MsgCtrlId,Dcm_0x31Type.RoutineCfgId,&Ret,ErrorCode);
                if (Ret == E_NOT_OK)
                {
                    *ErrorCode = DCM_E_CONDITIONSNOTCORRECT;
                }
...
static FUNC(Std_ReturnType, DCM_CODE)Dcm_RCConditonCheck(
        ...
        P2VAR(uint8,AUTOMATIC,DCM_VAR) ResLen,
#endif
        P2VAR(uint8,AUTOMATIC,DCM_VAR) RoutineCfgId,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    Std_ReturnType  Ret = E_OK;
    uint8 MsgCtrlId = Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex;
...
#endif
    if ((E_OK == Ret) &&
         (Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen < DCM_UDS0X31_REQ_DATA_MINLENGTH))
    {
    ...
    if (E_OK == Ret)
    {
        ...
        {
            Dcm_0x31Type.RoutineCfgId = *RoutineCfgId;
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
            ...
            if (E_OK == Ret)
            {
                switch (SubFunction)
                {
                    case  DCM_UDS0X31_STARTROUTINE:
                        ResLen = Dcm_UdsRCStartRoutine(MsgCtrlId,RoutineCfgId,&Ret,ErrorCode);
                        break;
                    case  DCM_UDS0X31_STOPROUTINE:
                        ResLen = Dcm_UdsRCStopRoutine(MsgCtrlId,RoutineCfgId,&Ret,ErrorCode);
                        break;
                    case  DCM_UDS0X31_REQUESTROUTINERESULTS:
                        ResLen = Dcm_UdsRCReqRoutineResult(MsgCtrlId,RoutineCfgId,&Ret,ErrorCode);
                        break;
                    default:
                        *ErrorCode  = DCM_E_SUBFUNCTIONNOTSUPPORTED;
                        Ret = E_NOT_OK;
    ...
    if((E_OK == Ret) && ((uint16)((uint16)ResLen + 4u) >

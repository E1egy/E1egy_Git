...
static FUNC(Dcm_CommunicationModeType, DCM_CODE)DspInternalUDS0x28GetReqMode_0x01(
        uint8  subFunc)
{
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Dcm_CommunicationModeType, DCM_CODE)DspInternalUDS0x28GetReqMode_0x02(
        uint8  subFunc)
{
    Dcm_CommunicationModeType RequestedMode = DCM_ENABLE_RX_TX_NORM;
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Dcm_CommunicationModeType, DCM_CODE)DspInternalUDS0x28GetReqMode_0x03(
        uint8  subFunc)
{
    Dcm_CommunicationModeType RequestedMode = DCM_ENABLE_RX_TX_NORM;
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Dcm_CommunicationModeType, DCM_CODE)DspInternalUDS0x28_0x00_0x03_GetReqMode(
        uint8  subFunc,
        uint8 comType)
{
    Dcm_CommunicationModeType RequestedMode = DCM_ENABLE_RX_TX_NORM;
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(void, DCM_CODE)DspInternalUDS0x28_0x00_0x03_SubFuction(
        ...
        uint8 comType)
{
            ...
            && (NULL_PTR != Dcm_Cfg.pDcmDspCfg->pDcmDspComControl))
    {
        ...
        {
            for (iloop = 0; iloop < Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
            DcmDspComControlAllChannelNum; iloop++)
            {
                if (TRUE == Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                        DcmDspComControlAllChannel[iloop].DcmDspComControlAllChannelUsed)
                {
#if(DCM_BSWM_ENABLE == STD_ON)
                    BswM_Dcm_CommunicationMode_CurrentState(Dcm_Cfg.pDcmDspCfg->
                            pDcmDspComControl->DcmDspComControlAllChannel[iloop].DcmDspComMChannelId,
                            RequestedMode);
#else
                    Rte_DcmControlCommunicationMode(Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                            DcmDspComControlAllChannel[iloop].DcmDspComMChannelId,RequestedMode);
#endif
                }
            }
        }
        ...
        {
#if(DCM_BSWM_ENABLE == STD_ON)
            BswM_Dcm_CommunicationMode_CurrentState(Dcm_Cfg.pDcmDslCfg->pDcmDslProtocol->
                    pDcmDslProtocolRow[ProtocolCtrlId].pDcmDslConnection->
                    pDcmDslMainConnection->DcmDslProtocolComMChannelId,RequestedMode);
#else
                    Rte_DcmControlCommunicationMode(Dcm_Cfg.pDcmDslCfg->pDcmDslProtocol->
                            pDcmDslProtocolRow[ProtocolCtrlId].pDcmDslConnection->
                            pDcmDslMainConnection->DcmDslProtocolComMChannelId,RequestedMode);
#endif
        }
        ...
        {
            for (iloop = 0; iloop < Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
            DcmDspComControlSpecificChannelNum; iloop++)
            {
                if (TRUE == Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                 DcmDspComControlSpecificChannel[iloop].DcmDspComControlSpecificChannelUsed)
                {
#if(DCM_BSWM_ENABLE == STD_ON)
                    BswM_Dcm_CommunicationMode_CurrentState(Dcm_Cfg.pDcmDspCfg->
                            pDcmDspComControl->DcmDspComControlSpecificChannel[iloop].
                            DcmDspSpecificComMChannelId,RequestedMode);
#else
                    Rte_DcmControlCommunicationMode(Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                         DcmDspComControlSpecificChannel[iloop].DcmDspSpecificComMChannelId,
                         RequestedMode);
#endif
                }
            }
        ...
        {
            for (iloop = 0; iloop < Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
            DcmDspComControlSubNodeNum; iloop++)
            {
                if (TRUE == Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                        DcmDspComControlSubNode[iloop].DcmDspComControlSubNodeUsed)
                {
#if(DCM_BSWM_ENABLE == STD_ON)
                    BswM_Dcm_CommunicationMode_CurrentState(Dcm_Cfg.pDcmDspCfg->
                       pDcmDspComControl->DcmDspComControlSubNode[iloop].
                       DcmDspComMSubNodeChannelId,RequestedMode);
#else
                    Rte_DcmControlCommunicationMode(Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                       DcmDspComControlSubNode[iloop].DcmDspComMSubNodeChannelId,RequestedMode);
#endif
                }
            }
        }
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Dcm_CommunicationModeType, DCM_CODE)DspInternalUDS0x28_0x04_0x05_GetReqMode(
        uint8  subFunc,
        uint8 comType)
{
    Dcm_CommunicationModeType RequestedMode = DCM_ENABLE_RX_TX_NORM;
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)DspInternalUDS0x28_0x04_0x05_SubFuction(
        uint16 Subnet,
        uint8  ProtocolCtrlId,
        uint8  subFunc,
        uint8 comType)
{
    ...
    if((Subnet == 0x00u)
        && ((NULL_PTR != Dcm_Cfg.pDcmDspCfg) && (NULL_PTR != Dcm_Cfg.pDcmDspCfg->
             pDcmDspComControl) && (NULL_PTR != Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
...
#if(DCM_BSWM_ENABLE == STD_ON)
                BswM_Dcm_CommunicationMode_CurrentState(Dcm_Cfg.pDcmDspCfg->
                    pDcmDspComControl->DcmDspComControlAllChannel[iloop].
                    DcmDspComMChannelId,RequestedMode);
#else
                Rte_DcmControlCommunicationMode(Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                     DcmDspComControlAllChannel[iloop].DcmDspComMChannelId,RequestedMode);
#endif
                    ...
                    pDcmDslConnection->pDcmDslMainConnection)))
    {
        ...
        Rte_DcmControlCommunicationMode(Dcm_Cfg.pDcmDslCfg->pDcmDslProtocol->
          pDcmDslProtocolRow[ProtocolCtrlId].pDcmDslConnection->pDcmDslMainConnection->
          DcmDslProtocolComMChannelId,RequestedMode);
#endif
    }
    if((Subnet > 0x00u) && (Subnet < 0x0Fu))
    {
        if ((NULL_PTR != Dcm_Cfg.pDcmDspCfg) && (NULL_PTR != Dcm_Cfg.pDcmDspCfg->
              pDcmDspComControl) && (NULL_PTR != Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                      DcmDspComControlSpecificChannel))
        {
            for (iloop = 0; (iloop < Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                    DcmDspComControlSpecificChannelNum) && (Find == FALSE); iloop++)
            {
                if ((TRUE == Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                      DcmDspComControlSpecificChannel[iloop].DcmDspComControlSpecificChannelUsed)
                        && (Subnet == Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                                DcmDspComControlSpecificChannel[iloop].DcmDspSubnetNumber))
                {
#if(DCM_BSWM_ENABLE == STD_ON)
                    BswM_Dcm_CommunicationMode_CurrentState(Dcm_Cfg.pDcmDspCfg->
                         pDcmDspComControl->DcmDspComControlSpecificChannel[iloop].
...
#else
                    Rte_DcmControlCommunicationMode(Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                     DcmDspComControlSpecificChannel[iloop].DcmDspSpecificComMChannelId,RequestedMode);
#endif
                    Find = TRUE;
                }
            }
    ...
    else
    {
        ...
        {
            for (iloop = 0; (iloop < Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->
                    DcmDspComControlSubNodeNum) && (Find == FALSE); iloop++)
            {
                if ((TRUE == Dcm_Cfg.pDcmDspCfg->pDcmDspComControl->DcmDspComControlSubNode->
                      DcmDspComControlSubNodeUsed) && (Subnet == Dcm_Cfg.pDcmDspCfg->
                      pDcmDspComControl->DcmDspComControlSubNode[iloop].DcmDspComControlSubNodeId))
                {
#if(DCM_BSWM_ENABLE == STD_ON)
                    BswM_Dcm_CommunicationMode_CurrentState(Dcm_Cfg.pDcmDspCfg->
                        pDcmDspComControl->DcmDspComControlSubNode[iloop].
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x28_ConditionCheck(
        uint8  ProtocolCtrlId,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if (E_OK == ret)
    {
        ...
        && (FALSE == Flag); SidTabIndex++)
        {
            ...
            {
                for(ServiceIndex=0;
                    (ServiceIndex < (Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabIndex].
                       DcmDsdSidTab_ServiceNum) && (FALSE == Flag); ServiceIndex++)
                {
                                ...
                                pDcmDsdService[ServiceIndex].DcmDsdSubfuncAvial == TRUE))
                    {
                             ...
                             (Flag == FALSE)); iloop++)
                        {
                            if (subFunc == (Dcm_DsdCfg.pDcmDsdServiceTable)[SidTabIndex].
                                    pDcmDsdService[ServiceIndex].DcmDsdSubService[iloop].
                                    DcmDsdSubServiceId)
                            {
                                Flag = TRUE;
                            }
    ...
    /*check the massage length*/
    if ((E_OK == ret) && (((DCM_UDS0X28_REQ_DATA_MAXLENGTH != Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)
        && ((subFunc == 0x04u) || (subFunc == 0x05u))) ||
         ((DCM_UDS0X28_REQ_DATA_MINLENGTH != Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)
        && (subFunc < 0x04u)) || (((DCM_UDS0X28_REQ_DATA_MINLENGTH !=
         Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen) || (DCM_UDS0X28_REQ_DATA_MAXLENGTH
           != Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)) && (subFunc > 0x05u))))
    {
        /*the length of massage is not correct,send NRC 0x13*/
        *ErrorCode = DCM_E_INCORRECTMESSAGELENGTHORINVALIDFORMAT;
        ret = E_NOT_OK;
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if (E_OK == ret)
    {
        ...
        {
            Subnet = (uint16)((uint16)((uint16)Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[3] <<
                    8u) | (uint16)Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[4]);
            if (E_NOT_OK == DspInternalUDS0x28_0x04_0x05_SubFuction(
                    Subnet,ProtocolCtrlId,subFunc,comType))
            {

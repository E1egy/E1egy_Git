...
typedef struct
{
    ...
    P2VAR(uint8, AUTOMATIC, DCM_VAR_NOINIT)NoFindDidReadNum;
    P2VAR(uint8, AUTOMATIC, DCM_VAR_NOINIT)DidSessionSupportNum;
    P2VAR(uint8, AUTOMATIC, AUTOMATIC)NoFindDidNum;
    P2VAR(uint8, AUTOMATIC, AUTOMATIC)MixPid;
    P2VAR(uint8, AUTOMATIC, AUTOMATIC)noFindPidNum;
    uint16 DidCfgIndex;
    P2VAR(boolean, AUTOMATIC, AUTOMATIC)readDidSignalFlag;
}Dcm_0x22Types;
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    ...
    for (Index = 0; (Index < Dcm_DspCfg.pDcmDspDid[pDidCfgIndex].DcmDspDidSignalNum)
    && (E_OK == ret); Index++)
    {
        ...
        DidSignalPos += ((Dcm_DspCfg.pDcmDspDid[pDidCfgIndex].
                pDcmDspDidSignal[Index].DcmDspDidDataPos + 7u) >> 3u);
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    ...
    for (index = 0; (index < DCM_DSP_DDDID_MAX_NUMBER)
    && (Find == FALSE); index++)
    {
                ...
                else if (DDDid->Subfunction == DCM_UDS0X2C_02_DDBYMEMORY)
                {
                    for(Index = 0u; Index < Dcm_DspCfg.pDcmDspMemory->DcmDspMemoryIdInfoNum; Index++)
                    {
                        ...
                        for(RangeInfoIndex = 0;(RangeInfoIndex < pDcmDspMemoryIdInfo->
                            DcmDspReadMemoryRangeInfoNum) && (pDcmDspMemoryIdInfo != NULL_PTR);
                            RangeInfoIndex++)
                        {
                            ...
                            if ((pDcmDspReadMemoryRangeInfo != NULL_PTR)
                                && ((pDcmDspReadMemoryRangeInfo->DcmDspReadMemoryRangeLow
                                        <= DDDid->Data) && (pDcmDspReadMemoryRangeInfo->
                                       DcmDspReadMemoryRangeHigh >= (DDDid->Data + DDDid->Size - 1u))))
                            {
                                MemoryIdentifier = Dcm_DspCfg.pDcmDspMemory->
                                        DcmDspMemoryIdInfo[Index].DcmDspMemoryIdValue;
                            }
                        }
                    }
                    ret = Dcm_ReadMemory(OpStatus,MemoryIdentifier,DDDid->Data,
                            (uint32*)&(DDDid->Size),&(Dcm_Channel[*Dcm_0x22Type->ResOffset]),pNrc);
    ...
    if (FALSE == Flag)
    {
        ...
        if ((Dcm_0x22Type->RecDid < 0xF200u) || (Dcm_0x22Type->RecDid > 0xF3FFu))
        {
            ...
            for (Idx = 0; (Idx < Dcm_DspCfg.DcmDspDidRangeNum) && (FALSE == Flagx); Idx++)
            {
                ...
                if ((Dcm_0x22Type->RecDid >= Dcm_DspCfg.pDcmDspDidRange[Idx].
                        DcmDspDidRangeIdentifierLowerLimit)
                    && (Dcm_0x22Type->RecDid <= Dcm_DspCfg.pDcmDspDidRange[Idx].
                    ...
                    if (TRUE == Dcm_DspCfg.pDcmDspDidRange[Idx].DcmDspDidRangeHasGaps)
                    {
                        if (Dcm_DspCfg.pDcmDspDidRange[Idx].DcmDspDidRangeIsDidAvailableFnc != NULL_PTR)
                        {
                            ret = Dcm_DspCfg.pDcmDspDidRange[Idx].
                             DcmDspDidRangeIsDidAvailableFnc(Dcm_0x22Type->RecDid,OpStatus,&DidSupported);
        ...
        Dcm_0x22Types* Dcm_0x22Type)
{
    ...
    *Dcm_0x22Type->readDidSignalFlag = FALSE;
    ...
    {
        pDspDidData = Dcm_DspCfg.pDcmDspDid[Dcm_0x22Type->DidCfgIndex].
        ...
        if (USE_ECU_SIGNAL == pDspDidData->DcmDspDataUsePort)
        {
            ...
            (*Dcm_0x22Type->ResOffset) += ((pDspDidData->DcmDspDataSize + 7u) >> 3u);
        ...
        else if (USE_BLOCK_ID == pDspDidData->DcmDspDataUsePort)
        {
               ...
               *Dcm_0x22Type->readDidSignalFlag = TRUE;
                ...
                {
                    (void)NvM_ReadBlock(pDspDidData->DcmDspDataBlockId,
                            (void*)&Dcm_Channel[*Dcm_0x22Type->ResOffset]);
                }
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    ...
    if (NULL_PTR == Dcm_DspCfg.pDcmDspDidRange[Dcm_0x22Type->RangeDidCfgIndex].
    ...
    {
        ret = Dcm_DspCfg.pDcmDspDidRange[Dcm_0x22Type->RangeDidCfgIndex].
              DcmDspDidRangeReadDataLengthFnc(Dcm_0x22Type->RecDid,OpStatus,pDidSize);
    }
    return ret;
}
#define DCM_STOP_SEC_CODE
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    ...
    for (DidSignalIndex = 0; (DidSignalIndex < DidSignalNum) && (E_OK == ret); DidSignalIndex ++)
    {
        ...
        if (E_OK == ret)
        {
            DidSignalPos = *ResOffset + ((Dcm_DspCfg.pDcmDspDid[DidCfgIndex].
                    pDcmDspDidSignal[DidSignalIndex].DcmDspDidDataPos + 7u) >> 3u);
            if ((NULL_PTR == pDspDidData->DcmDspDataReadFnc) || ((DidSignalPos + Size) > DCM_CHANNEL_LENGTH))
            {
                /*DcmDspDataReadFnc is NULL,send NRC 0x22*/
                *pNrc = DCM_E_CONDITIONSNOTCORRECT;
                ret = E_NOT_OK;
        ...
        P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, AUTOMATIC)pNrc)
{
    ...
    if (E_OK == ret)
    {
        ret = Dcm_DspCfg.pDcmDspDidRange[Dcm_0x22Type->RangeDidCfgIndex].DcmDspDidRangeReadDidFnc(
                Dcm_0x22Type->RecDid,&Dcm_Channel[*Dcm_0x22Type->ResOffset],OpStatus,Dcm_0x22Type->DidSize,pNrc);
        if(E_OK == ret)
        {
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)DspInternalUDS0x22_ObdDidDealF4(
        uint8 ProtocolCtrlId,Dcm_0x22Types* Dcm_0x22Type)
{
    Std_ReturnType ret = E_OK;
    uint32 SupportBuffer = 0;
    ...
    case 0xF4C0u:
    case 0xF4E0u:
        if ((*Dcm_0x22Type->MixPid == NORMAL_REQUEST) ||
        ...
        else
        {
            if (Dcm_DspCfg.DcmDspEnableObdMirror == TRUE)
            {
                ...
                if (SupportBuffer != 0UL)
                {
                    ...
                    Dcm_Channel[*Dcm_0x22Type->ResOffset] =
                            (uint8)((Dcm_0x22Type->RecDid) >> 8u); /*echo of DID MSB*/
            ...
            {
                if (E_OK != DspInternalUDS0x22_CheckSupported(
                        Dcm_0x22Type->RecDid, Dcm_0x22Type->ResOffset))
                {
                    *(Dcm_0x22Type->noFindPidNum) += 1u;
                }
            }
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)DspInternalUDS0x22_ObdDidDealF8(
        uint8 ProtocolCtrlId,Dcm_0x22Types* Dcm_0x22Type)
{
    Std_ReturnType ret = E_OK;
    uint32 SupportBuffer = 0;
        ...
        if (Dcm_DspCfg.DcmDspEnableObdMirror == TRUE)
            {
                ...
                if ((TidNum != 0u) && (ret == E_OK))
                {
                    ...
                    {
                        ret = Dem_DcmGetDTRData(Mid,iloop,&TIDvalue,&UaSID,
                                &Testvalue,&Lowlimvalue,&Upplimvalue);
                        if (ret == E_OK)
                        {
                            SchM_Enter_Dcm(Dcm_Channel);
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] =
                                    (uint8)((Dcm_0x22Type->RecDid) >> 8u);
                            /*echo of DID MSB*/
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] = (uint8)(Dcm_0x22Type->RecDid);
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            ...
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] = (uint8)((Testvalue &
                                    (OBD_DATA_LSB_MASK << 8u)) >> 8u);
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] =
                                    (uint8)(Testvalue & OBD_DATA_LSB_MASK);
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] = (uint8)((Lowlimvalue &
                                    (OBD_DATA_LSB_MASK << 8u)) >> 8u);
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] =
                                    (uint8)(Lowlimvalue & OBD_DATA_LSB_MASK);
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] = (uint8)((Upplimvalue &
                                    (OBD_DATA_LSB_MASK << 8u)) >> 8u);
                            (*Dcm_0x22Type->ResOffset) += 1u;
                            Dcm_Channel[*Dcm_0x22Type->ResOffset] =
                                    (uint8)(Upplimvalue & OBD_DATA_LSB_MASK);
                            SchM_Exit_Dcm(Dcm_Channel);
                            (*Dcm_0x22Type->ResOffset) += 1u;
                        }
                    }
...
)
{
...
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
    uint8     TxChannelCfgIndex =
    Dcm_ChannelCtrl[Dcm_MsgCtrl[MsgCtrlId].Dcm_TxCtrlChannelIndex].Dcm_ChannelCfgIndex;
    uint16 Offset = (Dcm_DslCfg.pDcmChannelCfg)[TxChannelCfgIndex].offset;
#endif
    ...
    if (FALSE == readDidSignalFlag)
    {
        ...
        {
#if((STD_ON == DCM_UDS_SERVICE0X2C_ENABLED) && (DCM_DSP_DDDID_MAX_NUMBER > 0u))
           if ((Dcm_DspCfg.pDcmDspDidInfo[Dcm_DspCfg.pDcmDspDid[Dcm_0x22Type->DidCfgIndex].
                     DcmDspDidInfoIndex].DcmDspDidDynamicallyDefined == TRUE)
                  && (Dcm_DspCfg.pDcmDspDidInfo[Dcm_DspCfg.pDcmDspDid[Dcm_0x22Type->DidCfgIndex].
           ...
           {
                /*set the response message*/
               ret = Dcm_UdsAssembleResponse(
                       OpStatus,Dcm_0x22Type->DidCfgIndex,Dcm_0x22Type->ResOffset,ErrorCode);
               if (DCM_E_PENDING == ret)
               {
                   SchM_Enter_Dcm(Dcm_MsgCtrl);
                   Dcm_MsgCtrl[MsgCtrlId].Dcm_OpStatus = DCM_PENDING;
...
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
            if((E_OK == ret) && ((*Dcm_0x22Type->ResOffset - Offset)
                > (Dcm_DslCfg.pDcmChannelCfg[TxChannelCfgIndex].Dcm_DslBufferSize)))
            {
                SchM_Enter_Dcm(Dcm_PageBufferData);
                Dcm_PageBufferData.LastFilled = FALSE;
    ...
    else
    {
        ...
        if (E_OK == ret)
        {
            ...
            ret = DspInternalUDS0x22_DidSessionCheck(
                    DidInfoCfgIndex,Dcm_0x22Type->DidSessionSupportNum,ErrorCode);
            if(E_NOT_OK == ret)
            {
        ...
        if ((E_OK == ret) && (DidSessionCheck == TRUE))
        {/*set the response message*/
            SchM_Enter_Dcm(Dcm_Channel);
            Dcm_Channel[*Dcm_0x22Type->ResOffset] = (uint8)((Dcm_0x22Type->RecDid) >> 8u);
            /*echo of DID MSB*/
            ...
            {
                Dcm_0x22Type->DidCfgIndex = DidCfgIndex;
                ret = DspInternalUDS0x22_NonObdDidNOrangeDeal(OpStatus,ProtocolCtrlId,Dcm_0x22Type,ErrorCode);
            }
            else
            {
                Dcm_0x22Type->RangeDidCfgIndex = RangeDidCfgIndex;
                 /*Read the current length of the data in DID*/
                 ret = DspInternalUDS0x22_ReadRangeDidDataLength(
                         OpStatus,Dcm_0x22Type,&DidSize,ErrorCode);
                if (DCM_E_PENDING == ret)
                {
                    SchM_Enter_Dcm(Dcm_MsgCtrl);
                    Dcm_MsgCtrl[MsgCtrlId].Dcm_OpStatus = DCM_PENDING;
                ...
                else if (E_OK == ret)
                {
                    ...
                    if (DidSize > Dcm_DspCfg.pDcmDspDidRange[RangeDidCfgIndex].
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    *ErrorCode = DCM_E_REQUESTOUTOFRANGE;
    ...
    if(E_OK == ret)
    {
        for (; (Index0 < DidNum) && (E_OK == ret)
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
        && ((Dcm_PageBufferData.LastFilled == TRUE) || (Dcm_PageBufferData.TotalSize == 0UL))
#endif
        ; Index0++)
        {
            RecDid = (uint16)(((uint16)Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[ReqOffset])
              << 8u) |((uint16)(Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[ReqOffset + 1u]));
...
#endif
            if (((MixPid == NEED_CALL_UDS_API) || (MixPid == NONE_PID)) && (E_OK == ret))
            {
                Dcm_0x22Type.RecDid = RecDid;
                ...
                Dcm_0x22Type.ResOffset = &ResOffset;
                ret = DspInternalUDS0x22_NonObdDidDeal(
                        OpStatus,ProtocolCtrlId,&Dcm_0x22Type,ErrorCode);
            }
        }
        if (((NoFindDidNum == DidNum) || (NoFindDidReadNum == DidNum) || (noFindPidNum == DidNum)
                || (DidSessionSupportNum == 0u)) && (E_OK == ret))
        {
    ...
    {
        /* check tx data length */
        ret = Dcm_UDS0x22_CheckTxLength(ProtocolCtrlId,
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
                Index0,ReqOffset,
#endif
                ResOffset);

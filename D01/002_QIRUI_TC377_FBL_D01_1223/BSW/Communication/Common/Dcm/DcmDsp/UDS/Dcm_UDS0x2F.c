...
typedef struct
{
    ...
    P2VAR(uint8, AUTOMATIC, DCM_VAR_NOINIT) pControlEnableMaskRecord;
        ...
        for (iloop = 0; iloop < DCM_DSP_DID_FOR_2F_NUM; iloop++)
        {
            if (DcmOnControlDidfor2F[iloop] != 0u)
            {
                    ...
                    DidIndex < Dcm_DspCfg.DcmDspDidNum; DidIndex++)
                {
                    ...
                    && (DcmOnControlDidfor2F[iloop] ==
                       Dcm_DspCfg.pDcmDspDid[DidIndex].DcmDspDidId);SingalIndex++)
                    {
                        if ((Dcm_DspCfg.pDcmDspDid[DidIndex].pDcmDspDidSignal[SingalIndex].
                            pDcmDspDidData->DcmDspDataUsePort == USE_ECU_SIGNAL)
                            && (Dcm_DspCfg.pDcmDspDid[DidIndex].pDcmDspDidSignal[SingalIndex].
                                    pDcmDspDidData->DcmDspDataEcuSignalFnc != NULL_PTR))
                        {
                            Dcm_DspCfg.pDcmDspDid[DidIndex].pDcmDspDidSignal[SingalIndex].
                            pDcmDspDidData->DcmDspDataEcuSignalFnc(
                                    DCM_UDS0X2F_RETURNCONTROLTOECU,0xFF);
                        }
                        else if (((Dcm_DspCfg.pDcmDspDid[DidIndex].pDcmDspDidSignal[SingalIndex].
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) pNrc)
{
    ...
    {
        ret = pDspDidData->DcmDspDataReadFnc(OpStatus,pControlStatusRecord,pNrc);
    }
    return ret;
}
#define DCM_STOP_SEC_CODE
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    switch (InputOutControlParameter)
    {
        case DCM_UDS0X2F_RETURNCONTROLTOECU:/*ReturnControlToEcu*/
            if (Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->
                    DcmDspDataReturnControlToECUFnc != NULL_PTR)
            {
                ret = Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->
                      DcmDspDataReturnControlToECUFnc(
                              Dcm_0x2FType->pControlEnableMaskRecord,ErrorCode);
                if (ret == E_OK)
                {
                    ret = Dcm_Uds0x2FServiceDidCallReadAPI(OpStatus,
                          Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData,
                          Dcm_0x2FType->pControlStatusRecord,ErrorCode);
                }
            }
            else
            {
        ...
        case DCM_UDS0X2F_RESETTODEFAULT: /*ResetToDefault*/
            if ((Dcm_0x2FType->pDspDidControl->DcmDspDidResetToDefault == TRUE)
                && (Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataResetToDefaultFnc
                        != NULL_PTR))
            {
                ret = Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->
                        DcmDspDataResetToDefaultFnc(OpStatus,
                                Dcm_0x2FType->pControlEnableMaskRecord,ErrorCode);
                if (ret == E_OK)
                {
                    ret = Dcm_Uds0x2FServiceDidCallReadAPI(OpStatus,
        ...
        case DCM_UDS0X2F_FREEZECURRENTSTATE: /*FreezeCurrentState*/
            if ((Dcm_0x2FType->pDspDidControl->DcmDspDidFreezeCurrentState == TRUE)
                && (Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataFreezeCurrentStateFnc
                        != NULL_PTR))
            {
                ret = Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->
                        DcmDspDataFreezeCurrentStateFnc(OpStatus,
                                Dcm_0x2FType->pControlEnableMaskRecord,ErrorCode);
                if (ret == E_OK)
                {
                    ret = Dcm_Uds0x2FServiceDidCallReadAPI(OpStatus,
        ...
        case DCM_UDS0X2F_SHORTTERMADJUSTMENT:/*ShortTermAdjustment*/
            if (0u != Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataSize)
            {
                pControlOptionRecord = &(Dcm_MsgCtrl[MsgCtrlId].
                        MsgContext.pReqData[4u + Dcm_0x2FType->InputDataOffset]);
            }
            if ((Dcm_0x2FType->pDspDidControl->DcmDspDidShortTermAdjustement == TRUE)
                && (Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataShortTermAdjustmentFnc
                        != NULL_PTR))
            {
                ret = Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->
                        DcmDspDataShortTermAdjustmentFnc(pControlOptionRecord,
                        ((Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataSize + 7u) >> 3u),
                        OpStatus,Dcm_0x2FType->pControlEnableMaskRecord,ErrorCode);
                if (ret == E_OK)
                {
                    ret = Dcm_Uds0x2FServiceDidCallReadAPI(OpStatus,
                            Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData,
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    uint16 Offset = (Dcm_DslCfg.pDcmChannelCfg)[Dcm_ChannelCtrl[Dcm_MsgCtrl[MsgCtrlId].
                     Dcm_TxCtrlChannelIndex].Dcm_ChannelCfgIndex].offset;
    uint8 InputOutControlParameter = Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[3u];
        ...
        && (Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataEcuSignalFnc != NULL_PTR))
    {
        ...
        Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataEcuSignalFnc(
                InputOutControlParameter,SignalValue);
    }
    else if ((Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataUsePort ==
            USE_DATA_SYNCH_FNC) || (Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->
            DcmDspDataUsePort == USE_DATA_ASYNCH_FNC) || (Dcm_0x2FType->pDcmDspDidSignal->
            pDcmDspDidData->DcmDspDataUsePort == USE_DATA_ASYNCH_FNC_ERROR) ||
            (Dcm_0x2FType->pDcmDspDidSignal->pDcmDspDidData->DcmDspDataUsePort ==
            USE_DATA_SYNCH_CLIENT_SERVER) || (Dcm_0x2FType->pDcmDspDidSignal->
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    /*session check*/
    if (pDspDidControl->DcmDspDidControlSessionRefNum != 0u)
    {
        for (Index = 0; (Index < pDspDidControl->DcmDspDidControlSessionRefNum) &&
        (FALSE == Flag); Index++)
        {
    ...
    {
        Flag = FALSE;
        if (pDspDidControl->DcmDspDidControlSecurityLevelRefNum != 0u)
        {
            for (Index = 0; (Index < pDspDidControl->DcmDspDidControlSecurityLevelRefNum) &&
            (FALSE == Flag); Index++)
            {
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    /*min-length check*/
    if ((E_OK == ret) &&
            (DCM_UDS0X2F_REQ_DATA_MINLENGTH > Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen))
    {
    ...
    if(E_OK == ret)
    {
        ...
        /*configuration check*/
        if ((NULL_PTR != pDcmDspDidSignal) && ((pDspDidControl->DcmDspDidControlMask != DCM_CONTROLMASK_NO) &&
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    /*total length check*/
    if (((Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen != (uint32)((uint32)((uint32)4UL
        + Dcm_0x2FType->InputDidSize + Dcm_0x2FType->pDspDidControl->DcmDspDidControlMaskSize))) &&
        (InputOutControlParameter == DCM_UDS0X2F_SHORTTERMADJUSTMENT)) ||
        ((Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen != (uint32)((uint32)((uint32)4UL +
    ...
    {
        ret = Dcm_Uds0x2FServiceDidSessionAndSecutityCheck(Dcm_0x2FType->pDspDidControl,ErrorCode);
    }
    if(E_OK == ret)
    {
        if ((Dcm_DspCfg.pDcmDspDid)[Dcm_0x2FType->DidCfgIndex].DcmDspDidSignalNum > 1u)
        {
            ...
            {
                pDcmDspDidSignal =
                        (Dcm_DspCfg.pDcmDspDid)[Dcm_0x2FType->DidCfgIndex].pDcmDspDidSignal;
                for (iloop = 0; iloop <
                (Dcm_DspCfg.pDcmDspDid)[Dcm_0x2FType->DidCfgIndex].DcmDspDidSignalNum; iloop++)
                {
                    if (Dcm_0x2FType->pDspDidControl->DcmDspDidControlMask ==
                            DCM_CONTROLMASK_INTERNAL)
                    {
                        ...
                        if ((Dcm_0x2FType->Mask &
                                (uint32)((uint32)((uint32)0x01u << (Postion - iloop)))) != 0UL)
                        {
                            Dcm_0x2FType->InputDataOffset = InputDataOffset;
                            Dcm_0x2FType->pDcmDspDidSignal = pDcmDspDidSignal;
                            ret = Dcm_Uds0x2FServiceDidCallAPI(
                                    OpStatus,Dcm_0x2FType,MsgCtrlId,ErrorCode);
                        }
                    }
                    else if (Dcm_0x2FType->pDspDidControl->DcmDspDidControlMask ==
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
   ...
   /*if the required protocol is configured,get the index of runtime datum*/
    ...
    if((E_OK == ret) && ((4u + (uint32)ControlStatusRecordSize) >

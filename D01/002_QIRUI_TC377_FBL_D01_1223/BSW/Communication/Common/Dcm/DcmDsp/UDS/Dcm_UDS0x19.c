...
typedef struct
{
    ...
    P2VAR(boolean,AUTOMATIC,DCM_VAR) NotAllowedFlag;
    P2VAR(Dcm_MsgLenType,AUTOMATIC,DCM_VAR) MsgLen;
    P2VAR(uint16,AUTOMATIC,DCM_VAR) Index;
    uint16 Offset;
        ...
        if (E_OK == ret)
        {
            ...
            else
            {
                ...
                ReturnSetDTCFilter = Dem_DcmSetDTCFilter(DtcStatusMask,
                        DEM_DTC_KIND_ALL_DTCS,DEM_DTC_FORMAT_UDS,
                        DEM_DTC_ORIGIN_PRIMARY_MEMORY,FALSE,
                        DEM_SEVERITY_NO_SEVERITY,FALSE);
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x19_02_SubDealTwo(uint8 ProtocolCtrlId,
        Dcm_MsgLenType  MsgLen,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    uint8     TxChannelCtrlIndex;
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(void, DCM_CODE)Dcm_UDS0x19_02_SubDealone(uint8 ProtocolCtrlId,
        Dcm_UDS0x19Types* Dcm_UDS0x19Type)
{
   ...
   ReturnSetDTCFilter = Dem_DcmSetDTCFilter(DtcStatusMask,DEM_DTC_KIND_ALL_DTCS,
      DEM_DTC_FORMAT_UDS,DEM_DTC_ORIGIN_PRIMARY_MEMORY,FALSE,
      DEM_SEVERITY_NO_SEVERITY,FALSE);
   if (DEM_FILTER_ACCEPTED == ReturnSetDTCFilter)
   {
       ...
       switch(ReturnGetNumberOfFilteredDTC)
       {
           case DEM_NUMBER_OK:
                Flag = FALSE;
                for(*(Dcm_UDS0x19Type->Index) = 0; (*(Dcm_UDS0x19Type->Index) <
                        *(Dcm_UDS0x19Type->DtcCount)) && (FALSE == Flag)
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
                ...
                {
                   ReturnGetNextFilteredDTC = Dem_DcmGetNextFilteredDTC(
                           &Dtc,&DtcStatus);
                   switch(ReturnGetNextFilteredDTC)
                   {
                       case  DEM_FILTERED_OK:
                             ...
                             {
                                 SchM_Enter_Dcm(Dcm_Channel);
                                 Dcm_Channel[Dcm_UDS0x19Type->Offset +
                                             *(Dcm_UDS0x19Type->MsgLen)] = (uint8)(Dtc >> 16u);
                                 *(Dcm_UDS0x19Type->MsgLen)  = *(Dcm_UDS0x19Type->MsgLen) + 1u;
                                 Dcm_Channel[Dcm_UDS0x19Type->Offset + *(Dcm_UDS0x19Type->MsgLen)] =
                                         (uint8)(Dtc >> 8u);
                                 *(Dcm_UDS0x19Type->MsgLen)  = *(Dcm_UDS0x19Type->MsgLen) + 1u;
                                 Dcm_Channel[Dcm_UDS0x19Type->Offset +
                                             *(Dcm_UDS0x19Type->MsgLen)] = (uint8)(Dtc);
                                 *(Dcm_UDS0x19Type->MsgLen)  = *(Dcm_UDS0x19Type->MsgLen) + 1u;
                                 Dcm_Channel[Dcm_UDS0x19Type->Offset +
                                             *(Dcm_UDS0x19Type->MsgLen)] = DtcStatus;
                                 SchM_Exit_Dcm(Dcm_Channel);
                                 *(Dcm_UDS0x19Type->MsgLen)  = *(Dcm_UDS0x19Type->MsgLen) + 1u;
                             }
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
                             if ((*(Dcm_UDS0x19Type->MsgLen)) > (Dcm_DslCfg.
                                   pDcmChannelCfg[TxChannelCfgIndex].
                                   Dcm_DslBufferSize))
                             {
                                 SchM_Enter_Dcm(Dcm_PageBufferData);
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x19_03_SubDeal(uint8 ProtocolCtrlId,
        Dcm_MsgLenType  MsgLen,
        uint16 iloop,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if (E_OK == ret)
    {
        ...
        if (E_OK == ret)
        {
            ...
            if (DEM_WRONG_FILTER == Dem_DcmSetFreezeFrameRecordFilter(DEM_DTC_FORMAT_UDS,
    ...
    {
        MsgLen = 2u;
        for (iloop = 0;((iloop < NumberOfFilteredRecords)
             && (ReturnGetNextFilteredElement != DEM_FILTERED_NO_MATCHING_ELEMENT)); iloop++)
        {
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x19_04_SubSubDeal(uint8 ProtocolCtrlId,
        Dcm_UDS0x19Types* Dcm_UDS0x19Type,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    uint8     TxChannelCtrlIndex;
    ...
    *(Dcm_UDS0x19Type->MsgLen) = *(Dcm_UDS0x19Type->MsgLen) + 1u;
    ...
    ReturnDisableDTCRecordUpdate = Dem_DcmDisableDTCRecordUpdate(Dtc,
            DEM_DTC_ORIGIN_PRIMARY_MEMORY);
    if(DEM_DISABLE_DTCRECUP_OK == ReturnDisableDTCRecordUpdate)
    {
        for(Index = 1u; (Index < 0xEFu) && (ret == E_OK); Index++)
        {/*reserve 1byte to store record number*/
            CurBufferSize = (ChannelSize - (uint16)*(Dcm_UDS0x19Type->MsgLen) - 1u);
            retGetFreezeFrameDataByDTC = Dem_DcmGetFreezeFrameDataByDTC(Dtc,
                    DEM_DTC_ORIGIN_PRIMARY_MEMORY,Index,
                    &Dcm_Channel[Offset + (uint16)*(Dcm_UDS0x19Type->MsgLen) + 1u],
                    &CurBufferSize);
            if(DEM_GET_FFDATABYDTC_OK == retGetFreezeFrameDataByDTC)
            {
                ...
                SchM_Exit_Dcm(Dcm_Channel);
                *(Dcm_UDS0x19Type->MsgLen) = *(Dcm_UDS0x19Type->MsgLen) + CurBufferSize + 1u;
            }
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x19_04_SubDeal(uint8 ProtocolCtrlId,
        Dcm_UDS0x19Types* Dcm_UDS0x19Type,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    uint8     TxChannelCtrlIndex;
    ...
    *(Dcm_UDS0x19Type->MsgLen) = *(Dcm_UDS0x19Type->MsgLen) + 1u;
    ...
    {/*read-out the correspoding snapshot by SnapshotRecord*/
        ...
        {/*reserve 1byte to store record number*/
            CurBufferSize = (ChannelSize - (uint16)*(Dcm_UDS0x19Type->MsgLen) - 1u);
            retGetFreezeFrameDataByDTC = Dem_DcmGetFreezeFrameDataByDTC(Dtc,
              DEM_DTC_ORIGIN_PRIMARY_MEMORY,SnapshotRecord,
              &Dcm_Channel[Offset + (uint16)*(Dcm_UDS0x19Type->MsgLen)+1u],&CurBufferSize);
            if(DEM_GET_FFDATABYDTC_OK == retGetFreezeFrameDataByDTC)
            {
                SchM_Enter_Dcm(Dcm_Channel);
                Dcm_Channel[Offset + (uint16)*(Dcm_UDS0x19Type->MsgLen)] = SnapshotRecord;
                SchM_Exit_Dcm(Dcm_Channel);
                *(Dcm_UDS0x19Type->MsgLen) = *(Dcm_UDS0x19Type->MsgLen) + CurBufferSize + 1u;
            }
            else if(DEM_GET_FFDATABYDTC_PENDING == retGetFreezeFrameDataByDTC)
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x19_06SubDeal(uint8 ProtocolCtrlId,
        P2VAR(boolean,AUTOMATIC,DCM_VAR) PendingFlag,
        P2VAR(Dcm_MsgLenType,AUTOMATIC,DCM_VAR) MsgLen,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    for (; Index <= 0xEFu; Index++)
    {
        ...
        ReturnGetExtendedDataRecord = Dem_DcmGetExtendedDataRecordByDTC(Dtc,
              DEM_DTC_ORIGIN_PRIMARY_MEMORY,Index,
              &Dcm_Channel[Offset + (uint16)(*MsgLen) + 1u],&CurChannelSize);
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    else
    {
        ...
        ReturnDisableDTCRecordUpdate = Dem_DcmDisableDTCRecordUpdate(
                Dtc,DEM_DTC_ORIGIN_PRIMARY_MEMORY);
        if(DEM_DISABLE_DTCRECUP_OK == ReturnDisableDTCRecordUpdate)
        {
            ...
            else
            {
                ...
                ReturnGetExtendedDataRecord = Dem_DcmGetExtendedDataRecordByDTC(Dtc,
                        DEM_DTC_ORIGIN_PRIMARY_MEMORY,DtcExtendedDataRecordNumber,
                        &Dcm_Channel[Offset + MsgLen + 1u],&CurChannelSize);
                if ((DEM_RECORD_OK == ReturnGetExtendedDataRecord)&&(CurChannelSize != 0u))
                {
 ...
 */
/*************************************************************************/
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x19_0ADeal(uint8 ProtocolCtrlId,
        P2VAR(uint16,AUTOMATIC,DCM_VAR) DtcCount,
        P2VAR(Dcm_MsgLenType,AUTOMATIC,DCM_VAR) MsgLen,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    else
    {
        /*one byte for the storage of DTC status availability mask*/
        (*MsgLen) = (*MsgLen) + 1u;
        ReturnSetDTCFilter = Dem_DcmSetDTCFilter(0x00,DEM_DTC_KIND_ALL_DTCS,
                DEM_DTC_FORMAT_UDS,DEM_DTC_ORIGIN_PRIMARY_MEMORY,FALSE,
                DEM_SEVERITY_NO_SEVERITY,FALSE);
        if (DEM_FILTER_ACCEPTED == ReturnSetDTCFilter)
        {
                ...
                case DEM_NUMBER_OK:
                     ...
                     ; Index++)
                     {
                        ...
                        case  DEM_FILTERED_OK:
                              ...
                              if (((Dcm_PageBufferData.TotalSize == 0UL)
                              || (Index > Dcm_PageBufferData.IloopOne)
                              || (Index == 0u)) && (((Dcm_PageBufferData.TotalSize != 0UL)
...
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
                              if ((*MsgLen) > (Dcm_DslCfg.
                                pDcmChannelCfg[TxChannelCfgIndex].Dcm_DslBufferSize))
                              {
                                  SchM_Enter_Dcm(Dcm_PageBufferData);
                                  Dcm_PageBufferData.LastFilled = FALSE;
                                  SchM_Exit_Dcm(Dcm_PageBufferData);
...
/*************************************************************************/
/*******************************************/
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x19_0E(uint8 ProtocolCtrlId,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    uint8     TxChannelCtrlIndex;
    uint8     TxChannelCfgIndex;

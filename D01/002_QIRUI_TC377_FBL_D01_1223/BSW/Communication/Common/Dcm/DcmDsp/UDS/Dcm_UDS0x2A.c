...
typedef struct
{
    ...
    P2VAR(uint8, AUTOMATIC, DCM_APPL_CONST) DidSessionSupportNum;
}Dcm_0x2ATypes;
...
#include "Dcm_MemMap.h"
static VAR(uint8, DCM_VAR_POWER_ON_INIT) SchedulerPengingDDDid = FALSE;
        ...
        P2VAR(uint8, AUTOMATIC, AUTOMATIC)Data)
{
    ...
    for (DidSignalIndex = 0;(DidSignalIndex < DidSignalNum) &&
    (ret == E_OK); DidSignalIndex ++)
    {
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)DspInternalUDS0x2A_DDDid_Read_Data(
        Dcm_OpStatusType OpStatus,
        uint16 SchedulerQueueIndex)
{
    Std_ReturnType ret = E_OK;
    P2CONST(Dcm_DDDidElementsDataTypes,TYPEDEF,DCM_CONST) DDDid;
    uint8 DDDidNum;
    ...
    if (Find == TRUE)
    {
        ...
        for (iloop = 0; iloop < DDDidNum; iloop++)
        {
            ...
            {
                for(IdInfoIndex = 0u;IdInfoIndex <
                Dcm_DspCfg.pDcmDspMemory->DcmDspMemoryIdInfoNum;IdInfoIndex++)
                {
                    ...
                    {
                        for(RangeInfoIndex = 0; RangeInfoIndex <
                        pDcmDspMemoryIdInfo->DcmDspReadMemoryRangeInfoNum; RangeInfoIndex++)
                        {
                            pDcmDspReadMemoryRangeInfo =
                ...
                ret = Dcm_ReadMemory(OpStatus,MemoryIdentifier,
                        DDDid->Data,(uint32*)&DDDid->Size,Data,&ErrorCode);
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(Std_ReturnType, DCM_CODE)DspInternalUDS0x2A_Did_Read_Data(
                        uint16 SchedulerQueueIndex)
{
    uint16 DidSignalPos = 0;
    uint8 DidSignalIndex;
    ...
    if (ret == E_OK)
    {
        ...
        for (DidSignalIndex = 0; DidSignalIndex < DidSignalNum; DidSignalIndex++)
        {
            ...
            DidSignalPos +=((Dcm_DspCfg.pDcmDspDid[DidCfgIndex].
                    pDcmDspDidSignal[DidSignalIndex].DcmDspDidDataPos + 7u) >> 3u);
            if ((NULL_PTR == pDspDidData->DcmDspDataReadFnc) || (Size > 3u))
            {
                ret = E_NOT_OK;
            }
            else
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE)Dcm_Scheduler_0x2A_Init(void)
{
    uint16 Index;
 ...
 */
/***********************************************************************/
static FUNC(boolean,DCM_CODE)Dcm_MainFunction_0x2A_SubDeal(uint16 iloop,
        uint16 index,
        P2CONST(Dcm_DslMainConnectionType,TYPEDEF,DCM_CONST)pDcmDslMainConnection)
{
    ...
    if (SchedulerQueue[iloop].Counter == 0u)
    {
        ...
        if (ret == E_OK)
        {
...
#if (STD_ON == DCM_GENERIC_CONNECTION)
            if ((pDcmDslMainConnection->pDcmDslPeriodicTranmissionConRef->
                    DcmDslPeriodicConnection[index].DcmDslMetaDataFlag & DCM_METADATA_LENGTH_MASK) > 0u)
            {
            ...
            {
                if (Scheduler_0x2A_Transmit[cnt].PduId == pDcmDslMainConnection->
                 pDcmDslPeriodicTranmissionConRef->DcmDslPeriodicConnection[index].
            ...
            switch (SchedulerQueue[iloop].SubFunction)
            {
                case DCM_UDS0X2A_01_SLOW:
                    SchedulerQueue[iloop].Counter = (uint8)(Dcm_DspCfg.
...
#define  DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
FUNC(void,DCM_CODE)Dcm_MainFunction_Scheduler_0x2A(uint8  ProtocolCtrlId)
{
    boolean Find;
    ...
    pDcmDslMainConnection = Dcm_DslCfg.pDcmDslProtocol->pDcmDslProtocolRow[ProtocolCtrlId].
                    pDcmDslConnection->pDcmDslMainConnection;
    if ((pDcmDslMainConnection == NULL_PTR) || (pDcmDslMainConnection->
            pDcmDslPeriodicTranmissionConRef == NULL_PTR))
    {
    ...
    if (ProcessContinue == TRUE)
    {
        for (index = 0; index < pDcmDslMainConnection->
        pDcmDslPeriodicTranmissionConRef->DcmPeriodicTxPduIdNum; index++)
        {
            for (cnt=0u;cnt<DCM_PERIODICCONNECTION_NUM;cnt++)
            {
                if ((Scheduler_0x2A_Transmit[cnt].PduId == pDcmDslMainConnection->
...
#if (DCM_DSP_DDDID_MAX_NUMBER > 0u)
                    for (iloop = 0; (iloop < DCM_DSP_DDDID_MAX_NUMBER)
                    && (E_NOT_OK == ret); iloop++)
                    {
                        if (RecDid == Dcm_DDDid[iloop].DDDid)
                        {
...
{
    uint16 Cnt;
    for (Cnt = Index; (Cnt < (SchedulerNum - 1u)) &&
    (SchedulerNum <= DCM_DSP_MAX_PERIODIC_DID_SCHEDULER);Cnt++)
    {
...
#if (STD_ON == DCM_GENERIC_CONNECTION)
        SchedulerQueue[Cnt].TA = SchedulerQueue[Cnt+1u].TA;
        SchedulerQueue[Cnt].SA = SchedulerQueue[Cnt+1u].SA;
#endif
    }
...
#define DCM_START_SEC_CODE
#include "Dcm_MemMap.h"
static FUNC(void, DCM_CODE)  Dcm_UDS0x2ACheckNewSession_SubDeal(uint16 Index,
        ...
        )
{
        ...
        && (NULL_PTR != pDspDidRead))
    {
        if (pDspDidRead->DcmDspDidReadSessionRefNum != 0u)
        {
            ...
            {
                if (Dcm_MkCtrl.Dcm_ActiveSes == pDspDidRead->
                        pDcmDspDidReadSessionRow[Idx])
                {
                    *SesSupport = TRUE;
                }
            }
...
FUNC(void, DCM_CODE)  Dcm_UDS0x2ACheckNewSession(Dcm_SesCtrlType NewSes)
{
    ...
    else
    {
        for(Index=0;Index<SchedulerNum;Index++)
        {
            ...
            for (iloop = 0; iloop < Dcm_DspCfg.DcmDspDidNum; iloop++)
            {
                Dcm_UDS0x2ACheckNewSession_SubDeal(Index,&SesSupport,&SecSupport
#if ((STD_ON == DCM_UDS_SERVICE0X2C_ENABLED) && (DCM_DSP_DDDID_MAX_NUMBER > 0u))
                        ,&DDDidSupport
#endif
                        );
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    for (index = 0; index < Dcm_DspCfg.pDcmDspDid[DidCfgIndex].
    DcmDspDidSignalNum; index++)
    {
        pDspDidData = Dcm_DspCfg.pDcmDspDid[DidCfgIndex].
        ...
        {
            ret = pDspDidData->DcmDspDataConditionCheckReadFnc(
                    OpStatus,ErrorCode);
        }
        if (E_OK == ret)
        {
            ...
            {
                DidRxQueue[DidSupportNum].Length =
...
static FUNC(Std_ReturnType, DCM_CODE)Dcm_UDS0x2A_SubFunDeal_0x01_0x03(
        Dcm_OpStatusType OpStatus,
        uint8  ProtocolCtrlId,Dcm_0x2ATypes* Dcm_0x2AType,
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    Std_ReturnType ret = E_OK;
    ...
    for (iloop = 0; iloop < DidNum; iloop++)
    {
        ...
        if ((Find == FALSE) && (E_OK == DspInternalUDS0x2A_DidCheck(RecDid,&DidCfgIndex
#if (DCM_DSP_DDDID_MAX_NUMBER > 0u)
                ,&DDDid,&DDDidIndex
#endif
                )))
            ...
            if (Dcm_DspCfg.pDcmDspDidInfo[DidInfoCfgIndex].pDcmDspDidRead != NULL_PTR)
            {
                ...
                if(E_OK == ret)
                {
                    ...
                    if (E_OK == ret)
                    {
                        ...
                        if ((DDDid == TRUE) &&
                                (*(Dcm_DspCfg.DcmDspDDDIDcheckPerSourceDID) == TRUE))
                        {
                            ...
                            if (DDDid == FALSE)
                        {
                            ret = Dcm_UDS0x2A_SubFunSubDeal(
                                    OpStatus,*Dcm_0x2AType->DidSupportNum,ErrorCode);
                        }
                        else
                        {
                            /*idle*/
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    case DCM_UDS0X2A_04_STOP:
        ...
        else
        {
            for (iloop = 0;iloop < DidNum;iloop++)
            {
                RecDid = (uint16)(((uint16)0xF2) << 8u) |
                      (Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[2u + iloop]);
                if (E_OK == DspInternalUDS0x2A_DidCheck(RecDid,&DidCfgIndex
#if (DCM_DSP_DDDID_MAX_NUMBER > 0u)
                        ,&DDDid,&DDDidIndex
#endif
                        ))
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    if((E_OK == ret)
        ...
        DCM_UDS0X2A_SUBFUNC0X04_REQ_DATA_MINLENGTH) || ((Dcm_MsgCtrl[MsgCtrlId].
        MsgContext.ReqDataLen < DCM_UDS0X2A_REQ_DATA_MINLENGTH) && ((DCM_UDS0X2A_01_SLOW ==
        Subfunction) || (DCM_UDS0X2A_02_MEDIUM == Subfunction) || (DCM_UDS0X2A_03_FAST ==
        Subfunction))) || ((uint32)((uint32)Dcm_DspCfg.DcmDspMaxPeriodicDidToRead + 2u) <
                Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)))
    {
    ...
    if((E_OK == ret)
        && ((Subfunction < DCM_UDS0X2A_01_SLOW) || (Subfunction > DCM_UDS0X2A_04_STOP)))
    {
        ...
        P2VAR(Dcm_NegativeResponseCodeType,AUTOMATIC,DCM_VAR) ErrorCode)
{
    ...
    uint8     Subfunction;
    boolean   Find;
    static VAR(uint8, DCM_VAR_POWER_ON_INIT) SchedulerIndexChanged = FALSE;
    /*if the required protocol is configured,get the index of runtime datum*/
    MsgCtrlId = Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex;
    Subfunction = Dcm_MsgCtrl[MsgCtrlId].MsgContext.pReqData[1u];
    ...
    if ((E_OK == ret) && (DCM_UDS0X2A_04_STOP != Subfunction))
    {
        ...
        {
            Find = FALSE;
            for (iloop = 0; (iloop < DCM_DSP_MAX_PERIODIC_DID_SCHEDULER) &&
            (Find == FALSE); iloop++)
            {
    ...
    if((E_OK == ret)
        && ((1u) > (Dcm_DslCfg.pDcmChannelCfg[TxChannelCfgIndex].Dcm_DslBufferSize)))
    {

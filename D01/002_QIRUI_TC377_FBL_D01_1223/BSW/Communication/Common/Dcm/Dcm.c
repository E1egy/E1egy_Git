...
boolean BootWarmStartCheck = FALSE;
boolean BootResponseRequired = FALSE;
...
FUNC(void,DCM_CODE)Dcm_Init(P2CONST(Dcm_CfgType, DCM_CONST, DCM_CONST_PBCFG)ConfigPtr)
{
...
#if ((DCM_DDDID_STORAGE == STD_ON)&&(STD_ON == DCM_UDS_SERVICE0X2C_ENABLED))
        Dcm_UDS0x2C_Init();
#endif
    ...
    if(*((uint32 *)(0xAF03F004))== 0x10021002)
    {
        BootResponseRequired = TRUE;
    }
    else
    {
        BootResponseRequired = FALSE;
    }
#else
    if(TRUE == AppReqProg)
    {
        BootResponseRequired = TRUE; /*APP Send 78 , FBL Response*/
    	DFlashDriver_ClearProgFlag();
    }
    else
    {
        BootResponseRequired = FALSE;
    ...
    for (SecCfgIndex = 0; SecCfgIndex < DCM_SECURITY_NUM; SecCfgIndex++)
    {
        if(DCM_SECTIMER_ON == (Dcm_SecCtrl.Dcm_RunDlyCtrl.Dcm_SecTimerState[SecCfgIndex]))
        {
            ...
            if(Timer >= ExpireTimer)
            {
                ...
                /*********************************/
                Dcm_SecCtrl.Dcm_FalseAcessCount[SecCfgIndex] = Dcm_SecCtrl.Dcm_FalseAcessCount[SecCfgIndex] - 1;
...
static FUNC(void,DCM_CODE)Dcm_BootloaderResponse(void)
{
   ...
   if(DCM_WARM_START == Dcm_GetProgConditions(&ProgConditions))
   {
      ...
      if (ProgConditions.ApplUpdated == TRUE)
      {
        ...
        if(BootWarmStartCheck == TRUE)
        {
            BootWarmStartCheck = FALSE;
            if(BootResponseRequired == TRUE)
            {
                Dcm_GetUDSPhyPduID(&pduRxId);
                PduInfo->SduLength = 0;
                (void)Dcm_StartOfReception(pduRxId,PduInfo,2,&availabeBufferLength);

...
(STD_ON == NVM_BSWM_SINGLEBLOCK_JOBSTATUS_INFORMATION))
#include "BswM.h"
#include "BswM_NvM.h"
#endif
...
static FUNC(void, NVM_CODE)NvM_RepireDataSave(
        P2CONST(uint8, TYPEDEF, NVM_APPL_DATA)DataSourceAddress,uint16 Length);
static FUNC(void, NVM_CODE)NvM_ReadStaticBlockID(
        P2CONST(uint8, AUTOMATIC, NVM_APPL_DATA)Src ,NvM_BlockCRCType crctype);
...
static FUNC(void, NVM_CODE)NvM_WriteStaticBlockID(
        P2VAR(uint8, AUTOMATIC, NVM_APPL_DATA)Dest ,NvM_BlockCRCType crctype);
static FUNC(void, NVM_CODE)NvM_ReadErrorHandle(
        NvM_CrcStaticIdStatusType CRCResult,NvM_CrcStaticIdStatusType StaticIDResult);
static FUNC(void, NVM_CODE)NvM_MultiJobResultFeedBack(
        NvM_ServiceIdType ServiceId,NvM_RequestResultType Result);
static FUNC(void, NVM_CODE)NvM_SingleJobResultFeedBack(
        NvM_BlockIdType BlockId,NvM_RequestResultType Result);
...
static FUNC(Std_ReturnType, NVM_CODE) NvM_MemCpy
(P2CONST(uint8, AUTOMATIC, NVM_VAR) SrcAddrPtr,
 P2VAR(uint8, AUTOMATIC, NVM_VAR) DstAddrPtr);
...
#if(STD_ON==NVM_JOB_PRIORITIZATION)
static FUNC(Std_ReturnType, NVM_CODE) NvM_Get16Bits1stSettedBit
(uint16 Number, P2VAR(uint8, AUTOMATIC, NVM_VAR) SettedBitPtr);

static FUNC(Std_ReturnType, NVM_CODE) NvM_GetHighestPriJob
(P2VAR(uint8, AUTOMATIC, NVM_VAR) JobQueueIndexPtr);

static FUNC(Std_ReturnType, NVM_CODE) NvM_GetQueueSpace
(NvM_JobTypeType JobType, P2VAR(uint8, AUTOMATIC, NVM_VAR) JobQueueIndexPtr);

static FUNC(void, NVM_CODE) NvM_RelQueueSpace
(NvM_JobTypeType JobType, uint8 JobQueueIndex);
#endif
#endif
...
#include "NvM_MemMap.h"
VAR(NvM_ModuleType, NVM_VAR_POWER_ON_INIT) NvM_Module =
{
    STD_OFF,
...
VAR(NvM_MultiJobType, NVM_VAR_POWER_ON_INIT) NvM_MultiJob =
{
    STD_OFF,
#if(NVM_API_CONFIG_CLASS_1!=NVM_API_CONFIG_CLASS)
    NVM_NONE_SERV_ID,
...
VAR(NvM_AtomJobType, NVM_VAR_POWER_ON_INIT) NvM_AtomJob =
{
    0,
    {
...
VAR(NvM_CurRunningType, NVM_VAR_POWER_ON_INIT) NvM_CurRunning =
{
    0,
    0,
    0,
...
#if(NVM_API_CONFIG_CLASS_1!=NVM_API_CONFIG_CLASS)
#if(STD_ON==NVM_JOB_PRIORITIZATION)
VAR(NvM_RoundRobinQueueManageType, NVM_VAR_POWER_ON_INIT) NvM_ImmedQueueManage =
{0x00u, 0x00u, 0x00u};
#else
VAR(NvM_RoundRobinQueueManageType, NVM_VAR_POWER_ON_INIT) NvM_StandQueueManage =
{0x00u, 0x00u, 0x00u};
#endif
#endif
...
static VAR(uint8, NVM_VAR_NOINIT)
        NVM_TemporaryRAMForRepaire[NVM_MAX_LENGTH_REDUNDANT_BLOCK];
#define NVM_STOP_SEC_VAR_CLEARED_8
#include "NvM_MemMap.h"
...
static void NvM_MemIfReqSubDeal(NvM_MemIfReqType NvM_MemIfRequest)
{
    uint16 l_BlockNum;
    uint16 l_Length;
    uint8 STATICBLOCKIDCHECK;
    NvM_BlockIdType CurBlockId = 0;

    l_BlockNum = (uint16)(NvM_CurRunning.BaseNumber<<NVM_DATASET_SELECTION_BITS)
            + (uint16)NvM_CurRunning.Index;
    ...
    case NVM_MEMIF_REQ_READ:
        ...
        STATICBLOCKIDCHECK = NvM_GetWordBitState(
                NvM_BlockDescriptor[CurBlockId].FlagGroup,
                (uint8)NVM_BLOCK_DESC_STATICBLOCKIDCHECK);
        if((uint8)STD_ON == STATICBLOCKIDCHECK)
        {
            l_Length = l_Length + 2U;
        }
        if ((Std_ReturnType)E_OK == MemIf_Read(NvM_CurRunning.DeviceId,
                l_BlockNum, 0, NvM_NvDataBuffer, l_Length))
        {
    ...
    case NVM_MEMIF_REQ_WRITE:
        if ((Std_ReturnType)E_OK == MemIf_Write(NvM_CurRunning.DeviceId,
                l_BlockNum, NvM_NvDataBuffer))
        {
    ...
    case NVM_MEMIF_REQ_ERASE:
        if ((Std_ReturnType)E_OK == MemIf_EraseImmediateBlock(
                NvM_CurRunning.DeviceId, l_BlockNum))
        {
            NvM_Module.MemIfJobState = NVM_MEMIF_JOB_ASYNC_PENDING;
        }
        else
    ...
    case NVM_MEMIF_REQ_INVALID:
        if ((Std_ReturnType)E_OK == MemIf_InvalidateBlock(
                NvM_CurRunning.DeviceId, l_BlockNum))
        {
            NvM_Module.MemIfJobState = NVM_MEMIF_JOB_ASYNC_PENDING;
        }
...
static void NvM_MemIfReq(NvM_MemIfReqType NvM_MemIfRequest)
{
    if (NVM_MEMIF_JOB_ASYNC_READY == NvM_Module.MemIfJobState)
    {
        ...
        else if (MEMIF_UNINIT == MemIf_GetStatus(NvM_CurRunning.DeviceId))
        {
            NvM_JobOverSetFlag(NVM_REQ_NOT_OK, NVM_CRC_REMAIN);
        }
        else
...
#else
void NvM_MemIfReq(NvM_MemIfReqType NvM_MemIfRequest)
{
    if (NVM_MEMIF_JOB_ASYNC_READY == NvM_Module.MemIfJobState)
    {
        if (MEMIF_IDLE == MemIf_GetStatus(NvM_CurRunning.DeviceId))
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE) NvM_SetWordBitState
(P2VAR(uint16, AUTOMATIC, NVM_VAR) WordPtr, uint8 SetBit, uint8 OnOff)
{
 ...
 P2VAR(uint8, AUTOMATIC, NVM_VAR) DstAddrPtr)
{
    ...
    {
        NOT = NvM_GetWordBitState(NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                NVM_ADMIN_RAMMIRROR_OR_NOT);
        if ((uint8)STD_ON == NOT)
        {
...
#endif
            {
                if ((Std_ReturnType)E_OK ==
                        (*NvM_CurRunning.NvM_WriteRamBlockToNvm)((void*)SrcAddrPtr))
                {
                            ...
                            &&(NVM_VALIDATE_ALL_SERV_ID != NvM_CurRunning.ServiceId))
                        {
                            ...
                            NvM_SetWordBitState(
                                    ...
                                    NVM_ADMIN_RAM_VALID, STD_ON);
                            NvM_SetWordBitState(
                                    &NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                                    NVM_ADMIN_RAM_CHANGED, STD_ON);
            ...
            /***********************************/
            else if((NVM_READ_BLOCK_SERV_ID == NvM_CurRunning.ServiceId)
            ...
            ||(NvM_Module.JobStep == NVM_JOB_STEP_READ_ROM))
            {
                    ...
                    if(NVM_JOB_STEP_READ_ROM != NvM_Module.JobStep)
                    {
                        NvM_SetWordBitState(
                                &NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                                NVM_ADMIN_RAM_VALID, STD_OFF);
                        NvM_SetWordBitState(
                                &NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                                NVM_ADMIN_RAM_CHANGED, STD_OFF);
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE) NvM_AtomJobReq(NvM_AtomJobEType AtomJobReqId)
{
    NvM_AtomJob.Count++;
...
{
    uint8 SYNCMECHANISM;
    NvM_CurRunning.ManagementType = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            NvmBlockManagementType;
#if(NVM_API_CONFIG_CLASS_1!=NVM_API_CONFIG_CLASS)
    if (NVM_BLOCK_DATASET != NvM_CurRunning.ManagementType)
    {
        NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].CurrentIndex = 0;
    }
#else
    NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].CurrentIndex = 0;
#endif
    ...
    NvM_CurRunning.Index = NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].CurrentIndex;
    NvM_CurRunning.StaticId = 0xFFFF;
    NvM_CurRunning.AdminFlagGroup = NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup;
    NvM_CurRunning.Crc = NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].Crc;
    NvM_CurRunning.ESingleReqResult = NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult;
    NvM_CurRunning.WriteTimesCounter = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            NvMMaxNumOfWriteRetries + 1U;     /*req 659*/
    NvM_CurRunning.ReadRetryCounter = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            NvMMaxNumOfReadRetries;     /*req 533*/
    NvM_CurRunning.VerificationSize = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            NvMWriteVerificationDataSize;
    NvM_CurRunning.VerificationCounter = 0;
    NvM_CurRunning.DeviceId = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmNvramDeviceId;
    NvM_CurRunning.NvNum = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmNvBlockNum;
    NvM_CurRunning.RomNum = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRomBlockNum;
    NvM_CurRunning.CRCType = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmBlockCRCType;
    NvM_CurRunning.BaseNumber = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            NvmNvBlockBaseNumber;
    NvM_CurRunning.Length = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmNvBlockLength;
    /***********************************/
#if(NVM_API_CONFIG_CLASS_1!=NVM_API_CONFIG_CLASS)
    if (NVM_BLOCK_DATASET != NvM_CurRunning.ManagementType)
    {
        NvM_CurRunning.RomAddr = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                NvmRomBlockDataAddress;
    }
    else if (NvM_CurRunning.NvNum <= NvM_CurRunning.Index)
    {
        NvM_CurRunning.RomAddr = &NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                NvmRomBlockDataAddress[((NvM_CurRunning.Index - NvM_CurRunning.NvNum) *
                        (uint8)NvM_CurRunning.Length)];
    }
    else
    {
    ...
    NvM_CurRunning.RomAddr = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            NvmRomBlockDataAddress;
#endif
    SYNCMECHANISM = NvM_GetWordBitState(NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].FlagGroup,
            (uint8)NVM_BLOCK_DESC_SYNCMECHANISM);
    if ((uint8)STD_ON == SYNCMECHANISM)
    {
        NvM_CurRunning.NvM_WriteRamBlockToNvm = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                NvM_WriteRamBlockToNvm;
        NvM_CurRunning.NvM_ReadRamBlockFromNvm = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                NvM_ReadRamBlockFromNvm;
    }
    /***********************************/

#if (NVM_API_CONFIG_CLASS_1 != NVM_API_CONFIG_CLASS)
    NvM_CurRunning.InitCallback = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            NvmInitBlockCallback;
#endif
#if (STD_OFF == NVM_POLLING_MODE)
    NvM_CurRunning.SingleCallback = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
...
static FUNC(Std_ReturnType, NVM_CODE) NvM_MultiJobDispatch(void)
{
...
#if ((STD_OFF == NVM_POLLING_MODE)&&(STD_OFF == NVM_BSWM_MULTIBLOCK_JOBSTATUS_INFORMATION))
            if (NULL_PTR != NvmMultiBlockCallback)
            {
                /* call the callback function to notify the up
                 * layer after job processing is over */
                (*NvmMultiBlockCallback)((uint8)(NvM_CurRunning.ServiceId),
                        NvM_MultiJob.ReqResult);
            }
#endif
        }
        else
    ...
    else
    {
		...
		(NVM_SET_RAM_BLOCK_STATUS_SERV_ID == NvM_CurRunning.ServiceId) || 
		((NVM_BLOCK_NUM_ALL < NvM_CurRunning.BlockId) &&
                        ((uint8)STD_OFF == NvM_MultiJob.ID1WriteMark)))
        {
#if (STD_ON == NVM_DRV_MODE_SWITCH)
            /*switch the low layer process mode to slow mode after job over*/
            MemIf_SetMode(MEMIF_MODE_SLOW);
            ...
            if (NULL_PTR != NvmMultiBlockCallback)
            {
                ...
                (*NvmMultiBlockCallback)((uint8)(NvM_CurRunning.ServiceId),
                 ...
                 && ((uint8)STD_ON == NvM_MultiJob.ID1WriteMark))
        {
 ...
 * CallByAPI           Up layer
 */
FUNC(Std_ReturnType, NVM_CODE) NvM_JobEnqueue
 ...
 P2VAR(uint8, AUTOMATIC, NVM_APPL_DATA) DestSrcPtr)
{
    ...
    SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA)
    if (NVM_SIZE_STANDARD_JOB_QUEUE > NvM_StandQueueManage.Count)
    {
        ...
        if ((NVM_READ_ALL_SERV_ID == ServiceId)
                ||(NVM_WRITE_ALL_SERV_ID == ServiceId)
        ...
        else
        {
            ...
            NvM_SetWordBitState(&NvM_AdminBlock[BlockIdIndex].FlagGroup,
                    NVM_ADMIN_ENQUEUE, STD_ON);
    ...
    l_ReturnValue = E_OK;
#endif
    SchM_Exit_NvM(0, INTERRUPT_PROTECTION_AREA)
    return l_ReturnValue;
}
...
(NvM_JobTypeType JobType, P2VAR(uint8, AUTOMATIC, NVM_VAR) JobQueueIndexPtr)
{
    ...
    {
        l_ReturnValue = NvM_Get16Bits1stSettedBit(
                NvM_QueueSpaceTalbe[NVM_TABLE_SIZE_JOB_QUEUE - 1U],&l_JobQueueGroup);
        ...
        {
            l_ReturnValue = NvM_Get16Bits1stSettedBit(
                    NvM_QueueSpaceTalbe[l_JobQueueGroup], &l_JobQueueRdyBit);
            l_JobQueueIndex = (uint8)(l_JobQueueGroup << 4u)+ l_JobQueueRdyBit;
            if (((Std_ReturnType)E_NOT_OK == l_ReturnValue) ||
                    (NVM_SIZE_STANDARD_JOB_QUEUE <= l_JobQueueIndex))
            {
            ...
            /***********************************/
            else if ((Std_ReturnType)E_OK == l_ReturnValue)
            {
                SchM_Enter_NvM(0, INTERRUPT_PROTECTION_AREA)
                *JobQueueIndexPtr = l_JobQueueIndex;
                /* clear the job queue space table, mark it as not available */

                NvM_QueueSpaceTalbe[l_JobQueueGroup] &=
                        (uint16)(~(uint16)((uint16)0x01u << l_JobQueueRdyBit));
...
NvM_RelQueueSpace(NvM_JobTypeType JobType, uint8 JobQueueIndex)
{
    ...
    /***********************************/
    if ((NVM_JOB_TYPE_SINGLE_IMMED == JobType) ||
            (NVM_JOB_TYPE_MULTI_GAP_SINGLE_IMMED == JobType))
    {
    ...
    else if (NVM_JOB_TYPE_SINGLE_NORMAL == JobType)
    {
        ...
        NvM_QueueSpaceTalbe[NVM_TABLE_SIZE_JOB_QUEUE - 1U] |=
                (uint16)((uint16)0x01U << l_QueueHighBits);
...
NvM_GetHighestPriJob(P2VAR(uint8, AUTOMATIC, NVM_VAR) JobQueueIndexPtr)
{
   ...
   uint8 l_SamePriHeadIndex;
    ...
    l_ReturnValue = NvM_Get16Bits1stSettedBit(
            NvM_PriorityTable[l_CurActPriTable][NVM_TABLE_SIZE_PRIORITY - 1U], &l_PriGroup);
    if ((Std_ReturnType)E_OK == l_ReturnValue)
    {
        ...
        {
            l_ReturnValue = NvM_Get16Bits1stSettedBit(
                    NvM_PriorityTable[l_CurActPriTable][l_PriGroup], &l_PriRdyBit);
            /***********************************/
            if ((Std_ReturnType)E_NOT_OK == l_ReturnValue)
            {
                /* Flag error happened, Correct the mistakes */
...
static FUNC(Std_ReturnType, NVM_CODE) NvM_MultiJobDispatchWrite(void)
{
    ...
    if ((2U == NvM_MultiJob.BlockId)
            || ((NVM_BLOCK_NUM_ALL < NvM_MultiJob.BlockId) &&
                    ((uint8)STD_OFF == NvM_MultiJob.ID1WriteMark)))
    {
#if (STD_ON == NVM_DRV_MODE_SWITCH)
...
#endif
        if (0U !=
            NvM_PriorityTable[1U - NvM_Module.CurActPriTable][NVM_TABLE_SIZE_PRIORITY - 1U])
        {
    ...
    else if((uint8)STD_ON == NvM_MultiJob.ID1WriteMark)
    {
        ...
        *((NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRamBlockDataAddress) + 1U) =
                (uint8)(((uint16)(NVM_COMPILED_CONFIG_ID))>>8U);
        NvM_CurRunning.RamAddr = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
...
#if ((STD_OFF == NVM_POLLING_MODE)&&(STD_OFF == NVM_BSWM_MULTIBLOCK_JOBSTATUS_INFORMATION))
            if ((NULL_PTR != NvmMultiBlockCallback) &&
                    (STD_OFF == NVM_BSWM_MULTIBLOCK_JOBSTATUS_INFORMATION))   /* req NVM745 */
            {
                /* if the callback func is registered and the switch is open, call it */
                (*NvmMultiBlockCallback)((uint8)(NvM_CurRunning.ServiceId),
                        NvM_MultiJob.ReqResult);
 ...
 P2VAR(uint8, AUTOMATIC, NVM_APPL_DATA) DestSrcPtr)
{
            ...
            ||(NVM_VALIDATE_ALL_SERV_ID == ServiceId))
    {
        ...
        if (NVM_READ_ALL_SERV_ID == ServiceId)
        {
            for (l_BlockIdLoop = 0; l_BlockIdLoop < NVM_BLOCK_NUM_ALL; l_BlockIdLoop++)
            {
    ...
    else if ((NVM_BLOCK_NUM_ALL >= BlockId) && (0U < BlockId))
    {
        ...
        if ((uint8)STD_OFF == ENQUEUE)
        {
            ...
            /*.Immedi write*/
            if ((0u == l_JobPri) && ((NVM_WRITE_BLOCK_SERV_ID == ServiceId)||
                    (NVM_WRITE_PRAM_BLOCK_SERV_ID == ServiceId)))
            {
            ...
            else
            {
                if (NVM_JOB_TYPE_SINGLE_IMMED == l_JobType)
                {
                    ...
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId - 1U].FlagGroup,
                            NVM_ADMIN_ENQUEUE, STD_ON);
                    NvM_SingleJobResultFeedBack(BlockId,NVM_REQ_PENDING);
                }
                else
                {
                    ...
                    if (0U != (NvM_PriorityTable[l_CurActPriTable][l_PriHighBits] &
                            (uint16)((uint16)0x01u << l_PriLowBits)))
                    {
                        /* add the new job to the queue's tail */
                        NvM_StandQueue[NvM_PriTable2Queue[l_CurActPriTable][l_JobPri].TailIndex].
                        NextIndex = l_AddJobQueueIndex;
                    }
                    else
                    {
...
static FUNC(void, NVM_CODE) NvM_GetRamAddress(uint8 QueueIndex)
{
    ...
    if((NvM_Module.CurrentJobType == NVM_JOB_TYPE_MULTI_GAP_SINGLE_IMMED) ||
            (NvM_Module.CurrentJobType == NVM_JOB_TYPE_SINGLE_IMMED))
    {
        SYNCMECHANISM = NvM_GetWordBitState(NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                FlagGroup,(uint8)NVM_BLOCK_DESC_SYNCMECHANISM);
        if (NULL_PTR != NvM_ImmedQueue[QueueIndex].DestSrcPtr)
        {
            NvM_CurRunning.RamAddr = NvM_ImmedQueue[QueueIndex].DestSrcPtr;
            NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].
                    FlagGroup,NVM_ADMIN_RAM_VALID_CHANGE_STATUS_USED,STD_OFF);
            NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].
                    FlagGroup,NVM_ADMIN_RAMMIRROR_OR_NOT,STD_OFF);
        }
...
#endif
            NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].
                    FlagGroup,NVM_ADMIN_RAM_VALID_CHANGE_STATUS_USED,STD_ON);
            NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].
                    FlagGroup,NVM_ADMIN_RAMMIRROR_OR_NOT,STD_ON);
        }
        else
        {
            NvM_CurRunning.RamAddr = NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                    NvmRamBlockDataAddress;
            NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                    NVM_ADMIN_RAM_VALID_CHANGE_STATUS_USED,STD_ON);
            NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                    NVM_ADMIN_RAMMIRROR_OR_NOT,STD_OFF);
        }
    }
...
#endif
    {
        REPAIR = NvM_GetWordBitState(NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                NVM_ADMIN_NV_REPAIR);
        if(((uint8)STD_ON == REPAIR) && (NVM_WRITE_ALL_SERV_ID == NvM_CurRunning.ServiceId))
        {
            NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                    NVM_ADMIN_RAM_VALID_CHANGE_STATUS_USED,STD_OFF);
...
#endif
                NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                        NVM_ADMIN_RAMMIRROR_OR_NOT,STD_ON);
            }
            else
            {
                NvM_CurRunning.RamAddr =
                        NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRamBlockDataAddress;
                NvM_SetWordBitState(&NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,
                        NVM_ADMIN_RAMMIRROR_OR_NOT,STD_OFF);
            }
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE) NvM_JobOverSetFlag
(NvM_RequestResultType SingleReqResult, NvM_CrcChangeType CrcChange)
{
    ...
    for(l_Curindex = 0; l_Curindex < totalnum;l_Curindex++)
    {
        ...
        l_BlockIdIndex++;
        if(l_BlockIdIndex > NVM_SIZE_STANDARD_JOB_QUEUE - 1u)
        {
            l_BlockIdIndex = 0;
        }
    }
    ...
    NvM_SetWordBitState(&NvM_CurRunning.AdminFlagGroup, NVM_ADMIN_ENQUEUE, STD_OFF);
     ...
     WRITEBLOCKONCE = NvM_GetWordBitState(NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            FlagGroup,(uint8)NVM_BLOCK_DESC_WRITEBLOCKONCE);
    ...
    {
        (*NvM_CurRunning.SingleCallback)((uint8)(NvM_CurRunning.ServiceId),
                NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult);
    }
#endif
...
static FUNC(void, NVM_CODE) NvM_MemIfReadOk(void)
{
    ...
    STATICBLOCKIDCHECK = NvM_GetWordBitState(NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
            FlagGroup,(uint8)NVM_BLOCK_DESC_STATICBLOCKIDCHECK);
    if((uint8)STD_ON == STATICBLOCKIDCHECK)
    {
    ...
    else if((uint8)STD_ON == STATICBLOCKIDCHECK)
    {
...
static FUNC(void, NVM_CODE) NvM_MemIfReadFailed(void)
{
    ...
    if (NVM_BLOCK_DATASET == NvM_CurRunning.ManagementType)
    {
        ...
        NvM_JobOverSetFlag(NVM_REQ_NOT_OK, NVM_CRC_REMAIN);  /* req NVM359  361*/
            ...
            || (NVM_JOB_STEP_READ_2ND_NV == NvM_Module.JobStep))
             && ((NULL_PTR !=
                     NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRomBlockDataAddress)
             ||(NULL_PTR !=
                     NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmInitBlockCallback)))
#else
...
#endif
    {
        ...
        NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult = NVM_REQ_INTEGRITY_FAILED;
#if (STD_ON == NVM_DEM_E_REQ_FAILED)
        ...
        NvM_AtomJobReq(NVM_ATOMJOB_READROM);
            ...
            && (NVM_JOB_STEP_READ_1ST_NV == NvM_Module.JobStep))
    {
...
static FUNC(void, NVM_CODE) NvM_MemIfInvalid(void)
{
...
#endif
    {
        ...
        else
        {
            NvM_UpdateValidandChangeStatus(STD_OFF,STD_OFF);
            NvM_JobOverSetFlag(
                    NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult, NVM_CRC_DELETE);
        }
    }
    else
    {
...
static FUNC(void, NVM_CODE) NvM_MemIfWriteOk(void)
{
    ...
    if ((NVM_BLOCK_REDUNDANT == NvM_CurRunning.ManagementType)
            && (NVM_JOB_STEP_WRITE_2ND_NV == NvM_Module.JobStep))
    {
    ...
    {
        WRITEVERIFICATION = NvM_GetWordBitState(NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                FlagGroup,(uint8)NVM_BLOCK_DESC_WRITEVERIFICATION);
...
static FUNC(void, NVM_CODE) NvM_MemIfWriteFailed(void)
{
    ...
    {
        NvM_CurRunning.WriteTimesCounter =
                NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvMMaxNumOfWriteRetries;
        NvM_Module.JobStep = NVM_JOB_STEP_WRITE_1ST_NV;
        NvM_Module.MemIfJobState = NVM_MEMIF_JOB_ASYNC_READY;
        NvM_CurRunning.Index =0;
        NVM_GetRepeatMirrorOperation();
    ...
    {
        if ((NVM_BLOCK_REDUNDANT == NvM_CurRunning.ManagementType) &&
                (NVM_REQ_OK == NvM_CurRunning.ESingleReqResult))
        {
...
/*
 * Brief               the low layer process the erase job success
 * ServiceId           None
 * Sync/Async          synchronous
 * Reentrancy          Reentrant
 * Param-Name[in]      None
...
/*
 * Brief               The low layer process the erase job failed
 * ServiceId           None
 * Sync/Async          synchronous
 * Reentrancy          Reentrant
 * Param-Name[in]      None
...
/*
 * Brief               The low layer process the invalid job success
 * ServiceId           None
 * Sync/Async          synchronous
 * Reentrancy          Reentrant
 * Param-Name[in]      None
...
/*
 * Brief               The low layer process the invalid job failed
 * ServiceId           None
 * Sync/Async          synchronous
 * Reentrancy          Reentrant
 * Param-Name[in]      None
...
FUNC(void, NVM_CODE) NvM_InterReadRom(void)
{
    ...
    if (NVM_JOB_STEP_CALC_CRC == NvM_Module.JobStep)
    {
        ...
        if((NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult ==
    ...
    else
    {
        ...
        {
            if((Std_ReturnType)E_OK == (*NvM_CurRunning.InitCallback)())
            {
                NvM_UpdateValidandChangeStatus(STD_ON,STD_ON);
                if((NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult ==
                       NVM_REQ_INTEGRITY_FAILED)
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE) NvM_InterCalcCrc(void)
{
    uint16 l_CrcLength;
    ...
    if (0U == NvM_CurRunning.CrcFlag)
    {
        ...
        if (NVM_CRC16 == NvM_CurRunning.CRCType)
        {
            NvM_CurRunning.Crc = Crc_CalculateCRC16(NvM_CurRunning.CrcAddr, l_CrcLength,
                    (uint16)NvM_CurRunning.Crc,TRUE);
        }
        else if (NVM_CRC32 == NvM_CurRunning.CRCType)
        {
            NvM_CurRunning.Crc = Crc_CalculateCRC32(NvM_CurRunning.CrcAddr, l_CrcLength,
                    NvM_CurRunning.Crc,TRUE);
        }
        else
        {
            NvM_CurRunning.Crc = Crc_CalculateCRC8(NvM_CurRunning.CrcAddr, l_CrcLength,
                    (uint8)NvM_CurRunning.Crc,TRUE);
        }
    ...
    else
    {
        ...
        {
            NvM_CurRunning.Crc = Crc_CalculateCRC16(NvM_CurRunning.CrcAddr, l_CrcLength,
                    (uint16)NvM_CurRunning.Crc,FALSE);
        }
        else if (NVM_CRC32 == NvM_CurRunning.CRCType)
        {
            NvM_CurRunning.Crc = Crc_CalculateCRC32(NvM_CurRunning.CrcAddr, l_CrcLength,
                    NvM_CurRunning.Crc,FALSE);
        }
        else
        {
            NvM_CurRunning.Crc = Crc_CalculateCRC8(NvM_CurRunning.CrcAddr, l_CrcLength,
                    (uint8)NvM_CurRunning.Crc,FALSE);
        }
        NvM_CurRunning.CrcLength -= l_CrcLength;
        NvM_CurRunning.CrcAddr += l_CrcLength;
    }
...
static FUNC(void, NVM_CODE) NvM_InterReadBlockSubDeal(void)
{
    ...
    else if((NVM_JOB_STEP_CALC_CRC_READ_1ST_NV == NvM_Module.JobStep)
      || (NVM_JOB_STEP_CALC_CRC_READ_2ND_NV == NvM_Module.JobStep))
    {
...
FUNC(void, NVM_CODE) NvM_InterWriteBlock(void)
{
   ...
   if (NVM_JOB_STEP_PENDING == NvM_Module.JobStep)
   {
       if (NvM_CurRunning.RepeatMirrorCounter > 0U)
       {
           ...
           {
               NvM_SetWordBitState(&NvM_CurRunning.AdminFlagGroup,
                       NVM_ADMIN_WRITE_COPY_RESULT,STD_ON);
           }
           else
           {/*Repeat again*/}
        }
   ...
   RESULT = NvM_GetWordBitState(NvM_CurRunning.AdminFlagGroup, NVM_ADMIN_WRITE_COPY_RESULT);
    ...
    {
        NvM_SetWordBitState(&NvM_CurRunning.AdminFlagGroup,
                NVM_ADMIN_WRITE_COPY_RESULT,STD_OFF);
    ...
    {
        CALCRAMBLOCKCRC = NvM_GetWordBitState(NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                FlagGroup,(uint8)NVM_BLOCK_DESC_CALCRAMBLOCKCRC);
        CRCCOMPMECHANISM = NvM_GetWordBitState(NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                FlagGroup,(uint8)NVM_BLOCK_DESC_CRCCOMPMECHANISM);
        ...
        if(((uint8)STD_ON == CALCRAMBLOCKCRC) && ((uint8)STD_ON == CRCCOMPMECHANISM)
            && ((uint8)STD_OFF == REPAIR) && ((uint8)STD_ON == VALID))  /*req 851 853 854*/
        {
            if(NvM_CurRunning.Crc == NvM_CurRunning.TempCrc)
            {
              NvM_UpdateValidandChangeStatus(STD_ON,STD_OFF);
              NvM_JobOverSetFlag(NVM_REQ_BLOCK_SKIPPED, NVM_CRC_REMAIN);        /*req NVM854*/
              ProcessOn = FALSE;
            }
        }
        if (ProcessOn == TRUE)
        {
            if (NVM_CRC16 == NvM_CurRunning.CRCType)
            {
                NvM_NvDataBuffer[NvM_CurRunning.Length] =
                        (uint8)NvM_CurRunning.Crc;
                NvM_NvDataBuffer[NvM_CurRunning.Length+1U] =
                        (uint8)(NvM_CurRunning.Crc >> 8U);
            }
            else if (NVM_CRC32 == NvM_CurRunning.CRCType)
            {
                NvM_NvDataBuffer[NvM_CurRunning.Length]   =
                ...
                NvM_NvDataBuffer[NvM_CurRunning.Length+1U] =
                        (uint8)(NvM_CurRunning.Crc >> 8U);
                NvM_NvDataBuffer[NvM_CurRunning.Length+2U] =
                        (uint8)(NvM_CurRunning.Crc >> 16U);
                NvM_NvDataBuffer[NvM_CurRunning.Length+3U] =
                        (uint8)(NvM_CurRunning.Crc >> 24U);
            }
            else
            {
...
FUNC(void, NVM_CODE) NvM_InterSetRamStatus(void)
{
    ...
    else
    {
        if (NvM_CurRunning.RepeatMirrorCounter > 0U)
        {
            if ((Std_ReturnType)E_OK == NvM_MemCpy(NvM_CurRunning.RamAddr, NvM_CurRunning.CrcAddr))
            {
                NvM_Module.JobStep = NVM_JOB_STEP_CALC_CRC;
                NvM_AtomJobReq(NVM_ATOMJOB_CALCCRC);
            }
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE) NvM_InterRestoreBlockDefaults(void)
{
    if (NVM_JOB_STEP_PENDING == NvM_Module.JobStep)
    {
        if((NULL_PTR != NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRomBlockDataAddress)
...
#if(STD_ON==NVM_JOB_PRIORITIZATION)
/*
 * Brief               The module internal function to process erase NV block request
 * Since the GC mechanism, Block pre-Erase before write.
 *  No Fee supprot in EB. The Erase always Not ok.
 * ServiceId           None
 * Sync/Async          synchronous
 * Reentrancy          Reentrant
 * Param-Name[in]      None
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE) NvM_InterInvalidateNvBlock(void)
{
    MemIf_JobResultType l_ReturnValue;
    ...
    else if (NVM_READALL_FIRST_BLOCKID < NvM_CurRunning.BlockId)
    {
...
#endif
        {
            ...
            if ((NVM_RUNTIME_EXTENDED == NvM_Module.RuntimeType)
                    && ((uint8)STD_OFF == l_ResistentChangedSw))
            {
                NvM_SetWordBitState(&NvM_CurRunning.AdminFlagGroup, NVM_ADMIN_NV_WRPRO, STD_OFF);
                NvM_UpdateValidandChangeStatus(STD_OFF,STD_OFF);
...
FUNC(void, NVM_CODE) NvM_InterWriteAll(void)
{
    ...
    if ((uint8)STD_ON == NvM_MultiJob.CancelWriteAll)
    {
        ...
        NvM_MultiJob.Enqueue = STD_OFF;
#if(NVM_API_CONFIG_CLASS_1!=NVM_API_CONFIG_CLASS)
        NvM_MultiJobResultFeedBack(NvM_MultiJob.ServiceId,NVM_REQ_CANCELED);
        NvM_MultiJob.ServiceId = NVM_NONE_SERV_ID;
#else
        NvM_MultiJobResultFeedBack(NvM_CurRunning.ServiceId,NVM_REQ_CANCELED);
    ...
    else
    {
        ...
        /*req 474*/
        if (((uint8)STD_ON == l_NvRepair) || (((uint8)STD_ON == l_RamValid) &&
                ((uint8)STD_ON == l_RamChanged) && ((uint8)STD_OFF == l_NvProt)
        ...
        {
            USED = NvM_GetWordBitState(NvM_CurRunning.AdminFlagGroup,
                    NVM_ADMIN_RAM_VALID_CHANGE_STATUS_USED);
 ...
 * CallByAPI           Up layer
 */
FUNC(void, NVM_CODE)NvM_UpdateValidandChangeStatus(uint8 ValidStatus, uint8 ChangeStatus)
{
    uint8 USED = NvM_GetWordBitState(NvM_CurRunning.AdminFlagGroup,
            NVM_ADMIN_RAM_VALID_CHANGE_STATUS_USED);
    if ((uint8)STD_ON == USED)
    {
...
static FUNC(void, NVM_CODE)NvM_MemCpyStep(void)
{
    uint8 NOT = NvM_GetWordBitState(
            NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup,NVM_ADMIN_RAMMIRROR_OR_NOT);
    uint8 LOSS;
    if (((uint8)STD_ON == NOT) && (NvM_CurRunning.RepeatMirrorCounter > 0U))
    {
...
static FUNC(void, NVM_CODE)NvM_SuboneInterWriteVerification(void)
{
    ...
    for(l_Counter = 0;l_Counter<NvM_CurRunning.VerificationSize;l_Counter++)
    {
        ...
        {
            NvM_SetWordBitState(&NvM_CurRunning.AdminFlagGroup,
                    NVM_ADMIN_CRC_VALID,STD_OFF);
            l_TempStatus = E_NOT_OK;
            NvM_CurRunning.VerificationCounter = 0;
            if(NvM_CurRunning.WriteTimesCounter > 0U)
            {
                 NvM_SetWordBitState(&NvM_CurRunning.AdminFlagGroup,
                         NVM_ADMIN_WRITE_COPY_RESULT,STD_ON); /* req NVM529*/
                 NvM_Module.MemIfJobState = NVM_MEMIF_JOB_IDLE;
                 NvM_AtomJob.Count--;
            }
            else
            {
                 ...
                 NvM_JobOverSetFlag(NVM_REQ_NOT_OK, NVM_CRC_DELETE);
#if (STD_ON == NVM_DEM_E_REQ_FAILED)
                     Dem_ReportErrorStatus(NVM_E_REQ_FAILED,
                             DEM_EVENT_STATUS_FAILED);   /* req NVM659 805 684*/
#endif
#if (STD_ON == NVM_DEM_E_VERIFY_FAILED)
                     Dem_ReportErrorStatus(NVM_E_VERIFY_FAILED,
                             DEM_EVENT_STATUS_FAILED);  /* req NVM528 867*/
#endif
            }
            break;
        }
    ...
    {
        NvM_CurRunning.VerificationCounter =
        NvM_CurRunning.VerificationCounter + NvM_CurRunning.VerificationSize;
    }
}
 ...
 * CallByAPI           Up layer
 */
static FUNC(void, NVM_CODE)NvM_SubtwoInterWriteVerification(void)
{
    uint16 l_Counter;
    for(l_Counter = 0;l_Counter<(NvM_CurRunning.Length -
            NvM_CurRunning.VerificationCounter);l_Counter++)
    {
        if(NvM_CurRunning.RamAddr[NvM_CurRunning.VerificationCounter + l_Counter]
            != NvM_NvDataBuffer[l_Counter + NvM_CurRunning.VerificationCounter])
        {
...
FUNC(void, NVM_CODE)NvM_InterWriteVerification(void)
{
    ...
    if((NvM_Module.JobStep == NVM_JOB_STEP_READ_1ST_NV)
          ||(NvM_Module.JobStep == NVM_JOB_STEP_READ_2ND_NV))
    {
        NvM_MemIfReq(NVM_MEMIF_REQ_READ);
        if (NVM_MEMIF_JOB_ASYNC_PENDING == NvM_Module.MemIfJobState)  /**/
        {
           ...
           if(MEMIF_JOB_OK == l_ReturnValue)
           {
               ...
               {
                   if ((NvM_CurRunning.Length - NvM_CurRunning.VerificationCounter) >=
                           NvM_CurRunning.VerificationSize)
                   {
        ...
        P2CONST(uint8, AUTOMATIC, NVM_APPL_DATA)Src ,NvM_BlockCRCType crctype)
{
    if( NVM_CRC_NOT_USED == crctype)
    {
         NvM_CurRunning.StaticId = (uint16)((uint16)Src[NvM_CurRunning.Length] << 8U) +
                 (uint16)(Src[NvM_CurRunning.Length + 1U]);
    }
    else if( NVM_CRC8 == crctype)
    {
        NvM_CurRunning.StaticId = (uint16)((uint16)Src[NvM_CurRunning.Length + 1U] << 8U) +
                (uint16)(Src[NvM_CurRunning.Length + 2U]);
    }
    else if( NVM_CRC16 == crctype)
    {
        NvM_CurRunning.StaticId = (uint16)((uint16)Src[NvM_CurRunning.Length + 2U] << 8U) +
                (uint16)(Src[NvM_CurRunning.Length + 3U]);
    }
    else
    {
        NvM_CurRunning.StaticId = (uint16)((uint16)Src[NvM_CurRunning.Length + 4U] << 8U) +
                (uint16)(Src[NvM_CurRunning.Length + 5U]);
    }
}
 ...
 * CallByAPI           Up layer
 */
static FUNC(void, NVM_CODE)NvM_ReadErrorHandleRedundant(
        NvM_CrcStaticIdStatusType CRCResult,NvM_CrcStaticIdStatusType StaticIDResult)
{
    if((NVM_JOB_STEP_CALC_CRC_READ_1ST_NV == NvM_Module.JobStep)
        ||( NVM_JOB_STEP_CHECK == NvM_Module.JobStep))
    {
        ...
        {
            NvM_CurRunning.ReadRetryCounter =
                NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvMMaxNumOfReadRetries;
    ...
    else
    {
        if(NvM_CurRunning.ReadRetryCounter > 0U)
        {
            ...
            NvM_Module.MemIfJobState = NVM_MEMIF_JOB_ASYNC_READY;
                ...
                NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRomBlockDataAddress)
                 ||(NULL_PTR != NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                         NvmInitBlockCallback))
        {
            NvM_UpdateValidandChangeStatus(STD_OFF,STD_OFF);    /* req NVM317*/
            NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].SingleReqResult =
    ...
    ||(NVM_JOB_STEP_CALC_CRC_READ_2ND_NV == NvM_Module.JobStep))
    {
...
#endif
        {
            ...
            else
            {
                ...
                if((NULL_PTR != NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].
                     NvmRomBlockDataAddress) || (NULL_PTR !=
                      NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmInitBlockCallback))
                {
                    NVM_GetRepeatMirrorOperation();
                    NvM_Module.JobStep = NVM_JOB_STEP_READ_ROM;
        ...
        NvM_BlockIdType BlockId,NvM_RequestResultType Result)
{
    ...
    uint8 BSWBLOCKSTATUSINFO = NvM_GetWordBitState(NvM_BlockDescriptor[BlockId-1U].FlagGroup,
            (uint8)NVM_BLOCK_DESC_BSWBLOCKSTATUSINFO);
    if (((uint8)STD_ON == BSWBLOCKSTATUSINFO)
    && (NVM_READ_ALL_SERV_ID != NvM_AdminBlock[BlockId - 1U].ServiceID)
...
{
#if (STD_ON == NVM_BSWM_MULTIBLOCK_JOBSTATUS_INFORMATION)
        uint8 NotiServiceId = 0;
	switch(ServiceId)
	{
	case NVM_READ_ALL_SERV_ID:
		NotiServiceId = (uint8)NVM_READ_ALL;
		break;
	case NVM_WRITE_ALL_SERV_ID:
		NotiServiceId = (uint8)NVM_WRITE_ALL;
		break;
	case NVM_VALIDATE_ALL_SERV_ID:
		NotiServiceId = (uint8)NVM_VALIDATE_ALL;
		break;
	case NVM_CANCEL_WRITE_ALL_SERV_ID:
		NotiServiceId = (uint8)NVM_CANCEL_WRITE_ALL;
		break;
	default:
                /*idle*/
		break;
...
#if(NVM_API_CONFIG_CLASS_1!=NVM_API_CONFIG_CLASS)
/*
 * Brief               The module internal function to process
 * validate all block request
 * ServiceId           None
 * Sync/Async          synchronous
 * Reentrancy          Reentrant
 * Param-Name[in]      None
...
{
    uint8 REPAIR = NvM_GetWordBitState(
            NvM_AdminBlock[NvM_CurRunning.BlockId - 1U].FlagGroup, NVM_ADMIN_NV_REPAIR);
    ...
    else
    {
        ...
        SYNCMECHANISM = NvM_GetWordBitState(
                NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].FlagGroup,
                (uint8)NVM_BLOCK_DESC_SYNCMECHANISM);
        if (((uint8)STD_ON == AUTOVALIDATION) && ((uint8)STD_ON == VALID) && ((NULL_PTR !=
                  NvM_BlockDescriptor[NvM_CurRunning.BlockId - 1U].NvmRamBlockDataAddress)

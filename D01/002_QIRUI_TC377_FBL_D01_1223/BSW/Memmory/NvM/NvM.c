...
#define NVM_C_AR_MAJOR_VERSION 4U
#define NVM_C_AR_MINOR_VERSION 2U
#define NVM_C_AR_PATCH_VERSION 2U
#define NVM_C_SW_MAJOR_VERSION 2U
#define NVM_C_SW_MINOR_VERSION 0U
#define NVM_C_SW_PATCH_VERSION 0U
...
/* req NVM089 */
#if ((NVM_C_AR_MAJOR_VERSION != NVM_H_AR_MAJOR_VERSION)\
   ||(NVM_C_AR_MINOR_VERSION != NVM_H_AR_MINOR_VERSION)\
   ||(NVM_C_AR_PATCH_VERSION != NVM_H_AR_PATCH_VERSION)\
   ||(NVM_C_SW_MAJOR_VERSION != NVM_H_SW_MAJOR_VERSION)\
   ||(NVM_C_SW_MINOR_VERSION != NVM_H_SW_MINOR_VERSION)\
   ||(NVM_C_SW_PATCH_VERSION != NVM_H_SW_PATCH_VERSION)\
    /*cfg check*/\
   ||(NVM_C_AR_MAJOR_VERSION != NVM_CFG_H_AR_MAJOR_VERSION)\
   ||(NVM_C_AR_MINOR_VERSION != NVM_CFG_H_AR_MINOR_VERSION)\
   ||(NVM_C_AR_PATCH_VERSION != NVM_CFG_H_AR_PATCH_VERSION))

#error " NvM.c version mismatching with NvM.h"
...
FUNC(void, NVM_CODE) NvM_Init(const NvM_ConfigType* ConfigPtr)
{
    ...
    for (l_BlockIdLoop = 0; l_BlockIdLoop <= l_LoopCount; l_BlockIdLoop++)
    {
        ...
        NvM_QueueSpaceTalbe[NVM_TABLE_SIZE_JOB_QUEUE - 1U] |=
                (uint16)((uint16)0x0001u << l_BlockIdLoop);
    ...
    for (l_BlockIdLoop = 0; l_BlockIdLoop < NVM_BLOCK_NUM_ALL; l_BlockIdLoop++)
    {
        ...
        WRITEPROT = NvM_GetWordBitState(NvM_BlockDescriptor[l_BlockIdLoop].FlagGroup,
                (uint8)NVM_BLOCK_DESC_WRITEPROT);
        if ((uint8)STD_ON == WRITEPROT)
        {
            NvM_SetWordBitState(&NvM_AdminBlock[l_BlockIdLoop].FlagGroup,
                    NVM_ADMIN_NV_WRPRO, STD_ON);
        }
        else
        {
            NvM_SetWordBitState(&NvM_AdminBlock[l_BlockIdLoop].FlagGroup,
                    NVM_ADMIN_NV_WRPRO, STD_OFF);
        }
    }
...
(NvM_BlockIdType BlockId, boolean ProtectionEnabled) /* req NVM450 */
{
...
#endif
    {
        ...
        WRITEBLOCKONCE = NvM_GetWordBitState(NvM_BlockDescriptor[BlockId - 1U].FlagGroup,
                (uint8)NVM_BLOCK_DESC_WRITEBLOCKONCE);
        LOCKED = NvM_GetWordBitState(NvM_AdminBlock[BlockId - 1U].FlagGroup,
                NVM_ADMIN_BLOCK_LOCKED);
        ...
        {
            if ((boolean)STD_ON == ProtectionEnabled)
            {
                NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                        NVM_ADMIN_NV_WRPRO, STD_ON); /* req NVM016*/
            }
            else
            {
                NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
...
NvM_InvalidateNvBlock(NvM_BlockIdType BlockId)
{
...
#endif
    {
        ...
        WRITEBLOCKONCE = NvM_GetWordBitState(
                NvM_BlockDescriptor[BlockId - 1U].FlagGroup,(uint8)NVM_BLOCK_DESC_WRITEBLOCKONCE);
        if ((uint8)STD_ON == ENQUEUE)
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
            /* req NVM639 */
        ...
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
            NVM_DET_REPORTERROR((uint8)NVM_INVALIDATE_NV_BLOCK_SERV_ID, NVM_E_WRITE_PROTECTED);
#endif
        }
...
FUNC(Std_ReturnType, NVM_CODE) NvM_EraseNvBlock(NvM_BlockIdType BlockId)
{
...
#endif
    {
        ...
        else if ((STD_OFF == NVM_JOB_PRIORITIZATION)
        ...
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
            NVM_DET_REPORTERROR((uint8)NVM_ERASE_NV_BLOCK_SERV_ID, NVM_E_WRITE_PROTECTED);
#endif
        }
        else if (((uint8)STD_ON == WRITEBLOCKONCE) &&
                (NvM_AdminBlock[BlockId-1U].ReadReq == FALSE))
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
            /* req NVM952 */
...
(NvM_BlockIdType BlockId, NvM_RequestResultType* RequestResultPtr) /* req NVM451 */
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
...
(NvM_BlockIdType BlockId, boolean BlockChanged) /* req NVM453 */
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 WRPRO;
    uint8 ENQUEUE;
...
#endif
    {
        ...
        {
            WRPRO = NvM_GetWordBitState(NvM_AdminBlock[BlockId-1U].FlagGroup,
                    NVM_ADMIN_NV_WRPRO);
            SYNCMECHANISM = NvM_GetWordBitState(NvM_BlockDescriptor[BlockId-1U].FlagGroup,
            ...
            {
                if ((boolean)STD_ON == BlockChanged)
                {
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                            NVM_ADMIN_RAM_VALID, STD_ON); /* req NVM241 NVM406 344 */
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                    ...
                    REPAIR = NvM_GetWordBitState(NvM_AdminBlock[BlockId-1U].FlagGroup,
                            NVM_ADMIN_NV_REPAIR);
                    if ((uint8)STD_ON == CALCRAMBLOCKCRC)
                    {
                        /* req NVM121 : The Req is useless,
                         * no further clarification about the use of Crc*/
                        if ((Std_ReturnType)E_NOT_OK == NvM_JobEnqueue(
                                BlockId, NVM_SET_RAM_BLOCK_STATUS_SERV_ID, NULL_PTR))
                        {
                            /*Reserved*/
                        }
                    }
                ...
                {
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                            NVM_ADMIN_RAM_VALID, STD_OFF); /* req NVM405  345*/
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                            NVM_ADMIN_RAM_CHANGED, STD_OFF); /* req NVM405 */
                }
                l_ReturnValue = E_OK;
...
#endif
    {
        ...
        else
        {
            ...
            if (((NULL_PTR != NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress)
                || ((uint8)STD_ON == SYNCMECHANISM)))
            {
                if ((boolean)STD_ON == BlockLocked)
                {
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                            NVM_ADMIN_BLOCK_LOCKED, STD_ON);
                }
                else
                {
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                            NVM_ADMIN_BLOCK_LOCKED, STD_OFF);
                }
            }
        }
    }
...
(NvM_BlockIdType BlockId, uint8 DataIndex)
{
...
#endif
    {
        ...
        else if ((NvM_BlockDescriptor[BlockId-1U].NvmNvBlockNum +
                NvM_BlockDescriptor[BlockId-1U].NvmRomBlockNum) <= DataIndex)
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
...
(NvM_BlockIdType BlockId, P2VAR(uint8, AUTOMATIC, NVM_VAR) DataIndexPtr) /* req NVM449 */
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
...
#endif
    {
        if (NVM_BLOCK_DATASET == NvM_BlockDescriptor[BlockId-1U].NvmBlockManagementType)
        {
            *DataIndexPtr = NvM_AdminBlock[BlockId-1U].CurrentIndex; /* req NVM021 */
            if ((NvM_BlockDescriptor[BlockId-1U].NvmNvBlockNum +
                    NvM_BlockDescriptor[BlockId-1U].NvmRomBlockNum) > *DataIndexPtr)
            {
                l_ReturnValue = E_OK;
            }
        }
...
(NvM_BlockIdType BlockId, void* NvM_DstPtr) /* req NVM454 */
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 SYNCMECHANISM;
    uint8 ENQUEUE;
...
#endif
    {
        SYNCMECHANISM = NvM_GetWordBitState(NvM_BlockDescriptor[BlockId-1U].FlagGroup,
                (uint8)NVM_BLOCK_DESC_SYNCMECHANISM);
        ENQUEUE = NvM_GetWordBitState(NvM_AdminBlock[BlockId-1U].FlagGroup,NVM_ADMIN_ENQUEUE);
        if ((NULL_PTR == NvM_DstPtr) && (NULL_PTR == NvM_BlockDescriptor[BlockId-1U].
            NvmRamBlockDataAddress) && ((uint8)STD_OFF == SYNCMECHANISM))
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
        ...
        else
        {
            ...
            {
                if (((NvM_DstPtr == NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress)&&
                        (NULL_PTR != NvM_DstPtr)) || ((NULL_PTR == NvM_DstPtr) &&
                          (NULL_PTR != NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress)))
                {
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                            NVM_ADMIN_RAM_VALID, STD_OFF); /* req NVM198 */
                    NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
...
FUNC(Std_ReturnType, NVM_CODE) NvM_ReadPRAMBlock (NvM_BlockIdType BlockId)
{
...
#endif
    {
        SYNCMECHANISM = NvM_GetWordBitState(
                NvM_BlockDescriptor[BlockId-1U].FlagGroup,(uint8)NVM_BLOCK_DESC_SYNCMECHANISM);
        ENQUEUE = NvM_GetWordBitState(NvM_AdminBlock[BlockId-1U].FlagGroup,NVM_ADMIN_ENQUEUE);
        if (((NULL_PTR == NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress) &&
        ...
        else
        {
            ...
            if (((Std_ReturnType)E_OK == l_ReturnValue) && ((uint8)STD_OFF == SYNCMECHANISM))
            {
                /* req NVM198 */
...
(NvM_BlockIdType BlockId, void* NvM_SrcPtr)
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 SYNCMECHANISM;
    uint8 ENQUEUE;
...
#endif
    {
        ...
        else
        {
            ...
            {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
                NVM_DET_REPORTERROR((uint8)NVM_WRITE_BLOCK_SERV_ID, NVM_E_WRITE_PROTECTED);
#endif
            }
            else if((uint8)STD_ON == LOCKED)
            {
            ...
            {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
                NVM_DET_REPORTERROR((uint8)NVM_WRITE_BLOCK_SERV_ID,
                        NVM_E_WRITE_ONCE_STATUS_UNKNOWN);
#endif
            }
            else
            {
                l_ReturnValue = NvM_JobEnqueue(BlockId, NVM_WRITE_BLOCK_SERV_ID,
                        (uint8*)NvM_SrcPtr);
                if ((Std_ReturnType)E_OK == l_ReturnValue)
                {
#if(STD_ON==NVM_JOB_PRIORITIZATION)
                    ...
                    if (((NvM_SrcPtr == NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress)&&
                            (NULL_PTR != NvM_SrcPtr))
                       ||((NULL_PTR == NvM_SrcPtr) &&
                     (NULL_PTR != NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress)))
                    {
                        ...
                        NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup,
                                NVM_ADMIN_RAM_CHANGED, STD_ON);             /* req NVM303 */
                    }
                }
            }
        }
...
FUNC(Std_ReturnType, NVM_CODE) NvM_WritePRAMBlock(NvM_BlockIdType BlockId)
{
...
#endif
    {
        ...
        else
        {
            ...
            {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
                NVM_DET_REPORTERROR((uint8)NVM_WRITE_PRAM_BLOCK_SERV_ID, NVM_E_WRITE_PROTECTED);
#endif
            }
            else
            {
...
(NvM_BlockIdType BlockId, void* NvM_DestPtr)
{
    Std_ReturnType l_ReturnValue = E_NOT_OK;
    uint8 SYNCMECHANISM;
    uint8 ENQUEUE;
...
#endif
    {
        SYNCMECHANISM = NvM_GetWordBitState(
                NvM_BlockDescriptor[BlockId-1U].FlagGroup,(uint8)NVM_BLOCK_DESC_SYNCMECHANISM);
        ENQUEUE = NvM_GetWordBitState(NvM_AdminBlock[BlockId-1U].FlagGroup,NVM_ADMIN_ENQUEUE);
        if (((NULL_PTR == NvM_DestPtr) && ((NULL_PTR == NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress)
            && ((uint8)STD_OFF == SYNCMECHANISM))))
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
        ...
        else if ((NULL_PTR == NvM_BlockDescriptor[BlockId-1U].NvmRomBlockDataAddress)
            && (NULL_PTR == NvM_BlockDescriptor[BlockId-1U].NvmInitBlockCallback))
        {
#if (STD_ON == NVM_DEV_ERROR_DETECT)
            NVM_DET_REPORTERROR((uint8)NVM_RESTORE_BLOCK_DEFAULTS_SERV_ID, NVM_E_BLOCK_CONFIG);
            NVM_DET_REPORTERROR((uint8)NVM_RESTORE_BLOCK_DEFAULTS_SERV_ID,
        ...
        else
        {
                      ...
                      || (NULL_PTR != NvM_BlockDescriptor[BlockId-1U].NvmInitBlockCallback)))
            {
                ...
                if ((Std_ReturnType)E_OK == l_ReturnValue)
                {
                    if (((NvM_DestPtr == NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress)
                            && (NULL_PTR != NvM_DestPtr)) || ((NULL_PTR == NvM_DestPtr) &&
                     ((NULL_PTR != NvM_BlockDescriptor[BlockId-1U].NvmRamBlockDataAddress) ||
                             ((uint8)STD_ON == SYNCMECHANISM))))
                    {
                        NvM_SetWordBitState(&NvM_AdminBlock[BlockId-1U].FlagGroup, NVM_ADMIN_RAM_VALID, STD_OFF);
...
FUNC(Std_ReturnType, NVM_CODE) NvM_RestorePRAMBlockDefaults(NvM_BlockIdType BlockId)
{
    ...
    {
        NVM_DET_REPORTERROR((uint8)NVM_RESTORE_PRAM_BLOCK_DEFAULTS_SERV_ID,
...
#if (STD_ON == NVM_DEV_ERROR_DETECT)
            /*req 886*/
            NVM_DET_REPORTERROR((uint8)NVM_RESTORE_PRAM_BLOCK_DEFAULTS_SERV_ID,NVM_E_BLOCK_CONFIG);
...
static FUNC(Std_ReturnType, NVM_CODE) NvM_CancelJobInQueue(NvM_BlockIdType BlockId)
{
    ...
    volatile NvM_JobTypeType l_JobType = NVM_JOB_TYPE_NONE;
    volatile uint8 l_JobPri = 0xFFu;
    ...
    if ((l_JobType == NVM_JOB_TYPE_SINGLE_IMMED) && ((Std_ReturnType)E_OK == Immedret))
    {
   ...
   else if((l_JobType == NVM_JOB_TYPE_SINGLE_NORMAL) && ((Std_ReturnType)E_OK == Normalret))
#else
     Normalret = NvM_NormalQueueSort(BlockId);
...
static FUNC(Std_ReturnType, NVM_CODE) NvM_NormalQueueSort(NvM_BlockIdType BlockId)
{
    uint8 l_Preindex = 0;
    uint8 l_Curindex = 0;
    uint8 l_BlockIdIndex = 0;
    uint8 l_QueueCounter = 0;
    Std_ReturnType ret = E_OK;
    uint8 l_BlockFind = FALSE;
    ...
    else
    {
        ...
        if(((0 != NvM_StandQueueManage.TailIndex)
           && (l_BlockIdIndex == (NvM_StandQueueManage.TailIndex - 1)))
           || ((0 == NvM_StandQueueManage.TailIndex)
           && (l_BlockIdIndex == (NVM_SIZE_STANDARD_JOB_QUEUE - 1))))
         {
        ...
        /*This block takes the middle or fist position of the NvM_ImmedQueueManage */
         else
         {
    ...
    else
    {
        ...
        if(l_BlockIdIndex == NvM_PriTable2Queue[l_CurActPriTable][l_JobPri].HeadIndex)
        {
            ...
            if(NvM_StandQueue[l_BlockIdIndex].NextIndex == l_BlockIdIndex)
            {
                ...
                NvM_PriorityTable[l_CurActPriTable][(l_JobPri & 0xF0U)] &=
                        (uint16)(~(uint16)((uint16)0x01u << (l_JobPri & 0x0FU)));
                if (0U == NvM_PriorityTable[l_CurActPriTable][(l_JobPri & 0xF0U)])
                {
        ...
        else
        {
            ...
            while (l_Curindex != l_BlockIdIndex)
            {
                l_QueueCounter++;
            ...
            if (ret == E_OK)
            {
                ...
                {
                    NvM_StandQueue[l_Preindex].NextIndex =
                            NvM_StandQueue[l_BlockIdIndex].NextIndex;
                    NvM_StandQueue[l_BlockIdIndex].NextIndex = l_BlockIdIndex;
                }
            }
        ...
        if (ret == E_OK)
        {
            ...
            NvM_QueueSpaceTalbe[(l_BlockIdIndex & 0xF0U)] |=
...
#if(NVM_API_CONFIG_CLASS_1!=NVM_API_CONFIG_CLASS)
        if (((uint8)STD_ON == NvM_MultiJob.Enqueue)
                && (NVM_WRITE_ALL_SERV_ID == NvM_MultiJob.ServiceId))
#else
        if (((uint8)STD_ON == NvM_MultiJob.Enqueue)
...
#endif
    {
        for (l_BlockIdLoop = 0; ((l_BlockIdLoop < NVM_BLOCK_NUM_ALL) &&
                (flag == FALSE)); l_BlockIdLoop++)
        {
            WRITEBLOCKONCE = NvM_GetWordBitState(NvM_BlockDescriptor[l_BlockIdLoop].FlagGroup,
                    (uint8)NVM_BLOCK_DESC_WRITEBLOCKONCE);
            if (((uint8)STD_ON == WRITEBLOCKONCE) &&
                    (NvM_AdminBlock[l_BlockIdLoop].ReadReq == FALSE))
            {
#if (STD_ON == NVM_DEV_ERROR_DETECT)

...
FUNC(Std_ReturnType, E2E_CODE) E2E_P05Protect(P2CONST(E2E_P05ConfigType, AUTOMATIC, E2E_APPL_DATA) ConfigPtr,P2VAR(E2E_P05ProtectStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) DataPtr, uint16 Length )
{
    Std_ReturnType Ret;
    uint16 Offset;
    uint16 ComputedCRC;
...
#else
        /*Copy 2-byte local variable CRC on bytes Data[Offset+0...Offset+1] using Big Endian order*/
...
#define E2E_START_SEC_CODE
#include "E2E_MemMap.h"
FUNC(Std_ReturnType, E2E_CODE) E2E_P05Check(P2CONST(E2E_P05ConfigType, AUTOMATIC, E2E_APPL_DATA) ConfigPtr,P2VAR(E2E_P05CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) DataPtr, uint16 Length )
{
    Std_ReturnType Ret;
...
#else
            /*Copy bytes Data[Offset+0...Offset+1] using Big Endian order on 2-byte local variable ReceivedCRC*/
            ReceivedCRC = (uint16)DataPtr[Offset + 1u] + (((uint16)DataPtr[Offset])<<8u);
#endif
            /*Compute CRC*/
            if((ConfigPtr->Offset) > 0u)
            {

...
static void E2E_P11Check_Seqence(P2CONST(E2E_P11ConfigType, AUTOMATIC, E2E_APPL_DATA) ConfigPtr,\
    P2VAR(E2E_P11CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,uint8 ReceivedCounter);
...
#define E2E_START_SEC_CODE
#include "E2E_MemMap.h"
FUNC(Std_ReturnType, E2E_CODE) E2E_P11Protect(P2CONST(E2E_P11ConfigType, AUTOMATIC, E2E_APPL_DATA) ConfigPtr,P2VAR(E2E_P11ProtectStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,P2VAR(uint8, AUTOMATIC, E2E_APPL_DATA) DataPtr, uint16 Length )
{
    Std_ReturnType Ret;
    uint16 Offset;
    uint8 ComputedCRC = 0;
    ...
    if(Ret == E2E_E_OK)
    {
        ...
        {
            DataPtr[ConfigPtr->DataIDNibbleOffset>>3u] = (DataPtr[ConfigPtr->DataIDNibbleOffset>>3u]&(0xFu<<(ConfigPtr->CounterOffset & 0x7u))) + (uint8)(((ConfigPtr->DataID & 0x0F00u)>>8u)<<(ConfigPtr->DataIDNibbleOffset & 0x7u));
        }
        /*Write Counter*/
        DataPtr[ConfigPtr->CounterOffset>>3u] = ((DataPtr[ConfigPtr->CounterOffset>>3u])&(0xFu<<(ConfigPtr->DataIDNibbleOffset & 0x7u)))+((StatePtr->Counter&0xFu)<<(ConfigPtr->CounterOffset&0x7u));
        /*Compute CRC*/
...
#define E2E_START_SEC_CODE
#include "E2E_MemMap.h"
FUNC(Std_ReturnType, E2E_CODE) E2E_P11Check(P2CONST(E2E_P11ConfigType, AUTOMATIC, E2E_APPL_DATA) ConfigPtr,P2VAR(E2E_P11CheckStateType, AUTOMATIC, E2E_APPL_DATA) StatePtr,P2CONST(uint8, AUTOMATIC, E2E_APPL_DATA) DataPtr, uint16 Length )
{
    Std_ReturnType Ret;
    ...
    {
        if(((DataPtr != NULL_PTR) && (Length != 0u)) || ((DataPtr == NULL_PTR) && (Length == 0u)))
        {
            if(DataPtr != NULL_PTR)
            {
                if(Length == ((ConfigPtr->DataLength)/8u))
    ...
    if(Ret == E2E_E_OK)
    {
        ...
        else
        {
            ...
            DataID_Hbyte = (uint8)(ConfigPtr->DataID>>8u);
            /*Read DataIDNibble*/
            ReceivedNibble = (DataPtr[(ConfigPtr->DataIDNibbleOffset >> 3u)]>>(ConfigPtr->DataIDNibbleOffset & 0x7u))&0xFu;
            /*Read Counter*/
        ...
        if(NewDataAvailable == TRUE)
        {
            ...
            {
                if(((ConfigPtr->DataIDMode == E2E_P11_DATAID_NIBBLE)&&(ReceivedNibble == ((uint8)(ConfigPtr->DataID>>8u)&0x0Fu)))\
                    ||(ConfigPtr->DataIDMode == E2E_P11_DATAID_BOTH))
                {
...
#include "E2E_MemMap.h"
/**
 * The function maps the check status of Profile 11 to a generic check status, which can be used by E2E state machine check function.
 * The E2E Profile 11 delivers a more fine-granular status, but this is not 

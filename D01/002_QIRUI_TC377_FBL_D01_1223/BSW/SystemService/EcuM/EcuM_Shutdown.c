...
#include "EcuM_MemMap.h"
static VAR(TaskType, ECUM_CLEARED)
EcuM_McShutdownTaskId;
...
#include "EcuM_MemMap.h"
volatile VAR(boolean, ECUM_CLEARED)
EcuM_ShutdownFlag;
...
#define ECUM_START_SEC_VAR_CLEARED_SHARE_BOOLEAN
#include "EcuM_MemMap.h"
static VAR(uint32, ECUM_CLEARED)
EcuM_SlaveShutdownEvent;
#define ECUM_STOP_SEC_VAR_CLEARED_SHARE_BOOLEAN
#include "EcuM_MemMap.h"
#endif /*ECUM_MAX_MCU_CORE_NUM > 1*/
/*******************************************************************************
    ...
    else
    {
        switch (shutdownTarget)
        {
            case ECUM_SHUTDOWN_TARGET_SLEEP:
                ...
                {
                    (void)Det_ReportError(ECUM_MODULE_ID, ECUM_INSTANCE_ID, ECUM_SID_SELECTSHUTDOWNTARGET, ECUM_E_INVALID_PAR);
                    ret = E_NOT_OK;
                }
                break;
...
)
{
    ...
    else if ((cause != ECUM_CAUSE_ECU_STATE) &&
            (cause != ECUM_CAUSE_WDGM) &&
            (cause != ECUM_CAUSE_DCM) &&
...
)
{
    ...
    else
    {
        (void)Det_ReportError(ECUM_MODULE_ID, ECUM_INSTANCE_ID,
                ECUM_SID_GODOWNHALTPOLL, ECUM_E_INVALID_PAR);
        ret = E_NOT_OK;
    }
    #endif/* (ECUM_DEV_ERROR_DETECT == STD_ON) */
    return ret;

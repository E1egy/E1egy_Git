...
static FUNC(boolean, ECUM_CODE)
EcuM_JudgeSleep(void);
#endif /*(ECUM_MAX_MCU_CORE_NUM > 1)*/

#if (ECUM_MAX_MCU_CORE_NUM > 1)
/*Clear slave ready sleep flag.*/
static FUNC(void, ECUM_CODE)
EcuM_ClrSlaveReadySleepFlag(void);

/*Set slave ready sleep flag.*/
static FUNC(void, ECUM_CODE)
EcuM_SetSlaveReadySleepFlag(void);
#endif /*(ECUM_MAX_MCU_CORE_NUM > 1)*/

/*After the wake-up event is verified, the action of other modules needs to be notified*/
static FUNC(void, ECUM_CODE)
EcuM_ValidateWkEvAction(
    uint8 wkSrcIdx
);

/*Handle SLEEP Phase*/
static FUNC(Std_ReturnType, ECUM_CODE)
EcuM_DoSleepPhase
(
    void
);
...
#include "EcuM_MemMap.h"
static volatile VAR(boolean, ECUM_CLEARED)
EcuM_SlaveReadySleepFlag[ECUM_SLAVE_CORE_NUM];
        ...
        switch (EcuMRunData.SdtgNext.Target)
        {
            ...
            default:
                ...
                (void)Det_ReportError(ECUM_MODULE_ID, ECUM_INSTANCE_ID,
                        ECUM_SID_GODOWNHALTPOLL, ECUM_E_STATE_PAR_OUT_OF_RANGE);
                #endif/* (ECUM_DEV_ERROR_DETECT == STD_ON) */
                break;
        }
    }
 ...
 * Return value: NA
 */
FUNC(void, ECUM_SETWAKEUPEVENT_CODE) EcuM_SetWakeupEvent
(
    EcuM_WakeupSourceType sources
)
{
    ...
    /*SWS_EcuM_02867*/
    else if ((ECUM_WKSOURCE_NONE == sources) ||
    ...
    #endif/* (ECUM_DEV_ERROR_DETECT == STD_ON) */
    {
        ...
        if ((sources & EcuM_SleepModeCfgs[sleepModeId].wkMask)
                != ECUM_WKSOURCE_NONE)
        {
            ...
            if (wksIdx != (uint8)ECUM_MAX_WAKE_UP_SOURCE_NUM)
            {
                ...
                {
                    #if (ECUM_USE_TIMER == ECUM_TIMER_USE_GPT)
                    ret = Tm_GetTimeSpan100us32bit(
                            &(pRt->Wks.wkTime[wksIdx]),
                            &spanTime);
                    #else
                    spanTime = Frt_CalculateElapsedMS(pRt->Wks.wkTime[wksIdx]);
                    #endif /*(ECUM_USE_TIMER == ECUM_TIMER_USE_GPT)*/
                ...
                if (EcuM_WkSourceCfgs[wksIdx].validationTimeout != 0u)
                {
                    ...
                    (void)Tm_ResetTimer100us32bit
                            (&(pRt->Wks.wkTime[wksIdx]));
                    #else /*Use Os counter.*/
                    pRt->Wks.wkTime[wksIdx] = Frt_ReadOutMS();
                    #endif /*(ECUM_USE_TIMER == ECUM_TIMER_USE_GPT)*/
                }
                else /*No validation.*/
                {
 ...
 * Return value: NA
 */
FUNC(void, ECUM_VALIDATEWAKEUPEVENT_CODE) EcuM_ValidateWakeupEvent
(
    EcuM_WakeupSourceType sources
)
{
    ...
    /* SWS_EcuM_02868 */
            ...
            (ECUM_WKSOURCE_NONE != (EcuM_WakeupSourceType)(sources & (~((EcuM_WakeupSourceType)ECUM_ALL_WKSOURCE)))))
    {
        (void)Det_ReportError(ECUM_MODULE_ID, ECUM_INSTANCE_ID, ECUM_SID_VALIDATEWAKEUPEVENT, ECUM_E_UNKNOWN_WAKEUP_SOURCE);
    }
    else
    #endif/* (ECUM_DEV_ERROR_DETECT == STD_ON) */
    {

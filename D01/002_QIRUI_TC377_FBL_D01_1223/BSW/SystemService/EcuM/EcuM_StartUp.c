...
static FUNC(void, ECUM_CODE) EcuM_MapReset2WakeupSource
(
...
uint8 EcuMWakeupSource_McuResetReason = FALSE; 
/*******************************************************************************
...
)
{
...
#endif/* (ECUM_DEV_ERROR_DETECT == STD_ON) */
    {
        ...
        /*Initialize BSWM*/
        BswM_Init((BswM_ConfigType*)(EcuM_ConfigPtr->modulePBCfg->bswMPbCfg));
        ...
        /*Calls the startup routine Rte_Start  at the end of startup phase II.*/
        Rte_Start();

        /*Initial value. Set by RTE when Rte_Start() has been called.*/
...
)
{
...
#endif /*(ECUM_MAX_MCU_CORE_NUM > 1)*/
    {
        ...
        if ((EcuM_ConfigPtr == NULL_PTR) ||
                (EcuM_ConfigConsistencyHash != EcuM_ConfigPtr->configConsistencyHash))
        {
        ...
        else
        {
            ...
            (void)EcuM_SelectShutdownTarget(
                    EcuM_ConfigPtr->defaultShutdownTgt,
                    EcuM_ConfigPtr->defaultShutdownMode);
...
)
{
    ...
    {
        for(rstRasaonIdx=0;
                rstRasaonIdx < EcuM_WkSourceCfgs[wkSrcIdx].resetResasonNum;
                rstRasaonIdx++
           )
        {
            ...
            {
                /*pRt->Wks.Pending =
                    (EcuM_WakeupSourceType)(pRt->Wks.Pending)
                    | (EcuM_WkSourceCfgs[wkSrcIdx].wkSource);*/
				EcuM_SetWakeupEvent(EcuM_WkSourceCfgs[wkSrcIdx].wkSource);
				EcuMWakeupSource_McuResetReason = TRUE;

...
static FUNC(uint8, ECUM_CODE)
EcuM_FindAlarmIdByUser(
    EcuM_UserType user
);

/*Find the next earliest user alarm clock value*/
static FUNC(EcuM_TimeType, ECUM_CODE)
EcuM_FindNextEarliestAlarm(
    void
);
...
)
{
    ...
    for (alarmIdx = 0;
            (alarmIdx < ECUM_MAX_ALARM_CLOCK_NUM) && (pRt->UserAlarm[alarmIdx] != pRt->MasterAlarm);
            alarmIdx++)
    {

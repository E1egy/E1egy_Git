...
)
{
    ...
    for (wksIdx = 0; wksIdx < (uint8)ECUM_MAX_WAKE_UP_SOURCE_NUM; wksIdx++)
    {
        ...
        {
			EcuM_StartWakeupSources(pWks->wkSource);
            BswM_EcuM_CurrentWakeup(pendingWkup, ECUM_WKSTATUS_PENDING);
...
)
{
     ...
     {
         //Rte_Switch_EcuM_CurrentMode_currentMode(EcuM_RteState);

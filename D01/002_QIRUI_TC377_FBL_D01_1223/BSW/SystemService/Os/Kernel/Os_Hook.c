...
static FUNC(void, OS_CODE) Os_ProtHookTerminateTask(StatusType osErrType)
{
    ...
    Os_SCB.sysRunningTCB->taskTop
        = Os_TaskStack[Os_GetObjLocalId(Os_TaskCfg[osRunningTaskID].osTaskStackId)].stackTop;
    ...
    #endif
    {
        ...
        if ((E_OS_PROTECTION_LOCKED != (StatusType)osErrType)
            && (E_OS_PROTECTION_TIME != (StatusType)osErrType))
        {
    ...
    if ((E_OS_PROTECTION_TIME == osErrType) || (E_OS_PROTECTION_LOCKED == osErrType))
    {
        if (Os_IntNestISR2 >= OS_TMPROT_TARGET_ISR_OFFSET)
        {
            Os_SCB.sysRunningIsrCat2Id =
                Os_SCB.sysIsrNestQueue[Os_IntNestISR2 - OS_TMPROT_TARGET_ISR_OFFSET];
        }
    }
    #endif /* TRUE == CFG_TIMING_PROTECTION_ENABLE */
 ...
 */
/******************************************************************************/
static FUNC(void, OS_CODE) Os_ProtHookTermAppl
(
...
)
{
    ...
    #if (TRUE == CFG_TIMING_PROTECTION_ENABLE)
    if ((E_OS_PROTECTION_LOCKED == osErrType)
        || (E_OS_PROTECTION_TIME == osErrType))
    {
        if (OS_TMPROT_HOOK_ISR == osWhoHook)
        {
 ...
 */
/********************************************************************/
FUNC(ProtectionReturnType, OS_CODE) Os_CallProtectionHook
(
    StatusType osErrType,
    uint32 osWhoHook
)
    ...
    if ((uint32)OS_NO_CARE == osWhoHook)
    {
        if ((StatusType)E_OS_SPINLOCK == osErrType)
        {
            for (spinLockIdx = 0u; spinLockIdx < CFG_SPINLOCK_MAX; spinLockIdx++)
            {
                ...
                {
                    if (Os_GetObjCoreId(Os_SLCB[spinLockIdx].objOccupyId) == Os_SCB.sysCore
                        && Os_GetObjLocalId(Os_SLCB[spinLockIdx].objOccupyId) == Os_SCB.sysRunningTaskID)
                    {

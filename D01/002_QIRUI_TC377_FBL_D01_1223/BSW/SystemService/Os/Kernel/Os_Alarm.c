...
static VAR(Os_AlarmType, OS_VAR) Os_CfgAutoAlarmMax  = 0U;
...
FUNC(void, OS_CODE) Os_InitAlarm(void)
{
    #if (CFG_ALARM_MAX > 0)
    Os_AlarmType                                     i;
    P2VAR(Os_ACBType volatile, AUTOMATIC, OS_VAR)    pACB = NULL_PTR;
    ...
    switch (Os_SCB.sysCore)
    {
        ...
        case 0:
            ...
            #if (CFG_AUTO_ALARM_MAX_CORE0 > 0U)
            Os_CfgAutoAlarmMax = CFG_AUTO_ALARM_MAX_CORE0;
            #endif /* CFG_AUTO_ALARM_MAX_CORE0 > 0U */
            break;
        ...
        case 1:
            ...
            #if (CFG_AUTO_ALARM_MAX_CORE1 > 0U)
            Os_CfgAutoAlarmMax = CFG_AUTO_ALARM_MAX_CORE1;
            #endif /* CFG_AUTO_ALARM_MAX_CORE1 > 0U */
            break;
        ...
        case 2:
            ...
            #if (CFG_AUTO_ALARM_MAX_CORE2 > 0U)
            Os_CfgAutoAlarmMax = CFG_AUTO_ALARM_MAX_CORE2;
            #endif /* CFG_AUTO_ALARM_MAX_CORE2 > 0U */
            break;
...
FUNC(StatusType, OS_CODE) GetAlarmBase(AlarmType AlarmID, AlarmBaseRefType Info)
{
    ...
    #if ((TRUE == CFG_USEGETSERVICEID) ||(TRUE == CFG_USEPARAMETERACCESS))
    OS_ARCH_DECLEAR_CRITICAL();
    #endif /* (TRUE == CFG_USEGETSERVICEID) ||(TRUE == CFG_USEPARAMETERACCESS) */
        ...
        #endif /*#if (OS_AUTOSAR_CORES > 1)*/
        {
            ...
            #endif /* OS_STATUS_EXTENDED == CFG_STATUS */
            {
                ...
                else
                {
                    ...
                    else if ((cycle != 0U) && ((cycle > counterMaxAllowedValue)
                                         || (cycle < counterMinCycle)))
                    {
                        #if (TRUE == CFG_ERRORHOOK)
                        OSError_Save_ServiceId(OSServiceId_SetRelAlarm);
                        OSError_Save_SetRelAlarm(AlarmID, increment, cycle);
                ...
                if ((StatusType)E_OK == OsReturnErr)
                {
                    ...
                    {
                        /* Timer type. */
                        Os_ACB[AlarmID].alarmStart = Os_CalcAbsTicks(Os_CCB[alarmCounter].counterCurVal,
        ...
        #endif /*#if (OS_AUTOSAR_CORES > 1)*/
        {
            ...
            if ((StatusType)E_OK == OsReturnErr)
            {
                ...
                else
                {
                    ...
                    if (Os_GetDistance(osCounterCurVal, start,alarmCounter)
 ...
 */
/******************************************************************************/
FUNC(void, OS_CODE) Os_InsertAlarm 
(
    Os_AlarmType alarmid, 
    Os_AlarmRefType almque,
    Os_TickType curTick
)
{
    ...
    {  
        ...
        if (curTick < alarmTick)
        {
            while (done != TRUE)
            {
                ...
                else if ((Os_ACB[next].alarmStart >= curTick)
                        && (Os_ACB[next].alarmStart <= alarmTick))
                {
                    pre = next;
 ...
 */
/******************************************************************************/
FUNC(void, OS_CODE) Os_DeleteAlarm(Os_AlarmType alarmid, Os_AlarmRefType almque)
{
    Os_AlarmType pre, next;

 ...
 */
/******************************************************************************/
FUNC(void, OS_CODE) Os_SuspendOsInt(void)
{
 ...
 */
/******************************************************************************/
FUNC(void, OS_CODE) Os_ResumeOsInt(void)
{
    Os_SuspendOsCount = Os_SuspendOsCount - 1u;/*Os_SuspendOsCount--*/
    if (0U == Os_SuspendOsCount)
    {
...
FUNC(void, OS_CODE) ResumeAllInterrupts(void)
{
    ...
    if (OS_LEVEL_TASK == Os_SCB.sysOsLevel)
    {
        ...
        if (Os_SCB.sysRunningTCB->taskSuspendAllCount > 0u)
        {
            ...
            Os_SCB.sysRunningTCB->taskSuspendAllCount =
                    Os_SCB.sysRunningTCB->taskSuspendAllCount - 1u;
        }
        else
        {
            OsReturnErr = E_NOT_OK;
...
FUNC(void, OS_CODE) ResumeOSInterrupts(void)
{
    ...
    if (OS_LEVEL_TASK == Os_SCB.sysOsLevel)
    {
        ...
        if ( Os_SCB.sysRunningTCB->taskSuspendOSCount > 0u)
        {
            ...
            Os_SCB.sysRunningTCB->taskSuspendOSCount =
                    Os_SCB.sysRunningTCB->taskSuspendOSCount - 1u;
        }
        else
        {
            OsReturnErr = E_NOT_OK;
...
FUNC(void, OS_CODE) Os_EnterISR2(void)
{
    ...
    {
        Os_TaskUsageTimerStop(Os_SCB.sysRunningTaskID);
        Os_IsrUsageTimerStart(Os_SCB.sysIsrNestQueue[Os_IntNestISR2 - 1U]);
    }
    else
    {
        Os_IsrUsageTimerStop(Os_SCB.sysIsrNestQueue[Os_IntNestISR2 - 2]);
        Os_IsrUsageTimerStart(Os_SCB.sysIsrNestQueue[Os_IntNestISR2 - 1]);
    }
    #endif
    ...
    if (Os_IntCfgIsrId < Os_CfgIsrMax)
    {
        ...
        if (Os_AppSysCfg != Os_SCB.sysRunningAppID)
        {
            ...
            Os_ArchSetIsrMemMap(Os_SCB.sysRunningIsrCat2Id,Os_SCB.sysRunningAppID,
...
FUNC(void, OS_CODE) Os_ExitISR2(void)
{
    ...
    if(0U == Os_IntNestISR2)
    {
        Os_IsrUsageTimerStop(Os_SCB.sysIsrNestQueue[Os_IntNestISR2]);
    ...
    #endif /* #if (TRUE == CFG_INT_NEST_ENABLE) */
    {
        ...
        if ((OS_TASK_IDLE == Os_SCB.sysRunningTaskID)
                && (OS_TASK_IDLE != Os_SCB.sysHighTaskID))
        #endif /* CFG_SCHED_POLICY != OS_PREEMPTIVE_NON */
        {
            ...
            if (Os_SCB.sysRunningTCB->taskSelfActCount > 0u)
            {
                Os_SCB.sysRunningTCB->taskSelfActCount =
                        Os_SCB.sysRunningTCB->taskSelfActCount - 1u;
            }
        ...
        else
        {
            ...
            if (Os_AppSysCfg != Os_SCB.sysRunningAppID)
            {
                ...
                Os_ArchSetTaskMemMap(Os_GetObjLocalId(
...
FUNC(void, OS_CODE) Os_EnterISR1(void)
{
	...
	if (0u == Os_IntNestISR1)
	{
		...
		{
			Os_IsrUsageTimerStop(Os_SCB.sysIsrNestQueue[Os_IntNestISR2 - 1]);
		}
	}
	#endif

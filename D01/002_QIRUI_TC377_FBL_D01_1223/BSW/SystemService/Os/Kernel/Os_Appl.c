 ...
 *  V0.1.0       2016-07-08  ss           Initial version.
 ...
 */
#if (CFG_ISR_MAX > 0)
static FUNC(void, OS_CODE) Os_TerminateISRObjectHandler
(
    P2CONST(Os_IsrType, AUTOMATIC, OS_VAR) pOsAppIsrRef,
    Os_IsrType OsAppIsrCnt
);
 ...
 */
static FUNC(void, OS_CODE) Os_TerminateTaskObjectHandler
(
    P2CONST(Os_TaskType, AUTOMATIC, OS_VAR) pOsAppTaskRef,
    Os_TaskType OsAppTaskCnt
);
...
static FUNC(void, OS_CODE) Os_ApplReadyQueueRemove
(
    Os_TaskType     OsTaskID, 
    Os_PriorityType OsQueuePriority
);
...
static FUNC(void, OS_CODE) Os_ApplReleaseResource
(
    ResourceType       ResID,
    ObjectTypeType     Object,
    Os_AppObjectIDType OsAppObjID
);
 ...
 */
#if (CFG_ALARM_MAX > 0)
static FUNC(void, OS_CODE) Os_TerminateAlarmObjectHandler
(
    P2CONST(Os_AlarmType, AUTOMATIC, OS_VAR) pOsAppAlarmRef,
    Os_AlarmType                             OsAppAlarmCnt
);
...
{
    ApplicationType ApplID = OS_APPLICATION_INVALID;
        ...
        else if (ObjectID < pObjectIDMaxTableRef[ObjectType])
        {
            ...
            else
            {
                ...
                if ((pObjectAppCfgRef[ObjectType][ObjectID].hostApp != Os_SCB.sysRunningAppID)
                    &&(Os_AppCB[pObjectAppCfgRef[ObjectType][ObjectID].hostApp].appState
 ...
 */
/******************************************************************************/
FUNC(void, OS_CODE) Os_TerminateObjects
(
    P2CONST(Os_ApplicationCfgType, AUTOMATIC, OS_VAR) pOsCurAppCfg
)
{
     ...
     */
    Os_TerminateAlarmObjectHandler((Os_AlarmRefType)pOsCurAppCfg->OsAppObjectRef[OBJECT_ALARM],
   ...
   /* MISRA-C: 17.4  Array subscripting applied to an object of pointer type. 
    ...
    Os_TerminateISRObjectHandler((Os_IsrType *)pOsCurAppCfg->OsAppObjectRef[OBJECT_ISR],
                                 pOsCurAppCfg->OsAppIsrRefCnt);
    #endif /* CFG_ISR_MAX > 0 */
     ...
     */
    Os_TerminateTaskObjectHandler((Os_TaskRefType)pOsCurAppCfg->OsAppObjectRef[OBJECT_TASK],
...
static FUNC(void, OS_CODE) Os_AppTerminateIsrKernelProc(Os_IsrType OsIsrID)
{
    #if ((TRUE == CFG_TIMING_PROTECTION_ENABLE) || (CFG_STD_RESOURCE_MAX > 0)\
        || (CFG_SPINLOCK_MAX > 0U))
    ...
    if (OsIsrID < Os_CfgIsr2Max)
    {
        ...
        if (osTemp > 0u)
        {
            for (i = osTemp; i > 0u; i--)
            {
                Os_ApplReleaseResource(pOsICB->IsrC2ResourceStack[i - 1u],
                                       OBJECT_ISR,
                                       OsIsrID);
            }
        }
...
#endif /*#if ((OS_BCC2 == CFG_CC) || (OS_ECC2 == CFG_CC))*/
/******************************************************************************/
/*

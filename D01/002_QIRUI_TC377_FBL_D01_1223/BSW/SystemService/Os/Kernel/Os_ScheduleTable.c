 ...
 *  Brief      : AutoSar SC2 Schedule Table Management
 *
 *  Revision History:
 *       
 *     1. 2015-03-10: Yan.Zhuang  Initial version, add this file.
 *     2. 2019-08-21  shun.li & zhiqiang.huang   Functional safety initial version.
 *     3. 2019-11-21  shun.li     Functional safety version.
 */
 /********************************************************************/
...
static VAR(Os_ScheduleTableType, OS_VAR) Os_CfgSchedTblMax                  = 0U;
...
static FUNC(void, OS_CODE) Os_StInnerErrHook
(
    Os_ServiceIdType osServiceId,
    StatusType osErrCode
);
...
static FUNC(void, OS_CODE) Os_StEpActiveTask
(
    P2CONST(Os_EPActivateTaskAction, AUTOMATIC, OS_VAR) pTaskList,
    uint16 osTaskCnt
);

#if (CFG_EXTENDED_TASK_MAX > 0)
static FUNC(void, OS_CODE) Os_StEpSetEvent
(
    P2CONST(Os_EPSetEventAction, AUTOMATIC, OS_VAR) pEventList,
    uint16 osEventCnt
);
...
static FUNC(boolean, OS_CODE) Os_InitStAutoStartChk
(
    P2CONST(Os_SchedTblAutostart, AUTOMATIC, OS_VAR) osSchedTblAutostartRef
);
...
)
{
    ...
    switch (osType)
    {
        case ST_START_ABSOLUTE:
            Os_StInnerStart(pStCfgRef->osSchedTblAutostartRef->osSchedTblAbsValue,
                            pStCB, pStCfgRef);
...
)
{
    ...
    if (NULL_PTR != osSchedTblAutostartRef)
    {
        ...
        if (0U != (osSchedTblAutostartRef->osSchedTblAppModeBitmap
                & Os_SCB.sysActiveAppMode))
        {
            bAutoStart = TRUE;
        }
    }
...
static FUNC(void, OS_CODE) Os_InitSchedTblCB(Os_ScheduleTableType StId)
{
    ...
    pStCB->stFinalDelay     = pStCfgRef->osSchedTblDuration -
                pStCfgRef->osSchedTblEP[pStCfgRef->osSchedTblEPsize - 1u].osSchedTblEPOffset;
...
static FUNC(void, OS_CODE) Os_InsertSTNode(Os_ScheduleTableType InsertStId)
{
    ...
    if ((NULL_PTR != pInsertStCB->stNextNode)
        || (NULL_PTR != pInsertStCB->stPreNode))
    {
        /*nothing to do*/
    }
    ...
    else
    {
        ...
        InsertStEpRelTick = Os_GetDistance(pCcb->counterLastVal,
                                           pInsertStCB->stNextEpAbsTick,
                                           stCounterId);
        ...
        while (NULL_PTR != pCurStCB)
        {
            ...
            if (InsertStEpRelTick >= Os_GetDistance(pCcb->counterLastVal,
                                pCurStCB->stNextEpAbsTick, stCounterId))
            {
                pPreStCB  = pCurStCB;
...
)
{
    ...
    pStCB->stNextEpAbsTick = Os_CalcAbsTicks(pStCB->stStartAbsTick,
...
)
{
    ...
    {
        (void)SetEvent(pEventList[i].osSchedTblSetEventTaskRef,
                       pEventList[i].osSchedTblSetEventRef);
    }
...
)
{
    ...
    /* Active task. */
    Os_StEpActiveTask(pStNode->stNextEP->osActivateTaskList,
                      pStNode->stNextEP->osActivateTaskListSize);

    /* Set event. */
    #if (CFG_EXTENDED_TASK_MAX > 0)
    Os_StEpSetEvent(pStNode->stNextEP->osSetEventList,
                    pStNode->stNextEP->osSetEventListSize);
    #endif /* CFG_EXTENDED_TASK_MAX > 0 */
...
)
{
    ...
    #endif /* (OS_SC2 == CFG_SC) || (OS_SC4 == CFG_SC) */
    {
        pStNode->stNextEpAbsTick = Os_CalcAbsTicks(pStNode->stNextEpAbsTick,
                                           pStNode->stDelay,
                                           pStCfgRef->osSchedTblCounterRef);
...
)
{
    ...
    /* Offset of pre EP. */
    osPreEpOffset = ((0u == pStCB->stEpId) ?
    (0u) : (pStCfgRef->osSchedTblEP[pStCB->stEpId - 1u].osSchedTblEPOffset));
    ...
    if (TRUE == pStCB->stIsAdjAdd)
    {
        ...
        else
        {
            /* Deviatifon. */
            pStCB->stDev =
                    ((pStCB->stDiff > pStCB->stNextEP->osSchedTblMaxAdvance) ?
                    (pStCB->stNextEP->osSchedTblMaxAdvance) : (pStCB->stDiff));

            /* pStCB->stAdjDevTotal <= pStCB->stDiff. */
            if ((pStCB->stDiff - pStCB->stAdjDevTotal)
                    <= pStCfgRef->osSchedTblSync.osSchedTblExplicitPrecision)
            {
                /* Sync success. */
                pStCB->stIsAdjust    = FALSE;
    ...
    {
        /* OS436 */
        if (((pStCB->stNextEP->osSchedTblEPOffset)
        ...
        {
            /* Deviatifon. */
            pStCB->stDev = ((pStCB->stDiff > pStCB->stNextEP->osSchedTblMaxRetard) ?
                            (pStCB->stNextEP->osSchedTblMaxRetard) : (pStCB->stDiff));
...
)
{
    ...
    pStCB->stStartAbsTick  = Os_CalcAbsTicks(oscounterCurVal,
                                            (pStCfgRef->osSchedTblDuration - osValue),
                                             pStCfgRef->osSchedTblCounterRef);
...
{
    Os_TickType osDiff       = 0u;
    Os_TickType osPosOnTbl;

    /* Position on Tbl. */
    osPosOnTbl = Os_GetDistance(oscounterCurVal,
    ...
    {
        osPosOnTbl = (pStCB->stNextEP->osSchedTblEPOffset - osPosOnTbl)
                + pStCfgRef->osSchedTblDuration - 1u;
    }
...
FUNC(void, OS_CODE) Os_WorkSchedTbl(Os_CounterType CounterID)
{
    ...
    Os_ApplicationType bakappID = OS_APPLICATION_INVALID;
    ...
    else
    {
        ...
        {
            if (Os_GetDistance(pstListHead->stNextEpAbsTick,
                    counterCurval,
                    CounterID) > pstCounterCfg->osCounterMaxAllowedValue)
            {
                break;
            }
...
)
{
    Os_TickType                                    osCounterCurVal;
    Os_TickType                                    osStartAbsTick;
    P2CONST(Os_SchedTblCfgType, AUTOMATIC, OS_VAR) pStCfgRef = NULL_PTR;
    P2CONST(Os_CounterCfgType, AUTOMATIC, OS_VAR)  pCounterCfgRef = NULL_PTR;
        ...
        #endif /*#if (OS_AUTOSAR_CORES > 1)*/
        {
            ...
            #endif /* OS_STATUS_EXTENDED == CFG_STATUS */
            {
                ...
                #endif /* (OS_SC2 == CFG_SC) || (OS_SC4 == CFG_SC) */
                {
                    ...
                    if ((Offset > (pCounterCfgRef->osCounterMaxAllowedValue - stInitOffset))
                        || (0u == Offset))
                    {
                        #if (TRUE == CFG_ERRORHOOK)
            ...
            if ((StatusType)E_OK == OsReturnErr)
            {
                ...
                /* Save abs start tick. */
                osStartAbsTick = Os_CalcAbsTicks(osCounterCurVal, Offset,
                                       pStCfgRef->osSchedTblCounterRef);
    ...
    if ((StatusType)E_OK == OsReturnErr)
    {
        ...
        #endif /* (OS_STATUS_EXTENDED == CFG_STATUS) */
        {
            ...
            if ((StatusType)E_OK == OsReturnErr)
            {
                ...
                /* OS283 */
                if ((SCHEDULETABLE_STOPPED == pStFromCB->stState)
                        || (SCHEDULETABLE_NEXT == pStFromCB->stState))
                {
                    #if (TRUE == CFG_ERRORHOOK)
            ...
            if ((StatusType)E_OK == OsReturnErr)
            {
                ...
                /* OS284 */
                pStToCB->stStartAbsTick  = Os_CalcAbsTicks(pStFromCB->stStartAbsTick,
                                                   pStFromCfgRef->osSchedTblDuration,
                                                   pStFromCfgRef->osSchedTblCounterRef);

                pStToCB->stNextEpAbsTick = Os_CalcAbsTicks(pStToCB->stStartAbsTick,

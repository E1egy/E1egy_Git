...
FUNC(void, OS_CODE) Os_InitTask(void)
{
    ...
    P2CONST(Os_TaskCfgType, AUTOMATIC, OS_VAR) pTaskCfg = NULL_PTR;
    ...
    for (i = 0u; i < Os_CfgTaskMax; i++)
    {
        ...
        if (0U != (pTaskCfg->osTaskAutoStartMode & Os_SCB.sysActiveAppMode))
        {
            ...
            if ((Os_SCB.sysHighPrio == OS_PRIORITY_INVALID)
                    || (pTCB->taskRunPrio > Os_SCB.sysHighPrio))
            {
...
FUNC(StatusType, OS_CODE) ActivateTask(TaskType TaskID)
{
    ...
    switch (Os_GetObjCoreId(TaskID))
    {
        #if (CFG_TASK_MAX_CORE0 > 0U)
        case 0:
            if (Os_GetObjLocalId(TaskID) >= CFG_TASK_MAX_CORE0)
            {
        ...
        #if (CFG_TASK_MAX_CORE1 > 0U)
        case 1:
            if (Os_GetObjLocalId(TaskID) >= CFG_TASK_MAX_CORE1)
            {
                #if (TRUE == CFG_ERRORHOOK)
                OSError_Save_ServiceId(OSServiceId_ActivateTask);
                OSError_Save_ActivateTask(TaskID);
        ...
        #if (CFG_TASK_MAX_CORE2 > 0U)
        case 2:
            if (Os_GetObjLocalId(TaskID) >= CFG_TASK_MAX_CORE2)
            {
                #if (TRUE == CFG_ERRORHOOK)
                OSError_Save_ServiceId(OSServiceId_ActivateTask);
                OSError_Save_ActivateTask(TaskID);
...
FUNC(StatusType, OS_CODE) TerminateTask(void)
{
    ...
    #if (TRUE == CFG_SERVICE_PROTECTION_ENABLE)
    else if (Os_WrongContext(OS_CONTEXT_TERMINATE_TASK) != TRUE)
    {
        Status = E_OS_CALLEVEL;
    }
    else if (Os_IgnoreService() != TRUE)
        ...
        #endif/* (OS_NOSC == CFG_SC) || (OS_SC1 == CFG_SC) || (OS_SC2 == CFG_SC)*/
        {
            ...
            #endif /* CFG_STD_RESOURCE_MAX > 0U */
            {
                ...
                for (i = 0u; i < CFG_SPINLOCK_MAX; i++)
                {
                    ...
                    {
                        if (Os_GetObjCoreId(Os_SLCB[i].objOccupyId) == Os_SCB.sysCore
                            && Os_GetObjLocalId(Os_SLCB[i].objOccupyId) == Os_SCB.sysRunningTaskID)
                        {
                ...
                if ((StatusType)E_OK == Status)
                {
                    ...
                    if (Os_SCB.sysRunningTCB->taskActCount > 0U)
                    {
                        Os_SCB.sysRunningTCB->taskActCount =
                                Os_SCB.sysRunningTCB->taskActCount - 1U;
                    }
                    ...
                    Os_ReadyQueueRemove(OS_LEVEL_TASK,
                    ...
                    Os_SCB.sysRunningTCB->taskRunPrio =
                            Os_TaskCfg[Os_SCB.sysRunningTaskID].osTaskPriority;
...
FUNC(StatusType, OS_CODE) GetTaskState(TaskType TaskID, TaskStateRefType State)
{
    ...
    #if (TRUE == CFG_SERVICE_PROTECTION_ENABLE)
    else if (Os_WrongContext(OS_CONTEXT_GET_TASK_STATE) != TRUE)
    {
        Status = E_OS_CALLEVEL;
    }

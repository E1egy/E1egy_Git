 ...
 *  
 *  File         Os_Tprot.c
 *  Brief        AutoSar Protection Management
 *
 *  Revision History:
 *       
...
static FUNC(void, OS_CODE) Os_TmProtInitTaskResLock
(
...
static FUNC(void, OS_CODE) Os_TmProtIsrFrameCounter
(
    Os_IsrType          osIsrId,
    Os_TmProtOptIsrType osOptType
);
static FUNC(void, OS_CODE) Os_TmProtIsrCounter
(
    Os_IsrType          osIsrId,
    Os_TmProtOptIsrType osOptType
);
...
#if (CFG_STD_RESOURCE_MAX > 0)
static FUNC(void, OS_CODE) Os_TmProtInitIsrResLock
(
    Os_IsrType                                 osIsrId,
    P2CONST(Os_IsrCfgType,  AUTOMATIC, OS_VAR) pIsrCfg
);
...
static FUNC(void, OS_CODE) Os_TmProtTaskFrameCounter
(
    Os_TaskType          osTaskId,
    Os_TmProtOptTaskType osOptType
);
static FUNC(void, OS_CODE) Os_TmProtTaskCounter
(
    Os_TaskType          osTaskId,
    Os_TmProtOptTaskType osOptType
);
 ...
 */
/********************************************************************/
FUNC(void, OS_CODE) Os_TmProtResStart
(
    ResourceType           osResId, 
    Os_TmProtResOccupyType osOccupyType
)
{
    uint8                                                 osWhoHook = 0U;
    Os_TickType                                           osBudget  = OS_TICK_INVALID;
...
FUNC(StatusType, OS_CODE) Os_TmProtTaskFrameChk(Os_TaskType osTaskId)
{
    ...
    P2VAR(Os_TmProtCbDataDef volatile, AUTOMATIC, OS_VAR) pCbData  = NULL_PTR;
    ...
    if (OS_TASK_IDLE != osTaskId)
    {
        ...
        if (OS_TICK_INVALID != pCbData->osTpBudget)
        {
            ...
            {
                /* Hook. */
                if (PRO_IGNORE != Os_CallProtectionHook(E_OS_PROTECTION_ARRIVAL,
                        OS_TMPROT_HOOK_TASK))
                {
                    osRet = E_OS_ID;   
                }
 ...
 */
/********************************************************************/
FUNC(void, OS_CODE) Os_TmProtTaskStart
(
    Os_TaskType          osTaskId,
    Os_TmProtOptTaskType osOptType
)
 ...
 */
/********************************************************************/
FUNC(void, OS_CODE) Os_TmProtTaskEnd
(
    Os_TaskType          osTaskId,
    Os_TmProtOptTaskType osOptType
)
 ...
 */
/********************************************************************/
FUNC(void, OS_CODE) Os_TmProtIsrStart
(
    Os_IsrType          osIsrId,
    Os_TmProtOptIsrType osOptType
)
 ...
 */
/********************************************************************/
FUNC(void, OS_CODE) Os_TmProtIsrEnd
(
    Os_IsrType          osIsrId,
    Os_TmProtOptIsrType osOptType
)
...
)
{
    ...
    else
    {
        ...
        Os_TmProtInitCbData(&pIcb->osTpIsr[TP_ISR_CAT2_SUS_OS_INT],
                pIsrTimePt->osIsrOsIptLockBudget);
        Os_TmProtInitCbData(&pIcb->osTpIsr[TP_ISR_CAT2_SUS_ALL_INT],
                pIsrTimePt->osIsrAllIptLockBudget);
        Os_TmProtInitCbData(&pIcb->osTpIsr[TP_ISR_CAT2_DIS_ALL_INT],
                pIsrTimePt->osIsrAllIptLockBudget);
    }
...
)
{
    ...
    else
    {
        ...
        Os_TmProtInitCbData(&pTcb->osTpTask[TP_TASK_SUS_OS_INT],
                pTaskTmProtCfgRef->osTaskOsInterruptLockBudget);
        Os_TmProtInitCbData(&pTcb->osTpTask[TP_TASK_SUS_ALL_INT],
                pTaskTmProtCfgRef->osTaskAllInterruptLockBudget);
        Os_TmProtInitCbData(&pTcb->osTpTask[TP_TASK_DIS_ALL_INT],
                pTaskTmProtCfgRef->osTaskAllInterruptLockBudget);

...
FUNC(void, OS_CODE) StartCore(CoreIdType CoreID, StatusType* Status)
{
    ...
    #endif/*#if (TRUE == CFG_SERVICE_PROTECTION_ENABLE)*/
    {
        ...
        else if (Os_CoreCB.coreStateSynPoint2[Os_SCB.sysCore] == TRUE)
        {
            *Status = E_OS_ACCESS;
        }
 ...
 */            
/********************************************************************/
FUNC(void, OS_CODE) Os_InterCoreAction(Os_CoreIdType osCoreId)
{
    ...
    if(osCoreId < OS_AUTOSAR_CORES)
	{
		switch (Os_InterCore[osCoreId].serviceId)
		{
			...
			case OSServiceId_ActivateTask:
				/*SWS_Os_00596*/
				Os_InterCore[osCoreId].interState = ActivateTask(
						Os_InterCore[osCoreId].interParam.taskId);
				break;
			...
			case OSServiceId_GetTaskState:
				Os_InterCore[osCoreId].interState = GetTaskState(
						Os_InterCore[osCoreId].interParam.taskId,
						Os_InterCore[osCoreId].interParam.taskState);
				break;
			#endif /*#if (CFG_TASK_MAX > 0U)*/

			#if (CFG_EXTENDED_TASK_MAX > 0)
			case OSServiceId_SetEvent:
				Os_InterCore[osCoreId].interState = SetEvent(
						Os_InterCore[osCoreId].interParam.taskId,
						Os_InterCore[osCoreId].interParam.eventMask);
				break;
			...
			case OSServiceId_GetAlarm:
				Os_InterCore[osCoreId].interState = GetAlarm(
						Os_InterCore[osCoreId].interParam.alarmId,
						Os_InterCore[osCoreId].interParam.alarmTickRef);
				break;

			case OSServiceId_GetAlarmBase:
				Os_InterCore[osCoreId].interState = GetAlarmBase(
						Os_InterCore[osCoreId].interParam.alarmId,
						Os_InterCore[osCoreId].interParam.alarmBaseRef);
				break;

			case OSServiceId_CancelAlarm:
				Os_InterCore[osCoreId].interState = CancelAlarm(
						Os_InterCore[osCoreId].interParam.alarmId);
				break;
			...
			case OSServiceId_StopScheduleTable:
				Os_InterCore[osCoreId].interState = StopScheduleTable(
						Os_InterCore[osCoreId].interParam.ScheduleTableId);
				break;
    	...
    	Os_InterCore[osCoreId].callCoreRequest[OsLocalCore] = FALSE;
    }
}
 ...
 */
/********************************************************************/
FUNC(void, OS_CODE) Os_InterCoreOperation(Os_CoreIdType osCoreId)
{
	Os_CoreIdType ix;

	for(ix = (Os_CoreIdType)0; ix < OS_AUTOSAR_CORES; ix++)
	{
		if(TRUE == Os_InterCore[ix].callCoreRequest[osCoreId])
		{

...
(type *)((void *)( (char *)(ptr) - OS_OFFSETOF(type, member) ));
...
const Os_IocPipeCfgType*   OSStdPipeCfgGlobalPtr  = (Os_IocPipeCfgType*)0;
...
#if (CFG_IOC_MAX_LOCAL > 0U)
static FUNC(StatusType, OS_CODE) Os_IocInterReceive(
    Os_IocAbsIdType       absId,
    const Os_IocDataSourceType* pRcvData
);
#endif

#if (CFG_IOC_MAX_LOCAL > 0U)
static FUNC(StatusType, OS_CODE) Os_IocInterTransmit(
    Os_IocAbsIdType       absId,
	const Os_IocDataSourceType* transData
);
#endif

#if (CFG_IOC_MAX_GLOBAL > 0U)
static FUNC(StatusType, OS_CODE) Os_IocCrossTransmit(
    Os_IocAbsIdType       absId,
    const Os_IocDataSourceType* transData
);
#endif

static FUNC(StatusType, OS_CODE) Os_LLSendData(
    const Os_IocDataSourceType* pData,
    Os_IocCBType* pIocCB
);

static FUNC(Os_IocMsgCBType*, OS_CODE) Os_MallocMessage(
    Os_IocListNodeType** pqHead,
    Os_IocListNodeType** pqNode
);

static FUNC(StatusType, OS_CODE) Os_LLWriteData(
    const Os_IocDataSourceType* pData,
    Os_IocCBType* pIocCB
);

static FUNC(void, OS_CODE) Os_InsertListAfter(
    Os_IocListNodeType* pNode,
    Os_IocListNodeType** pqObject
);

#if (TRUE == OS_IOC_CHECKINPARA)
static FUNC(StatusType, OS_CODE) Os_IocCheckInPara(
    Os_IocAbsIdType       absId,
    const Os_IocDataSourceType* pData
);
#endif

static FUNC(StatusType, OS_CODE) Os_IocCheckPipeMemPoolUsage(
    Os_IocAbsIdType       absId,
    const Os_IocDataSourceType* pDataSource,
    StatusType*           outRet
);
...
static FUNC(void, OS_CODE) Os_MemCopyWithoutOut(
    void * dest,
    void * src,
    Os_IocU32Type size
);
...
static FUNC(StatusType, OS_CODE) Os_CheckSafeContext(
    Os_IocAbsIdType      ix,
    Os_IocTransferType   transfer,
    StatusType*          outRet
);
#endif
...
static FUNC(void, OS_CODE) Os_InitIocCB(void)
{
    ...
    if (OS_CORE_ID_MASTER == Os_SCB.sysCore)
    {
        for (ix = (Os_IocU16Type)0; ix < OSIocCfgGlobalMax; ix++)
        {
            ...
            OSIocCBGlobalPtr[ix].IocMemPoolTailPtr = pMemPool + memSize - 1u;
    ...
    for (ix = (Os_IocU16Type)0; ix < OSIocCfgLocalMax; ix++)
    {
        ...
        OSIocCBLocalPtr[ix].IocMemPoolTailPtr = pMemPool + memSize - 1u;
...
static FUNC(void, OS_CODE) Os_InitIocMsgList(void)
{
    ...
    if (OS_CORE_ID_MASTER == Os_SCB.sysCore)
    {
        ...
        for(ix = (Os_IocU16Type)0; ix < OSIocMsgCfgGlobalMax; ix++)
        {
            Os_InsertListAfter(&(OSIocMsgCBGlobalPtr[ix].IocListNodeInfo),
                               &OSIocMsgNodeGlobalPtr);
        }
    }
    #endif
    ...
    for (ix = (Os_IocU16Type)0; ix < OSIocMsgCfgLocalMax; ix++)
    {
        Os_InsertListAfter(&(OSIocMsgCBlocalPtr[ix].IocListNodeInfo),
                           &OSIocMsgNodeLocalPtr);
    }
    #endif
}
 ...
 */
/******************************************************************************/
static FUNC(StatusType, OS_CODE) Os_CheckMemPoolUsage(
    const Os_IocDataSourceType* pDataSrc,
    const Os_IocCBType* pICB,
    Os_IocRelPipeNumType absId
)
{
    Os_IocU8Type ix;
	...
	{
		memSize = OSStdPipeCfgGlobalPtr[relId]
					 .IocPipeCfgPtr->IocMemPoolSize;
	}
	#endif
    ...
    {
        memSize = OSStdPipeCfgLocalPtr[relId]
                        .IocPipeCfgPtr->IocMemPoolSize;
    }
    #endif
 ...
 */
/******************************************************************************/
static FUNC(void, OS_CODE) Os_InsertListBefore(
    Os_IocListNodeType * pNode,
    Os_IocCBType * pqObject
)
{
...
)
{
    ...
    if ((Os_IocU8Type)1 == packNum)
    {
        ...
        (void)Os_MemCopyWithoutOut(pIocCB->IocMemPoolPtr,
                                   pData->IocDataPtr,
                                   pData->IocDataLenth
                                  );
    ...
    else
    {
        ...
        )
        {
            ...
            (void)Os_MemCopyWithOut(tempMemPoolPos,
 ...
 */
/******************************************************************************/
static FUNC(void, OS_CODE) Os_SaveDataToMailForLIFO(
...
)
{
    ...
    if ((Os_IocU8Type)1 == packNum)
    {
        ...
        /* NOTE!! ++-->Data will be copied to memory pool. */
        (void)Os_MemCopyWithOut(pIocCB->IocMemPoolPtr,
                                pData->IocDataPtr,
                                pData->IocDataLenth,
                                DIRECTION_INBUFF,
                                &ptr
 ...
 */
/******************************************************************************/
static FUNC(void, OS_CODE) Os_SaveDataToMailForFIFO(
    Os_IocMsgCBType* pMailCB,
    const Os_IocDataSourceType* pData,
    Os_IocCBType* pIocCB
)
{
    ...
    if ((Os_IocU8Type)1 == packNum)
    {
        ...
        else
        {
            ...
            pBuff = (Os_IocU8Type*)(pData->IocDataPtr) + memBlock1;
            /* NOTE!! ++-->Data will be copied to memory pool. */
    ...
    else
    {
        for (index = (Os_IocU8Type)0; index < packNum; index++)
        {
            ...
            else
            {
                ...
                pBuff = (Os_IocU8Type*)(pData[index].IocDataPtr) + memBlock1;
                /* NOTE!! ++-->Data will be copied to memory pool. */
                pIocCB->IocMemPoolPtr = pIocCB->IocMemPoolHeadPtr;
                (void)Os_MemCopyWithOut(pIocCB->IocMemPoolPtr,
                                        pBuff,
...
)
{
    ...
    if ((Os_IocCBType*)0 != pIocCB)
    {
        ...
        {
            workMail = Os_MallocMessage(&OSIocMsgHeadLocalPtr,
                                              &OSIocMsgNodeLocalPtr
                                             );
        }
        #endif
        ...
        if (CROSS_CORE_COM == comMark)
        {
        	...
        	workMail = Os_MallocMessage(&OSIocMsgHeadGlobalPtr,
                                        &OSIocMsgNodeGlobalPtr
                                       );
        ...
        if (workMail != (Os_IocMsgCBType*)0)
        {
            if (QUEUED_FIFO == pIocCB->IocQueueCom)
            {
                ...
                (void)Os_SaveDataToMailForFIFO(workMail,
                		                       pData,
											   pIocCB
                                              );
                /* NOTE!! ++-->insert into pipe list. */
                (void)Os_InsertListAfter(&(workMail->IocListNodeInfo),
                                         &(pIocCB->IocPtrTailNode)
                                        );
            }
            else
            {
                /* NOTE!! ++-->Data should be written into the mailbox. */
                (void)Os_SaveDataToMailForLIFO(workMail,
                		                       pData,
											   pIocCB
                                              );
                /* NOTE!! ++-->insert into pipe list. */
                (void)Os_InsertListBefore(&(workMail->IocListNodeInfo),
                		                  pIocCB
                                         );
            }
...
)
{
    ...
    if (IOC_E_OK == Os_RetErr)
    {
        ...
        if (CAT1_UNQUEUED == tempSema)
        {
            Os_RetErr = Os_LLWriteData(transData,
                                    pIocCB
                                    );
        }
        else
        {
            Os_RetErr = Os_LLSendData(transData,
                                    pIocCB
                                    );
        }
    }
...
)
{
    ...
    if ( (((Os_IocU16Type)1 << (Os_SCB.sysRunningAppID & (Os_IocU16Type)0x0F))
        & (pMask[Os_SCB.sysRunningAppID >> (Os_IocU16Type)4])) == (Os_IocU16Type)0
    )
    {
        Os_RetErr = IOC_E_NOK;
    }
    else if (Os_AppCB[Os_SCB.sysRunningAppID].appState !=
...
)
{
    ...
    if (CROSS_CORE_COM == tempComMark)
    {
        ...
        packNum = OSStdPipeCfgGlobalPtr[relPipeNum]
                                .IocPipeCfgPtr->IocCfgInParaNum;
    }
    #endif
    ...
    if (INTER_CORE_COM == tempComMark)
    {
        ...
        packNum = OSStdPipeCfgLocalPtr[relPipeNum]
                        .IocPipeCfgPtr->IocCfgInParaNum;
    }
    #endif
 ...
 */
/******************************************************************************/
FUNC(StatusType, OS_CODE) Os_IocHLTransmit(
    Os_IocAbsIdType       absId,
    const Os_IocDataSourceType* pData
)
{
    ...
    if (IOC_E_OK == Os_RetErr)
    {
        ...
        if (INTER_CORE_COM == tempComMark)
        {
            ...
            {
                Os_RetErr = Os_IocInterTransmit(absId,
                                                pData
                                               );
            }
        }
        ...
        if (CROSS_CORE_COM == tempComMark)
        {
            ...
            {
                Os_RetErr = Os_IocCrossTransmit(absId,
                                                pData
                                               );
            }
        }
 ...
 */
/******************************************************************************/
static FUNC(Os_IocMsgCBType*, OS_CODE) Os_MallocMail(
    Os_IocListNodeType* pNode,
    Os_IocListNodeType** pTail
)
{
...
)
{
    ...
    if ((pMailNodePrev != (Os_IocListNodeType*)0)
         && (pMailNodeNext != (Os_IocListNodeType*)0))
    {
 ...
 */
/******************************************************************************/
static FUNC(void, OS_CODE) Os_ReleaseMail(
    Os_IocMsgCBType* pMailCB,
    Os_IocListNodeType * const * pqHead
)
{
 ...
 */
/******************************************************************************/
static FUNC(StatusType, OS_CODE) Os_LLReadData(
    const Os_IocDataSourceType* pData,
    const Os_IocCBType* pIocCB
)
{
    ...
    if (pIocCB != (Os_IocCBType*)0)
    {
        ...
        if ((Os_IocU8Type)1 == packNum)
        {
            ...
            Os_MemCopyWithoutOut(pData->IocDataPtr,
        ...
        else
        {
            ...
            {
                /* NOTE!! ++-->The reception will get the data from memory pool.*/    
                (void)Os_MemCopyWithOut(pData[index].IocDataPtr,
                                        tempMemPoolPos,
                                        pData[index].IocDataLenth,
                                        DIRECTION_OUTBUFF,
                                        &tempMemPoolPos1
...
)
{
    ...
    packNum = pIocCB->IocInParaNum;
    ptr = pMailCB->IocPtrMemPoolPosition - pMailCB->IocDataLenth;
    tempMemPoolPos = ptr;
    ...
    else
    {
        ...
        {
            /* NOTE!! ++-->The reception will get the data from memory pool.*/
            Os_MemCopyWithOut(pData[index].IocDataPtr,
                              tempMemPoolPos,
                              pData[index].IocDataLenth,
                              DIRECTION_OUTBUFF,
                              &tempMemPoolPos1
...
)
{
    ...
    if ((Os_IocU8Type)1 == packNum)
    {
        ...
        else
        {
            ...
            /* NOTE!! ++-->1. block 1 copy.*/
            ptr = pIocCB->IocMemPoolTailPtr - memBlock1 + 1u;
            ptr1 = pData->IocDataPtr;
            Os_MemCopyWithoutOut(ptr1,
                                 ptr,
                                 memBlock1
                                );
            ...
            ptr1 = (Os_IocU8Type*)(pData->IocDataPtr) + memBlock1;
            Os_MemCopyWithoutOut(ptr1,
                                 ptr,
                                 memBlock2
                                 );
        ...
        else
        {
            ...
            )
            {
                ...
                else if ((memSize > 0u) && (memSize < pData[index].IocDataLenth))
                {
                    ...
                    /* NOTE!! ++-->block 2 copy. */
                    ptr1 = (Os_IocU8Type*)(pData[index].IocDataPtr) + memSize;
                    memSize = pData[index].IocDataLenth - memSize;
                    ptr = pIocCB->IocMemPoolHeadPtr;
                    (void)Os_MemCopyWithOut(ptr1,
                                            ptr,
                                            memSize,
                                            DIRECTION_OUTBUFF,
                                            &ptr2
 ...
 */
/******************************************************************************/
static FUNC(StatusType, OS_CODE) Os_LLReceiveData(
    const Os_IocDataSourceType* pData,
    Os_IocCBType* pIocCB
)
{
    Os_IocMsgCBType* pWorkMailCB;
    StatusType Os_RetErr;
    Os_IocCoreComType tempComMark;
    ...
    if (pIocCB != (Os_IocCBType*)0)
    {
        ...
        {
            Os_GetSpinlock(&Os_IocMailLock);
            pWorkMailCB = Os_MallocMail(&pIocCB->IocListHead,
                                        &pIocCB->IocPtrTailNode
                                    );
            Os_ReleaseSpinlock(&Os_IocMailLock);
        }
        ...
        if (pWorkMailCB != (Os_IocMsgCBType*)0)
        {
            ...
            if (QUEUED_FIFO == pIocCB->IocQueueCom)
            {
                (void)Os_GetDataFromMemPoolForFIFO(pData,
                                        pIocCB,
                                        pWorkMailCB
                                       );
            }
            ...
            {
                (void)Os_GetDataFromMemPoolForLIFO(pData,
                                        pIocCB,
                                        pWorkMailCB
                                        );
            }
            ...
            {
                Os_GetSpinlock(&Os_IocMailLock);
                (void)Os_ReleaseMail(pWorkMailCB,
                                     &OSIocMsgHeadGlobalPtr
                                    );
                Os_ReleaseSpinlock(&Os_IocMailLock);
            }
            #endif
 ...
 */
/******************************************************************************/
static FUNC(StatusType, OS_CODE) Os_IocCrossReceive(
    Os_IocAbsIdType       absId,
    const Os_IocDataSourceType* pRcvData
)
{
    ...
    if (pIocCBEntry->IocPipelistCnt > (Os_IoclistCntType)0)
    {
        if (CAT1_UNQUEUED == tempSema)
        {
            Os_RetErr = Os_LLReadData(pRcvData,
                                      pIocCBEntry
                                     );
        }
        else /* NOTE!! ++-->CAT2_QUEUED == tempSema */
        {
            Os_RetErr = Os_LLReceiveData(pRcvData,
                                         pIocCBEntry
                                        );
        }
    }
 ...
 */
/******************************************************************************/
FUNC(StatusType, OS_CODE) Os_IocHLReceive(
    Os_IocAbsIdType       absId,
    const Os_IocDataSourceType* pData
)
{
    ...
    if (IOC_E_OK == Os_RetErr)
    {
        ...
        if (INTER_CORE_COM == tempComMark)
        {
            ...
            {
                Os_RetErr = Os_IocInterReceive(absId,
                                               pData
                                              );
            }
        }
        ...
        if (CROSS_CORE_COM == tempComMark)
        {
            ...
            {
                Os_RetErr = Os_IocCrossReceive(absId,
                                               pData
                                              );
            }
        }
 ...
 */
/******************************************************************************/
static FUNC(void, OS_CODE) Os_ClearMailData(
    Os_IocCBType* pIocCB,
    Os_IocCoreComType comMark,
    Os_IocSemanticsType  sema
)
{
    Os_IocMsgCBType* pMailCB;
    Os_IocListNodeType* ptr1;
...
static FUNC(StatusType, OS_CODE) Os_LLEmptyCrossQueue (Os_IocU16Type index)
{
    ...
    if (pIocCBEntry->IocPipelistCnt != (Os_IoclistCntType)0)
    {
        if (CAT1_UNQUEUED == tempSema)
        {
            (void)Os_ClearMailData(pIocCBEntry,
                                   CROSS_CORE_COM,
                                   CAT1_UNQUEUED
                                  );
        }
 ...
 */
/******************************************************************************/
FUNC(StatusType, OS_CODE) Os_IocHLEmptyQueue (Os_IocAbsIdType absId)
{
    StatusType Os_RetErr;

 ...
 *
 *  File         Os_Sprot.c
 *  Brief        AutoSar Protection Managment
 *
 *  Revision History:
 *
 *     1. 2016-03-29: shi.shu; Initial version, add this file.
 *
 */
 /*******************************************************************/
...
FUNC(void, OS_CODE) Os_InitTrustedFunction(void)
{
    ...
    switch (Os_SCB.sysCore)
    {
        ...
        case 0:
            ...
            for (i = 0u; i < CFG_TRUSTED_SYSTEM_SERVICE_MAX_CORE0; i++)
            {
        ...
        case 1:
            ...
            for (i = 0u; i < CFG_TRUSTED_SYSTEM_SERVICE_MAX_CORE1; i++)
            {
                Os_TrustedFuncNestQueue[i] = OS_APPLICATION_INVALID;
            }
            break;
        ...
        case 2:
            ...
            for (i = 0u; i < CFG_TRUSTED_SYSTEM_SERVICE_MAX_CORE2; i++)
            {
                Os_TrustedFuncNestQueue[i] = OS_APPLICATION_INVALID;
            }
            break;
...
FUNC(boolean, OS_CODE) Os_IgnoreService(void)
{
    ...
    if (OS_LEVEL_TASK == Os_SCB.sysOsLevel)
    {
        ...
        || (Os_SCB.sysRunningTCB->taskSuspendAllCount > 0u)
 ...
 */
/********************************************************************/
FUNC(boolean, OS_CODE) Os_AddressWritable
(
...
)
{
    ...
    else
    {
        ...
        {
            if ((Os_TaskStack[TaskID].stackTop >= Address)
                    && (Os_TaskStack[TaskID].stackBottom <= Address))
            {
        ...
        {
            if ((Os_ISR2Stack[Isr2ID].stackTop >= Address)
                    && (Os_ISR2Stack[Isr2ID].stackBottom <= Address))
            {
                Access = OSMEMORY_SET_WRITEABLE(Access);
            }
            else
...
FUNC(boolean, OS_CODE) Os_WrongContext(uint16 AllowedContext)
{
    ...
    {
        if ((OS_CONTEXT_START_CORE == AllowedContext)
                || (OS_CONTEXT_START_NON_AUTOSAR_CORE == AllowedContext))
        {
            Status = TRUE;
        }
    }
...
FUNC(void, OS_CODE) Os_Isr2OccupyIntRes(void)
{
    ...
    pOsICB = &Os_ICB[Os_SCB.sysRunningIsrCat2Id];
    #if (CFG_STD_RESOURCE_MAX > 0)
 ...
 */
/********************************************************************/
FUNC(boolean, OS_CODE) Os_CheckObjAcs
(

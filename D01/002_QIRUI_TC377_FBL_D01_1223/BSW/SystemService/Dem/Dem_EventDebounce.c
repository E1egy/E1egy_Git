        ...
        if ((pTimer->Triggered == TRUE) && (pTimer->IsFreezing == FALSE))
        {
                    ...
                    && ((pTimer->CurStatus == DEM_EVENT_STATUS_PREFAILED)))
            {
                    ...
                    || (DemOperationCycle[pEventCfg->DemOperationCycleRef].DemOperationCycleType == DEM_OPCYC_OBD_DCY))
                {
                	...
                	Dem_GetFreezeFrame(pTimer->IntId, (&(pTimer->Dem_EventBuffer))->FFData);
#endif
#if(DEM_EXTENDED_DATA_CLASS_NUM > 0)/*SWS_Dem_00812]*/
					Dem_GetExtendedData(pTimer->IntId, (&(pTimer->Dem_EventBuffer))->ExtData);
#endif
...
{
    uint16 IntId = (pEventBuffer->ExtId -1);
...
P2CONST(Dem_EventParameterType, AUTOMATIC, DEM_CONST) pEventCfg)
{
    ...
    switch (Status)
    {
        ...
        default: /* DEM_EVENT_STATUS_PREFAILED */
        	{
        		Dem_DebouceProcessPreFailedCounter(pEventBuffer,pCfgCounter,pCounter);
        	}
            break;
    }

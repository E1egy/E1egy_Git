...
**                       Version  Check                                       **
*******************************************************************************/
#define DEM_C_AR_MAJOR_VERSION      4U
#define DEM_C_AR_MINOR_VERSION      2U
#define DEM_C_AR_PATCH_VERSION      2U
#define DEM_C_SW_MAJOR_VERSION      2U
#define DEM_C_SW_MINOR_VERSION      0U
#define DEM_C_SW_PATCH_VERSION      1U

#if (DEM_C_AR_MAJOR_VERSION != DEM_INTERNAL_H_AR_MAJOR_VERSION)
  #error "Dem.c : Mismatch in Specification Major Version"
#endif
#if (DEM_C_AR_MINOR_VERSION != DEM_INTERNAL_H_AR_MINOR_VERSION)
  #error "Dem.c : Mismatch in Specification Major Version"
#endif
#if (DEM_C_AR_PATCH_VERSION != DEM_INTERNAL_H_AR_PATCH_VERSION)
  #error "Dem.c : Mismatch in Specification Major Version"
#endif
#if (DEM_C_SW_MAJOR_VERSION != DEM_INTERNAL_H_SW_MAJOR_VERSION)
  #error "Dem.c : Mismatch in Specification Major Version"
#endif
#if (DEM_C_SW_MINOR_VERSION != DEM_INTERNAL_H_SW_MINOR_VERSION)
  #error "Dem.c : Mismatch in Specification Major Version"
#endif
...
Dem_SetEnableCondictionProcess(void);
...
#include "Dem_MemMap.h"
static FUNC(void, DEM_CODE)
Dem_SetStorageCondictionProcess(void);
#define DEM_STOP_SEC_CODE
#include "Dem_MemMap.h"
#endif
...
#include "Dem_MemMap.h"
static FUNC(void, DEM_CODE)
Dem_EventStatusPrepassedDebounce(
...
#include "Dem_MemMap.h"
static FUNC(void, DEM_CODE)
Dem_EventStatusPrefailedDebounce(
    P2CONST(Dem_EventParameterType, AUTOMATIC, DEM_CONST) pEventCfg,
    P2VAR(Dem_EventRelateInformationType, AUTOMATIC, DEM_VAR) pEvent
);
#define DEM_STOP_SEC_CODE
...
FUNC(void, DEM_CODE) Dem_Init(P2CONST(Dem_ConfigType, AUTOMATIC, DEM_CONST_PBCFG) ConfigPtr)
{
    ...
    P2CONST(Dem_EventParameterType, AUTOMATIC, DEM_CONST) pEventCfg = NULL_PTR;
#endif
...
Dem_GetVersionInfo(Std_VersionInfoType *versionInfo)
{
    ...
    versionInfo->sw_major_version = DEM_C_SW_MAJOR_VERSION;
    versionInfo->sw_minor_version = DEM_C_SW_MINOR_VERSION;
    versionInfo->sw_patch_version =DEM_C_SW_PATCH_VERSION;
    return;
}
#endif
    ...
    )
{
    ...
    if ((pEventCfg->AlgorithmType == DEM_DEBOUNCE_MONITOR_INTERNAL)
    ...
    /*soft filter when pass or prepass,if TF/TNCLSC or TNCTOC is flaged then return e_ok whith no deal*/
    if (((0x00u == DEM_FLAGS_ISSET(pEvent->UdsStatus,DEM_UDS_STATUS_TF))
        ...
        && ((EventStatus == DEM_EVENT_STATUS_PASSED) || (EventStatus == DEM_EVENT_STATUS_PREPASSED)))
    {
        return;
    }
#endif
    if (DemInitState != DEM_STATE_PRE_INIT)/*SWS_Dem_00851*/
    {
        ...
        {/*SWS_Dem_00447 SWS_Dem_00449 SWS_Dem_01091*/
            Dem_DebounceFreeze(EventId);/*In case of Dem-internal debouncing the related fault detection counter will be frozen or reset*/
            return;
        }
#endif
    ...
    )
{
    ...
    if(0x00u == DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_STORAGE_CONDICTION))
    {
        ...
        while ((index < DEM_STORAGE_CONDITION_NUM_BYTE) && (IsReplacement == FALSE))
        {
            ...
            {   /*SWS_Dem_01086*/
                if (((DemStorageConditionGroup[DemPbCfgPtr->DemEventParameter[EventId].DemStorageConditionGroupRef][index] & Mask) != 0u)
                       && (DemStorageCondition[Conditionindex].DemStorageConditionReplacementEventRef != DEM_EVENT_PARAMETER_INVALID)
                       && (EventStatus == DEM_EVENT_STATUS_FAILED))
                {
    ...
    )
{
    ...
    /*SWS_Dem_00676*/
    if ((DemInitState != DEM_STATE_INIT)
        && (DemPbCfgPtr->DemDebounceCounterBasedClass[DemPbCfgPtr->DemEventParameter[EventId].AlgorithmRef].DemDebounceCounterStorage == TRUE))
    {
    ...
    {/*SWS_Dem_00684*/
        if (DemPbCfgPtr->DemEventParameter[EventId].AlgorithmType == DEM_DEBOUNCE_COUNTER_BASED)
        {
            ...
            {
                DemDebounceCounterInfo[DemPbCfgPtr->DemEventParameter[EventId].AlgorithmIndex].InternalDebounceCounter = 0;
                DemDebounceCounterInfo[DemPbCfgPtr->DemEventParameter[EventId].AlgorithmIndex].CurStatus = DEM_EVENT_STATUS_PREPASSED;
            }
#endif
        }
        else if (DemPbCfgPtr->DemEventParameter[EventId].AlgorithmType == DEM_DEBOUNCE_TIME_BASE)
        {
            ...
            if(DemPbCfgPtr->DemEventParameter[EventId].AlgorithmIndex < DEM_DEBOUNCE_TIME_BASED_EVENT_NUM)
            {
                ...
                DemDebounceTimerInfo[DemPbCfgPtr->DemEventParameter[EventId].AlgorithmIndex].CurStatus = DEM_EVENT_STATUS_PREPASSED;
            }
#endif
        }
        else
        ...
        switch(DTCFormat)
        {
            case DEM_DTC_FORMAT_OBD:
                if ((DemPbCfgPtr->DemDTC[pEventCfg->DemDTCRef].DemObdDTCRef != DEM_OBD_DTC_INVALID)
                        && (DemPbCfgPtr->DemObdDTC[DemPbCfgPtr->DemDTC[pEventCfg->DemDTCRef].DemObdDTCRef].DemDtcValue != DEM_OBD_DTC_CFG_INVALID))
                {
                    *DTCOfEvent = DemPbCfgPtr->DemObdDTC[DemPbCfgPtr->DemDTC[pEventCfg->DemDTCRef].DemObdDTCRef].DemDtcValue;
                    res = E_OK;
                }
            break;
            ...
            case DEM_DTC_FORMAT_J1939:
                if ((DemPbCfgPtr->DemDTC[pEventCfg->DemDTCRef].DemObdDTCRef != DEM_OBD_DTC_INVALID)
                        && (DemPbCfgPtr->DemObdDTC[DemPbCfgPtr->DemDTC[pEventCfg->DemDTCRef].DemObdDTCRef].DemJ1939DTCValue != DEM_J1939_DTC_CFG_INVALID))
                {
                    *DTCOfEvent = DemPbCfgPtr->DemObdDTC[DemPbCfgPtr->DemDTC[pEventCfg->DemDTCRef].DemObdDTCRef].DemJ1939DTCValue;
                    res = E_OK;
                }
            break;
            default:
...
)
{
    ...
    pMemDest = DemPbCfgPtr->DemDTCAttributes[DemPbCfgPtr->DemDTC[dtcRef].DemDTCAttributesRef]
    ...
    if (*pMemDest != DEM_MEM_DEST_INVALID)
    {
        ...
        while (index < DEM_MEM_DEST_MAX_NUM_OF_DTC)
        {
            ...
            {
                if ((Entry->EventId != 0u) && (DemPbCfgPtr->DemEventParameter[Dem_GetEventInternalId(Entry->EventId)].DemDTCRef == dtcRef))
                {
    ...
    iloop = 0;
    pMemDest = DemPbCfgPtr->DemDTCAttributes[DemPbCfgPtr->DemDTC[DemPbCfgPtr->DemEventParameter[Dem_GetEventInternalId(EventId)].
                                                                 DemDTCRef].DemDTCAttributesRef].DemMemoryDestinationRef;
    while (iloop < DEM_MEM_DEST_MAX_NUM_OF_DTC)
    {
        if (*pMemDest != DEM_MEM_DEST_INVALID)
        {
            if((*pMemDest != DEM_DTC_ORIGIN_PERMANENT_MEMORY) && (*pMemDest != DEM_DTC_ORIGIN_MIRROR_MEMORY))/*SWS_Dem_01062 */
            {
...
static FUNC(void, DEM_CODE) Dem_SetEnableCondictionProcess(void)
{
    ...
    while (iloop < DEM_EVENT_PARAMETER_NUM)
    {
        pEvent = Dem_GetEventInfo(iloop);
        IsFulfilled = TRUE;
        if (DemPbCfgPtr->DemEventParameter[iloop].DemEnableConditionGroupRef
            != DEM_ENABLE_CONDITION_GROUP_INVALID)
        {
                pGroup = &DemEnableConditionGroup[DemPbCfgPtr->DemEventParameter[iloop]
                    .DemEnableConditionGroupRef][0];
                IsFulfilled = Dem_CheckCondictionFulfilled(
                        DemEnableConditionStatus, pGroup,
                        DEM_ENABLE_CONDITION_NUM_BYTE);
        }
        if (IsFulfilled == TRUE)
        {
...
static FUNC(void, DEM_CODE) Dem_SetStorageCondictionProcess(void)
{
    ...
    while (iloop < DEM_EVENT_PARAMETER_NUM)
    {
        ...
        IsReplacement = FALSE;
        IsFulfilled = TRUE;
        if (DemPbCfgPtr->DemEventParameter[iloop].DemStorageConditionGroupRef
            != DEM_STORAGE_CONDITION_GROUP_INVALID)
        {
                pGroup = &DemStorageConditionGroup[DemPbCfgPtr->DemEventParameter[iloop]
        ...
        if (IsFulfilled == TRUE)
        {
            if(0x00u == DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_STORAGE_CONDICTION))
            {
                ...
                while ((index < DEM_STORAGE_CONDITION_NUM_BYTE) && (IsReplacement == FALSE))
                {
                    ...
                    while ((Conditionindex < DEM_STORAGE_CONDITION_NUM) && (IsReplacement == FALSE) && (Counter < 8u))
                    {
                        if (((DemStorageConditionGroup[DemPbCfgPtr->DemEventParameter[iloop].DemStorageConditionGroupRef][index] & Mask) != 0u)
                            && (DemStorageCondition[Conditionindex].DemStorageConditionReplacementEventRef != DEM_EVENT_PARAMETER_INVALID))
                        {
                            (void)Dem_EventQueueAdd(Dem_GetEventExternalId(DemStorageCondition[Conditionindex].DemStorageConditionReplacementEventRef),
                                    DEM_EVENT_STATUS_PASSED);
                            /* SWS_Dem_01113  Monitor re-initialization  */
                            if (DemPbCfgPtr->DemEventParameter[(iloop)].DemCallbackInitMForE != NULL_PTR)
                            {
                                DemPbCfgPtr->DemEventParameter[(iloop)].DemCallbackInitMForE(DEM_INIT_MONITOR_STORAGE_REENABLED);
                            }
                            IsReplacement = TRUE;
                        }
                        Counter++;

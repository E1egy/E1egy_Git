 ...
 *  V1.0.0       2020-04-08  qinchun.yang    R19_11 BswM initial version.
...
)
{
    ...
    else if ((CurrentPncMode != COMM_PNC_NO_COMMUNICATION)
            && (CurrentPncMode != COMM_PNC_PREPARE_SLEEP)
            && (CurrentPncMode != COMM_PNC_READY_SLEEP)
            && (CurrentPncMode != COMM_PNC_REQUESTED)
        )
    {
        ret = E_NOT_OK;
        (void)Det_ReportError(
...
)
{
    ...
    else if ((CurrentState != (LinSM_ModeType)LINSM_FULL_COM)
            && ((CurrentState != (LinSM_ModeType)LINSM_NO_COM))
            )
    {
        ret = E_NOT_OK;
        (void)Det_ReportError(
...
)
{
    ...
    else if ((MultiBlockRequest != NVM_READ_ALL)
            && (MultiBlockRequest != NVM_WRITE_ALL)
            && (MultiBlockRequest != NVM_VALIDATE_ALL)
            && (MultiBlockRequest != NVM_FIRST_INIT_ALL)
...
)
{
    ...
    else
    {
        ...
        if ((boolean)TRUE == result)
        {
            ...
            {
                if (requestingUser ==
                        BswM_RuntimeStatus.bswmPartLCfgs[partIdx].modeRqstLCfg->genRqstUserRef[userIdx])
                {
                    break;
                }
            }
...
)
{
    ...
    else if ((CurrentClientState != SD_CLIENT_SERVICE_DOWN)
            && (CurrentClientState != SD_CLIENT_SERVICE_AVAILABLE)
        )
    {
        ret = E_NOT_OK;
        (void)Det_ReportError(
...
)
{
    ...
    else if ((ConsumedEventGroupState != SD_CONSUMED_EVENTGROUP_DOWN)

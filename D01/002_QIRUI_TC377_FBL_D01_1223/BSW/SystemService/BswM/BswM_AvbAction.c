...
)
{
    ...
    actionLCfgPtr = BswM_RuntimeStatus.bswmPartLCfgs[partIdx].acItemsLCfg;
    EcuM_SetState(
        actionLCfgPtr->ecuMStateSwitchActions[actionIdx].ecuMState);
    return ret;
}
...
)
{
    ...
    actionLCfgPtr = BswM_RuntimeStatus.bswmPartLCfgs[partIdx].acItemsLCfg;
    ret = FrSm_AllSlots(
            actionLCfgPtr->frSMAllSlotsActions[actionIdx].channel);
    return ret;
}
...
#if (BSWM_NM_ENABLED == STD_ON)
/*ACtion: NM control*/
FUNC(Std_ReturnType, BSWM_CODE) BswM_DoNMControl
(
    ApplicationType partIdx,
    uint8 actionIdx
)
...
)
{
    ...
    {
        triRet = Com_TriggerIPDUSend(
                actionLCfgPtr->triggerIpduSendActionLCfgs[actionIdx].pduIds[index]
        );
...
)
{
    ...
    actionLCfgPtr = BswM_RuntimeStatus.bswmPartLCfgs[partIdx].acItemsLCfg;
    ret = actionLCfgPtr->rteSwitchActions[actionIdx].rteSwitchFctPtr(
            actionLCfgPtr->rteSwitchActions[actionIdx].mode);
    return ret;
}
...
)
{
    ...
    actionLCfgPtr = BswM_RuntimeStatus.bswmPartLCfgs[partIdx].acItemsLCfg;
    ret = actionLCfgPtr->schMSwitchActions[actionIdx].schMSwitchFctPtr(
            actionLCfgPtr->schMSwitchActions[actionIdx].mode);
    return ret;
}
...
FUNC(Std_ReturnType, BSWM_CODE) BswM_DoTimerControl
(
    ApplicationType partIdx,
    uint8 actionIdx
)

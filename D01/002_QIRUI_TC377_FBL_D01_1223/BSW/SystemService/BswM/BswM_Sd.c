...
)
{
    ...
    if ((boolean)TRUE == result)
    {
        evGruCurState = BswM_RuntimeStatus.bswmPartPCCfgs[partIdx].modeRqstPortRunPtr->sdConsumedEvGrpCurrStaStatus[evIdx];
    }
...
)
{
    ...
    if ((boolean)TRUE == result)
    {
        evHandlerCurState = BswM_RuntimeStatus.bswmPartPCCfgs[partIdx].modeRqstPortRunPtr->sdEvHdlerCurrStaStatus[handlerIdx];
    }

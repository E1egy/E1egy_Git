...
#endif /*#if (BSWM_CANSM_ENABLED == STD_ON)*/
...
BswM_ChkBswMInitStatus(
    uint8 sid
);

#if (BSWM_MULTI_PARTITION_ENABLED == STD_ON)
FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkParRestart(
    void
);
#endif /*(BSWM_MULTI_PARTITION_ENABLED == STD_ON)*/
...
#if (BSWM_DCM_ENABLED == STD_ON)
FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkDcmUpdate(
    void
);

FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkDcmModeCurState(
...
#if (BSWM_ECUM_ENABLED == STD_ON)
FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkEcuMCurWakeUp(
    EcuM_WakeupStatusType state
);

FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkRequestedState(
    EcuM_StateType State,
    EcuM_RunStatusType CurrentStatus
);

FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkEcuMCurState(
    void
);
#endif /*(BSWM_ECUM_ENABLED == STD_ON)*/
...
#if (BSWM_NVM_ENABLED == STD_ON)
FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkCurNvmBlockMode(
    NvM_RequestResultType CurrentBlockMode
);

FUNC(Std_ReturnType, BSWM_CODE)
BswM_DetChkCurNvmJobMode(
...
BswM_GetBswModeIndStatus
(
    BswM_ModeGroupType bswChIdx
);
#endif /*(BSWM_SCHM_ENABLED == STD_ON)*/
...
BswM_GetCanSmIcomCfgId
(
    NetworkHandleType icomIdx
);
#endif /*(BSWM_CANSM_ICOM_ENABLED == STD_ON)*/

/*Get cansm indicate status*/
FUNC(CanSM_BswMCurrentStateType, BSWM_CANSM_CODE)
BswM_GetCanSmIndStatus
(
    NetworkHandleType cansmChIdx
);
...
BswM_GetComMIndStatus
(
    NetworkHandleType comChIdx
);
#endif /*(BSWM_COMM_ENABLED == STD_ON)*/
...
BswM_GetDcmCurModeStatus
(
    NetworkHandleType dcmChIdx
);
#endif /*(BSWM_DCM_ENABLED == STD_ON)*/
...
BswM_GetEcuMWakeSrcStatus
(
    EcuM_WakeupSourceType wkSrcIdx
);

FUNC(EcuM_RunStatusType, BSWM_ECUM_CODE)
BswM_GetEcuMRunRqstStatus
(
    EcuM_StateType rqstIdx
);
...
BswM_GetEthIfGruStateStatus
(
    EthIf_SwitchPortGroupIdxType ethIdx
);
#endif /*(BSWM_ETHIF_ENABLED == STD_ON)*/
...
BswM_GetEthSmIndStatus
(
    NetworkHandleType ethsmChIdx
);
#endif /*(BSWM_ETHSM_ENABLED == STD_ON)*/
...
BswM_GetFrSmIndStatus
(
    NetworkHandleType frsmChIdx
);
#endif /*(BSWM_FRSM_ENABLED == STD_ON)*/
...
BswM_GetJ1939ChBroadStatus
(
    NetworkHandleType j1939ChIdx
);
#endif /*(BSWM_J1939DCM_ENABLED == STD_ON)*/
...
BswM_GetLinSmIndStatus
(
    NetworkHandleType linsmChIdx
);

/*Get linsm current schedule*/
FUNC(LinIf_SchHandleType, BSWM_FRSM_CODE)
BswM_GetLinSmCurSchedule
(
    NetworkHandleType linsmChIdx
);
#endif /*(BSWM_LINSM_ENABLED == STD_ON)*/
...
BswM_GetLinTpRqstMode
(
    NetworkHandleType linTpChIdx
);
#endif /*(BSWM_LINTP_ENABLED == STD_ON)*/
...
FUNC(NvM_RequestResultType, BSWM_NVM_CODE)
BswM_GetCurNvmJobMode
(
    NvM_MultiBlockRequestType jobIdx
);
#endif /*(BSWM_NVM_ENABLED == STD_ON)*/
...
FUNC(BswM_ModeType, BSWM_CANSM_CODE)
BswM_GetSwcModeIndStatus
(
    BswM_ModeGroupType swcChIdx
);

/*Get swc request status*/
FUNC(BswM_ModeType, BSWM_CANSM_CODE)
BswM_GetSwcRqstStatus
(
    BswM_ModeGroupType swcChIdx
);
...
#if (BSWM_TIMER_ENABLED == STD_ON)
/*BswM Timer*/
FUNC(void, BSWM_CODE) BswM_InitBswMTimer
(
    void
);

FUNC(BswM_TimerEnumType, BSWM_CODE) BswM_GerTimerStatus
(
